<?php	
  ...
  if($vew_data->endpoint==''){
    $vew_data->endpoint='zcutp1_tin';
  ...
  //$lv_reqflddef = $vew_data->fldrec;
	...
	$lv_prgcod ='EVL';
	$actbuf= $vew_actcod;
	if($vew_actcod=='hltpatevlxol'){
  	$vew_actcod = $vew_data->evlcod!=''?'03':'01';
  }
	$vew_actcod = $vew_actcod =='hltpatevlxol02'?'02':$vew_actcod;
  ...
  include_once('_library.frm'); 
    ...
    $vew_data->infprg=strtoupper($vew_doc->getTagValue($lv_buf,'infprg'));
    $vew_data->infexe=strtoupper($vew_doc->getTagValue($lv_buf,'infexe'));
    $vew_data->evlcncmtv2=strtoupper($vew_doc->getTagValue($lv_buf,'evlcncmtv2'));
  ...
  $vew_tbl['clsL'] = array('pos'=>'L','per'=>($vew_data->hhcc=='X'), 'ttl'=>$vew_lang->close, 'id'=>'btncls','icn'=>'fas fa-arrow-left', 'css'=>'btn navbar-btn tmss-navbar-btn arrow-left', 'acc'=>$lv_sec.'_fncbckext();');
	//$vew_tbl['clsL'] = array('pos'=>'L','per'=>true, 'ttl'=>$vew_lang->close, 'id'=>'btncls','icn'=>'fas fa-arrow-left', 'css'=>'btn navbar-btn tmss-navbar-btn arrow-left', 'acc'=>'tmssTabSecCls( $("#'.$lv_sec.'") );');

  $vew_tbl['prnR'] =array('per'=>$vew_sec->hasPermission('HLT','EVL','05') && $vew_data->docsts=='A','id'=>'btnprn');
  $vew_tbl['sveL'] =array('per'=>!$vew_readonly,'acc'=>$lv_sec.'_fnc({action: '.chr(39).'hltpatevlxolsve'.chr(39).'});'); 
  $vew_tbl['sveR'] =array('per'=>!$vew_readonly,'acc'=>$lv_sec.'_fnc({action: '.chr(39).'hltpatevlxolsve'.chr(39).'});'); 
  $vew_tbl['modL'] = array('pos'=>'L','per'=>($vew_sec->hasPermission($lv_mdlcod, $lv_prgcod,'02') && $vew_actcod =='03' && $vew_data->evlinfprc =='1'), 'ttl'=>$vew_lang->modify, 'id'=>'','icn'=>'fas fa-pencil-alt', 'css'=>'btn navbar-btn tmss-navbar-btn tmssAlwaysEnabled tmssHiddeOnEdit tmss-desk-btn', 'acc'=>$lv_sec.'_fnc({action: '.chr(39).'hltpatevlxol02'.chr(39).'});' );
  $vew_tbl['del']  =array('per'=>$vew_sec->hasPermission('HLT','EVL','04') && $vew_data->evlcod!='','acc'=>$lv_sec.'_fnc({action: '.chr(39).'hltpatevlxoldel'.chr(39).'});');
  if($vew_data->evllon!=''){
...
<section id="<?= $lv_sec; ?>" data-title="<?= $lv_title; ?>">
          ...
          <div class="row">
            ...
            <div class="card" id="evlnoinf_div">
              <div class="card-body tmss-card-body-edit">
                 <?php
                    ...
                    echo vew_boot($lv_col210,array('label'=>$vew_lang->comments, 'input'=>gethtml('evlcnccmt','doccmt5x50',$vew_data->evlsub,$lvDisableOnModify) ));              
                  ?>                
              </div><!--/card evlnoinf_div -->
            </div>
            <div class="card" id="evlyesinf_div">
              ...
              <div class="card-body tmss-card-body-edit">
              	...
              	<?= vew_boot($lv_col210,array('label'=>'Dosis total', 'input'=>gethtml('matdos','doccmt1x50',$vew_data->matdos,$lv_default) ));?>
              	<?php
                  if(!$vew_readonly){
                  ...
                  <tbody id="tbldatbdy">
                    <?php
                      foreach($vew_data->evlmat as $lo_matrow){
                        echo '<tr name="dlvrow" >';
                        echo '<td><strong>'.$lo_matrow['mattxt'].'</strong> Cant. Viales: <strong>'.$lo_matrow['matqty'].' '.$lo_matrow['matuntcod'].'</strong><br>Lote: <strong>'.$lo_matrow['matbchcodext'].'</strong>&nbsp; vto:<strong>'.date('d/m/Y', strtotime($lo_matrow['matbchduedte'])).'</strong></td>';
                        //var_dump($lo_matrow['matbchduedte']);
                ...
                <?= vew_boot($lv_col233,array('label1'=>'Inf. Programada', 'input1'=>gethtml('infprg','docnum0800',$vew_data->infprg,$lvDisableOnModify) ,'input2'=>''));?>
                <?= vew_boot($lv_col233,array('label1'=>'Inf. realizadas', 'input1'=>gethtml('infexe','docnum0800',$vew_data->infexe,$lvDisableOnModify) ,'input2'=>''));?>
                
                <div id="<?php echo $lv_sec; ?>_mtv2">
                  <?= vew_boot($lv_col210,array('label'=>$vew_lang->motive, 
                                                'input'=>gethtml('evlcncmtv2',$vew_data->evlcncmtvlst2,
                               $vew_data->evlcncmtv2,$lvDisableOnModify) ));
                  ?>
                </div>
                <hr/>
                ...
                <?= vew_boot($lv_col210,array('label'=>$vew_lang->comments, 'input'=>gethtml('evlevl','doccmt5x50',$vew_data->evlevl,$lvDisableOnModify) ));?>
              </div><!-- /Card Boby -->
            </div><!-- /Card evlyesinf_div -->
          </div><!-- /Row -->
        </div> <!-- /_tab001 -->
      </div> <!-- /tab-content --> 
    </div> <!-- /container-fluid -->
  ...
  <script>
    ...
    var lv_infprg =document.querySelector("#<?php echo $lv_sec; ?> #infprg")
    var lv_infexe =document.querySelector("#<?php echo $lv_sec; ?> #infexe")
    function <?= $lv_sec; ?>_setMtv2(){
      infprg= parseInt((lv_infprg.value==''?'0':lv_infprg.value))
      infexe= parseInt((lv_infexe.value==''?'0':lv_infexe.value))
    	console.log(infprg,infexe)
      var lv_mtv2 = document.getElementById("<?php echo $lv_sec; ?>_mtv2")
      lv_mtv2.classList.add("hidden");
      if(infprg>infexe){
        lv_mtv2.classList.remove("hidden");
      }
    }
    lv_infprg.addEventListener("change", <?= $lv_sec; ?>_setMtv2);
    lv_infexe.addEventListener("change", <?= $lv_sec; ?>_setMtv2);
  </script>
  <script>
    // Cargar
      ...
      BootstrapDialog.show({
            ...
            buttons: [{
                  ...
                  action:function(dialogItself){
                    ...
                    var lv_row = "<tr name='dlvrow' data-matcod= '" + lv_matcod + "' data-mattxt= '" + lv_mattxt + "' data-matqty= '" + lv_matqty + "' data-matbchcodext= '" + lv_matbchcodext + "' data-matbchduedte= '" + lv_matbchduedte + "'>"
                                 ...
                                 +"<td><a  class='card-icon' onclick='<?= $lv_sec; ?>_removeRowEdit($(this));'><i class='fas fa-trash-alt'></i></a></td>"
                                 +"</tr>";
  ...
  <script>
    function <?= $lv_sec; ?>_fncbckext(lp_prm){
      ...
      if ( gv_<?php echo $lv_sec; ?>_last_action=="hltpatevlxol02") {
    ...
    function <?php echo $lv_sec; ?>_fncext( lp_prm ) {
        ...
        /*
				if (lp_prm["action"]=="hltpatevlxolsve"){if(!tmssCheckRequiredFields($("#<?php echo $lv_sec; ?>_frm"))){return false;}}
        ...
        */
        //lp_prm["action"]=lp_prm["action"]=='00'?'hltpatevlxolsve':lp_prm["action"];
				if (lp_prm["action"]=="hltpatevlxolsve" ) {
          if ($("#<?php echo $lv_sec; ?> #evlinfprc").prop("value")=="1"){
            ...
            if ( lv_matdos==""  && lv_fldrec.includes("matdos") ) {
            ...
            $("#<?=$lv_sec; ?> #infprg").parentsUntil(".tmss-form-group").parent().removeClass("has-error");
						if($("#<?=$lv_sec; ?> #infprg").val()=="" && lv_fldrec.includes("infprg")){
              $("#<?=$lv_sec; ?> #infprg").parentsUntil(".tmss-form-group").parent().addClass("has-error");
              lv_err++;
            }
            $("#<?=$lv_sec; ?> #infexe").parentsUntil(".tmss-form-group").parent().removeClass("has-error");
						if($("#<?=$lv_sec; ?> #infexe").val()=="" && lv_fldrec.includes("infexe")){
              $("#<?=$lv_sec; ?> #infexe").parentsUntil(".tmss-form-group").parent().addClass("has-error");
              lv_err++;
            }
            var lv_mtv2 = document.querySelector("#<?=$lv_sec; ?> #evlcncmtv2")
            infprg= parseInt((lv_infprg.value==''?'0':lv_infprg.value))
            infexe= parseInt((lv_infexe.value==''?'0':lv_infexe.value))
            $("#<?=$lv_sec; ?> #evlcncmtv2").parentsUntil(".tmss-form-group").parent().removeClass("has-error");
            if(infprg>infexe && lv_mtv2.value==''){ 
              $("#<?=$lv_sec; ?> #evlcncmtv2").parentsUntil(".tmss-form-group").parent().addClass("has-error");
              lv_err++;
            }
            ...
            tmssCallProcessFile("?prg=<?= $vew_data->endpoint; ?>&act=hltpatevlxolsve", lo_dat, <?= $lv_sec; ?>_fncbckext);
            // update file data
            return false;
          }else{
