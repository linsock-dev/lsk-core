<?php	
  ...
  $vew_tbl['sveL'] =array('per'=>!$vew_readonly,'acc'=>$lv_sec.'_fnc({action: '.chr(39).'zcuclh_frmact02_00'.chr(39).'});'); 
  $vew_tbl['sveR'] =array('per'=>!$vew_readonly,'acc'=>$lv_sec.'_fnc({action: '.chr(39).'zcuclh_frmact02_00'.chr(39).'});'); 
  $vew_tbl['del']  =array('per'=>$vew_sec->hasPermission('HLT','EVL','04') && $vew_data->evlcod!='','acc'=>$lv_sec.'_fnc({action: '.chr(39).'zcuclh_frmact02x4'.chr(39).'});');
  if($vew_data->evllon!=''){
              ...
              <div class="card-body tmss-card-body-edit">
                ...
                <div id="grp_patprsrlstxt">
                  ...
                  echo vew_boot($lv_col39, array('label'=>'Medico solicitante del sevicio', 
                                                  'input1'=>vew_boot(	array('style'=>'search', 'readonly'=>($vew_data->plnvew=='plnprs'?true:$vew_readonly)), 
                                                                      array('input'=>gethtml('patprsrlstxt', 'typeahead', $vew_data->patprsrlstxt,($vew_data->patprsrlstxt==''?$lv_always_disabled:$lv_default)) )) ));
  ...
  <script>
    ...
    function <?php echo $lv_sec; ?>_fncext( lp_prm ) {
			...
			if (lp_prm["action"]=="prn") {
				...
				if (lp_prm["action"]=="zcuclh_frmact02x4") {
					BootstrapDialog.confirm({
						title: 'Borrar Evolucion',
						message: '¿Desea borrar el documento?',
						type: BootstrapDialog.TYPE_WARNING,
						callback: function(result) {
							if(result) { <?php echo $lv_sec; ?>_fnc({action: "zcuclh_frmact02_04"}); }
						}
					});
					return false;
				}
        debugger;
				if (lp_prm["action"]=="zcuclh_frmact02_00" ) {
          if ($("#<?php echo $lv_sec; ?> #evlinfprc").prop("value")=="1"){
            ...
            tmssCallProcessFile("?prg=zcuclh&act=zcuclh_frmact02_00", lo_dat, <?= $lv_sec; ?>_fncbckext);
            // update file data
            return false;
          }else{
