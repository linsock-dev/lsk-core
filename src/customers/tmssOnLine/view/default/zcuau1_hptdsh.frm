<?php
  ...
  $lv_lnk = '?prg=zcuau1_hpt';
	...
	$lv_prstbl = ($vew_sec->hasPermission('ZCU','AU1','03')?true:false);
...
<section id="<?= $lv_sec; ?>" data-title="<?= $lv_title; ?>">
    ...
    <div class="container-fluid">
        ...
        <?php if ($vew_sec->hasPermission($lv_mdlcod, $lv_prgcod,'01')) { ?><a href="#" id="btnnew" data-prscod="<?= $vew_data->prscod; ?>" data-spccod="<?= $vew_data->spccod; ?>" class="btn navbar-btn tmss-navbar-btn tmssAlwaysEnabled tmss-desk-btn" title="<?= $vew_lang->new; ?>"><i class="fas fa-file"></i><span class="d-none d-sm-inline"> <?= $vew_lang->new; ?></span></a><?php } ?>
			...
			<ul class="nav navbar-right btn-toolbar tmss-navbar-right">
				<!--Filtrar-->
				<?php if ($vew_sec->hasPermission($lv_mdlcod,$lv_prgcod,'03') ) { ?><a href="#" id="btnflt001" class="btn navbar-btn tmssAlwaysEnabled tmss-navbar-btn" title="<?= $vew_lang->filter; ?>"><span class="fas fa-filter"></span><span id="fltcnt" class="badge"></span></a><?php } ?>
  ...
  <form method="POST" class="form-horizontal tmss-form-horizontal" id="<?= $lv_sec; ?>_frm">
		...
		<div class="container-fluid">
			<div class="row">
					...
					<div class="row">
						...
						<div class="col-sm-6">
							<div class="dashboard-box2">
								...
								<div class="dashboard-info">
									<label class="dashboard-label">Pacientes Tratados</label><br>
						...
						<div class="col-sm-6">
							<div class="dashboard-box2">
								...
								<div class="dashboard-info">
									<label class="dashboard-label">Pacientes Totales</label><br>
						...
						<div class="col-sm-6">
							<div class="dashboard-box2">
								...
								<div class="dashboard-info">
									<label class="dashboard-label">Profesionales</label><br>
				...
				<div class="col-sm-7">
					...
					<div class="text-center"><h1 style="margin: 10px !important; padding: 10px !important">Hepatitis C</h1></div>
					...
					<div class="col-xs-12 col-sm-12 col-md-12">
						<table class="table table-condensed table-bordered table-striped table-hover" id="divtblprs">
							<thead>
								<tr><th>Profesional</th><th>Pacientes</th><th>Ult.Seg.</th></tr>
							</thead>
							<tbody>
							</tbody>
						</table>
	...
	<script>
		...
		$("#<?= $lv_sec; ?> #btnflt001").on("click",function(e){
			e.preventDefault();
		...
		function <?= $lv_sec; ?>_GridRefresh001(lp_flt) {
			...
			tmssCallProcess("?prg=zcuau1_hpt&act=28", lv_pstdat, function(data){
				...
				$("#<?= $lv_sec; ?> #divtblpat tr[name='patrow']").on("click", function(e){
					var lv_pst_dat = [{name:"spccod",value: $(this).data("spccod")},
														{name:"prscod",value: $(this).data("prscod")},
														{name:"patcod",value: $(this).data("patcod")}
														];
					tmssLink("index.php?prg=zcuau1_hpt&act=02&prm_patcod="+$(this).data("patcod")+"&prm_prscod="+$(this).data("prscod")+"&prm_spccod="+$(this).data("spccod"), [{target: "_new_section",post_data: lv_pst_dat}] );
					e.preventDefault();
					e.stopPropagation();
				});
				<?php } ?>
				...
				$("#<?= $lv_sec; ?> #divtblprs tr[name='prsrow']").on("click", function(e){
					...
					tmssCallProcess("?prg=zcuau1_hpt&act=18", lv_pstdat, function(data) {
						BootstrapDialog.show({
							...
							onshow: function(dialog) {
								...
								$(lv_frm).find("#patlst tbody tr").on("click", function(e) {
									...
									tmssLink("?prg=zcuau1_hpt&act=02&prm_patcod="+$(this).data("patcod"), [{target: "_new_section",post_data: lv_pst_dat}] );
									e.stopPropagation();
									e.preventDefault();
									dialog.close();
								});
		...
		$("#divtblpat thead tr th").on("click", function(e){
			e.preventDefault();
	...
	<script>
		$("#<?= $lv_sec; ?> #btnnew").on("click", function(e){
			...
			tmssLink("index.php?prg=zcuau1_hpt&act=01&prm_prscod="+$(this).data("prscod")+"&prm_spccod="+$(this).data("spccod"), [{target: "_new_section",post_data: lv_pst_dat}] );
			e.preventDefault();
			e.stopPropagation();
		});
		...
		$("#<?= $lv_sec; ?> #btndwn001").on("click", function(e){
			BootstrapDialog.confirm({
				title: "Descargar",
				message:"Desea descargar la planilla de INFORMACI&Oacute;N GENERAL ?",
				type: BootstrapDialog.TYPE_PRIMARY,
				callback: function(result){
					if(result){	window.open("index.php?prg=zcuau1_hpt&act=r1"); }
				}
			});
			e.preventDefault();
			e.stopPropagation();
