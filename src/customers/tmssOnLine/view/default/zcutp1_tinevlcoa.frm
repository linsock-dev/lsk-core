<?php	
	...
	$lv_prgcod ='EVL';
	$actbuf= $vew_actcod;
	if($vew_actcod=='evlcoa'){
  	$vew_actcod = $vew_data->evlcod!=''?'03':'01';
  }
	$vew_actcod = $vew_actcod =='evlcoa02'?'02':$vew_actcod;
  ...
  $vew_tbl['sveR'] =array('per'=>!$vew_readonly,'acc'=>$lv_sec.'_fnc({action: '.chr(39).'evlcoa00'.chr(39).'});');
	$vew_tbl['modL'] = array('pos'=>'L','per'=>($vew_sec->hasPermission($lv_mdlcod, $lv_prgcod,'02') && $vew_actcod =='03' && $vew_data->evlinfprc =='1'), 'ttl'=>$vew_lang->modify, 'id'=>'','icn'=>'fas fa-pencil-alt', 'css'=>'btn navbar-btn tmss-navbar-btn tmssAlwaysEnabled tmssHiddeOnEdit tmss-desk-btn', 'acc'=>$lv_sec.'_fnc({action: '.chr(39).'evlcoa02'.chr(39).'});' );
          ...
          <div class="row">
            ...
            <div class="card" id="evlyesinf_div">
              ...
              <div class="card-body tmss-card-body-edit">
                ...
                <table id= "tbldat" class="table">
                  ...
                  <tbody id="tbldatbdy">
                    ...
                    foreach($vew_data->evlmat as $lo_matrow){
                      ...
                      $lv_matrow.=$lo_matrow['matbchduedte']==''?'':('vto:<strong>'.date('d/m/Y', strtotime($lo_matrow['matbchduedte'])).'</strong>');
                      echo $lv_matrow;
                      echo '</td>';
                ...
                <input type="hidden" id="evlmatdel" name="evlmatdel" class="hidden" value="">
                <!--<div class ="row"> -->
                  <?php
                    echo vew_boot($lv_col210,array('label'=>'M&eacute;dico tratante', 'input'=>gethtml('evlmtt','doccmt1x50',$vew_data->evlmtt,$lvDisableOnModify) ));
                    echo vew_boot($lv_col210,array('label'=>'Lugar Atenci&oacute;n', 'input'=>gethtml('evllug','doccmt1x50',$vew_data->evllug,$lvDisableOnModify) ));
                  ?>
                  <div class="form-group tmss-form-group">
                    <label class="col-sm-2 control-label"><?php echo 'Patolog&iacute;a'; ?></label>
                    <div class="col-sm-4"><?php echo gethtml('evlpa1','doccmt1x50',$vew_data->evlpa1,$lvDisableOnModify); ?></div>
                    <label class="col-sm-2 control-label">Otras</label>
                    <div class="col-sm-4"><?php echo gethtml('evlpa2','doccmt1x50',$vew_data->evlpa2,$lvDisableOnModify) ; ?></div>
                  </div>
                  <?php
                  echo vew_boot($lv_col210,array('label'=>'Apl. enfermero?', 'input'=>gethtml('aplenf','yesno',$vew_data->aplenf,$lvDisableOnModify) ));
                  ?>
                ...
                <?= vew_boot($lv_col210,array('label'=>$vew_lang->evolution, 'input'=>gethtml('evlevl','doccmt5x50',$vew_data->evlevl,$lvDisableOnModify) ));?>          
              </div><!-- /Card Boby -->
            </div><!-- /Card -->           
          </div><!-- /Row -->
  ...
  <script>
    // IMPRIMIR
			$("#<?php echo $lv_sec; ?> #btnprn").on("click",function(e){
				window.open("?prg=<?= $vew_data->endpoint; ?>&act=hltpatevlprn&prm_evlcod=<?php echo $vew_data->evlcod; ?>");
			});
  </script>
  <script>
    // agregar archivos
  ...
  <script>
    function <?= $lv_sec; ?>_fncbckext(lp_prm){
      ...
      */
      ...
      if ( gv_<?php echo $lv_sec; ?>_last_action=="evlcoa02") {
    ...
    function <?php echo $lv_sec; ?>_fncext( lp_prm ) {      
          ...
          if ($("#<?php echo $lv_sec; ?> #evlinfprc").prop("value")=="1"){
            ...
            for (var i=0; i<lo_datarr.length; i++) {
              const matlstdata= $(lo_datarr[i]);
              lv_arr.push({	"evlmatcod":"",
                            "matcod":matlstdata.data("matcod"),
                            "mattxt":matlstdata.data("mattxt"),
                            "matqty":matlstdata.data("matqty"),
                            "matuntcod":"UN",
                            "matbchcodext":matlstdata.data("matbchcodext"),
                            "matbchduedte":matlstdata.data("matbchduedte"),
                            "atrstrtme":"",
                            "atrendtme":"",
                            "atradvrea":""
                          });
            ...
            if("<?=$vew_data->custxt; ?>"!="ULTRAGENYX"&&"<?=$vew_data->custxt; ?>"!="AXA - NOVARTIS" && "<?=$vew_actcod;?>" != "02"){//???
            ...
            tmssCallProcessFile("?prg=<?= $vew_data->endpoint; ?>&act=evlcoa00", lo_dat, <?= $lv_sec; ?>_fncbckext);
            // update file data
            return false;
          }else{
