<?php
  ...
  $lv_lnk = '?prg=zcutp1&prm_evlcod='.$vew_data->evlcod;
	...
	// campos requeridos 
	$vew_input->RequiredFields( array('evlraqprc','patcod','pattxt','prscod','prstxt','spccod','spctxt','docsts','plnid','plndteid') );
	...
	*/
	if ($vew_data->evlcod=='') {
		...
		$vew_data->evlatrval001 = strtoupper($vew_data->evlatrval001);
	 	...
	 	$vew_data->evlraqprc = strtoupper($vew_doc->getTagValue($lv_buf,'evlraqprc'));

	 	if ($vew_data->evlraqprc ==''){
...
<section id="<?php echo $lv_sec; ?>" data-model="<?php echo $vew_model; ?>" data-title="<?php echo $lv_title; ?>">
				...
				<nav class="navbar navbar-default tmss-navbar">
					<div class="container-fluid">
						<ul class="nav navbar-nav tmss-navbar-left">
							<?php if(!$vew_readonly) { ?><a href="#" onclick="<?php echo $lv_sec; ?>_fnc({action: 'evlraq00'});" class="btn btn-default navbar-btn tmssAlwaysEnabled btn-success" title="<?php echo $vew_lang->save;  ?>"><span class="fas fa-save"></span><span class="hidden-xs"> <?php echo $vew_lang->save; ?></span></a><?php } ?>
							<?php if ($vew_sec->hasPermission('HLT','EVL','04') && $vew_data->evlcod!='') { ?><a href="#" onclick="<?php echo $lv_sec; ?>_fnc({action: 'evlraqx4'});" class="btn btn-default navbar-btn tmssAlwaysEnabled btn-danger" title="<?php echo $vew_lang->delete; ?>"><span class="fas fa-trash-alt"></span><span class="hidden-xs"> <?php echo $vew_lang->delete; ?></span></a><?php } ?>
				...
				<div class="container-fluid" role="tabpanel">
					...
					<div class="tab-content tmss-tab-content">
						...
						<div role="tabpanel" class="tab-pane active" id="<?php echo $lv_sec; ?>_tab001">
							<?php
								echo vew_boot($lv_col210,array('label'=>'Se aplic&oacute;?', 'input'=>gethtml('evlraqprc','yesno',$vew_data->evlraqprc,$lv_default) ));
							?>
							<div id="evlnoinf_div">
  ...
  <script>
	...
	<script>
		$("#<?php echo $lv_sec; ?> #evlraqprc").on("change",function(e){
	...
	<script>
		$(function(e){
			$("#<?php echo $lv_sec; ?> #evlraqprc").trigger("change");
  ...
  <script>
    ...
    function <?php echo $lv_sec; ?>_fnc( lp_prm ) {
			if (lp_prm["action"]=="prn") {
				...
				if (lp_prm["action"]=="evlraq00"){if(!tmssCheckRequiredFields($("#<?php echo $lv_sec; ?>_frm"))){return false;}}
				...
				if (lp_prm["action"]=="evlraq00" && $("#<?php echo $lv_sec; ?> #evlraqprc").prop("value")=="1" ) {
					var lv_err = 0;
