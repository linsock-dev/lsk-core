<?php	
  ...
  $vew_tbl['sveL'] =array('per'=>!$vew_readonly,'acc'=>$lv_sec.'_fnc({action: '.chr(39).'zcuclh_frmact03_00'.chr(39).'});'); 
  $vew_tbl['sveR'] =array('per'=>!$vew_readonly,'acc'=>$lv_sec.'_fnc({action: '.chr(39).'zcuclh_frmact03_00'.chr(39).'});'); 
  $vew_tbl['del']  =array('per'=>$vew_sec->hasPermission('HLT','EVL','04') && $vew_data->evlcod!='','acc'=>$lv_sec.'_fnc({action: '.chr(39).'zcuclh_frmact03_04'.chr(39).'});');
  if($vew_data->evllon!=''){
...
<section id="<?= $lv_sec; ?>" data-title="<?= $lv_title; ?>">
          ...
          <div class="row">
            ...
            <div class="card" id="evlnoinf_div">
              <div class="card-body tmss-card-body-edit">
                 <?php
                    ...
                    echo vew_boot($lv_col210,array('label'=>$vew_lang->comments, 'input'=>gethtml('evlcnccmt','doccmt5x50',$vew_data->evlsub,$lv_default) ));              
                  ?>                
            ...
            <div class="card" id="evlyesinf_div">
              ...
              <div class="card-body tmss-card-body-edit">
                  ...
                  <tbody id="tbldatbdy">
                    <?php
                      foreach($vew_data->evlmat as $lo_matrow){
                        echo '<tr>';
                        echo '<td><strong>'.$lo_matrow['mattxt'].'</strong> Cant. Viales: <strong>'.$lo_matrow['matqty'].' '.$lo_matrow['matuntcod'].'</strong><br>Lote: <strong>'.$lo_matrow['matbchcodext'].'</strong>&nbsp; vto:<strong>'.date_format($lo_matrow['matbchduedte'],'d/m/Y').'</strong></td>';
                ...
                <?= vew_boot($lv_col233,array('label1'=>'Inf. Programada', 'input1'=>gethtml('infprg','docnum0800',$vew_data->infprg,$lv_default) ,'input2'=>''));?>
                <?= vew_boot($lv_col233,array('label1'=>'Inf. realizadas', 'input1'=>gethtml('infexe','docnum0800',$vew_data->infexe,$lv_default) ,'input2'=>''));?>
                ...
                <div id="<?php echo $lv_sec; ?>_mtv2">
                  <?= vew_boot($lv_col210,array('label'=>$vew_lang->motive, 
                               ...
                               $vew_data->evlcncmtv2,$lv_default) ));
                  ?>
                </div>
                <hr/>
                ...
                <?= vew_boot($lv_col210,array('label'=>$vew_lang->comments, 'input'=>gethtml('evlevl','doccmt5x50',$vew_data->evlevl,$lv_default) ));?>
              </div><!-- /Card Boby -->
  ...
  <script>
    function <?= $lv_sec; ?>_fncbckext(lp_prm){
      if ( lp_prm && lp_prm['errtyp']!='S') {
        if(lp_prm['errtyp']=='E'){
        	toastr.error(lp_prm['errtxt'] );   
        }else{
    ...
    function <?php echo $lv_sec; ?>_fncext( lp_prm ) {
			...
			if (lp_prm["action"]=="prn") {
				...
				if (lp_prm["action"]=="zcuclh_frmact03_00" ) {
          if ($("#<?php echo $lv_sec; ?> #evlinfprc").prop("value")=="1"){
            ...
            if ( lv_fleqty==0  && lv_fldrec.includes("uplfle")) { 
            ...
            tmssCallProcessFile("?prg=zcuclh&act=zcuclh_frmact03_00", lo_dat, <?= $lv_sec; ?>_fncbckext);
            // update file data
            return false;
          }else{
