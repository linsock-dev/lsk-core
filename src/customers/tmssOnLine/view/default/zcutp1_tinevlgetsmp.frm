...
$vew_tbl['sveL'] =array('per'=>!$vew_readonly,'acc'=>$lv_sec.'_fncext({action: '.chr(39).'evlgetsmp00'.chr(39).'});'); 
$vew_tbl['sveR'] =array('per'=>!$vew_readonly,'acc'=>$lv_sec.'_fncext({action: '.chr(39).'evlgetsmp00'.chr(39).'});'); 
$vew_tbl['del']  =array('per'=>$vew_sec->hasPermission('HLT','EVL','04') && $vew_data->evlcod!='','acc'=>$lv_sec.'_fncext({action: '.chr(39).'evlgetsmpx4'.chr(39).'});');
?>
	...
	<script>
		...
		<?php if ($vew_sec->hasPermission('HLT','EVL','05') && $vew_data->evlcod!='') { ?>
			...
			$("#<?php echo $lv_sec; ?> #btnprn").on("click",function(e){
				window.open("?prg=zcutp1&act=evlgetsmp00prn&prm_evlcod="+<?php echo $vew_data->evlcod; ?>);
			});
		<?php } ?>
      ...
      if (lp_prm["action"]=="evlgetsmpx4") {
        ...
        BootstrapDialog.confirm({
          ...
          callback: function(result) {
            if(result) { 
             // <?php echo $lv_sec; ?>_fnc({action: "evlgetsmp04"});
            ...
            tmssCallProcessFile("?prg=zcutp1&act=evlgetsmp04", lo_dat, <?= $lv_sec; ?>_fncbckext);
            }
          }
        });
        return false;
      }
      if (lp_prm["action"]=="evlgetsmp00"){
        ...
        if ($("#<?php echo $lv_sec; ?> #evlgetsmp").prop("value")=="" && $("#<?php echo $lv_sec; ?> #evlgetsmpprc").prop("value")=="1" ) {
          ...
          debugger
          /* Llamada al controlaor */
          tmssCallProcessFile("?prg=zcutp1&act=evlgetsmp00", lo_dat, <?= $lv_sec; ?>_fncbckext);
          return false;
        }
      }
