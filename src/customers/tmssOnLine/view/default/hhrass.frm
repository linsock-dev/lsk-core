<?php
	/* url del formulario */
  $lv_lnk = '?prg=hhrass&prm_hhrasscod='.$vew_data->hhrasscod;

	/* campos requeridos */
	$vew_input->RequiredFields( array('srcobjtxt','srcobjcod','hhrassdte','docsts') );

	/* clave del documento */
	$lv_dockey = $vew_data->hhrasscod;

	/* titulo */
	$lv_title = $vew_lang->assistance;
	...
	if($vew_data->hhrasscod!=''){
		if($vew_data->hhrassend<$lv_minstrdte || $vew_data->hhrassdte>$lv_maxenddte ){ 
			$vew_lck_total=true;
		}
	}
...
<section id="<?= $lv_sec; ?>" data-title="<?= $lv_title; ?>">
  ...
  <form method="POST" class="form-horizontal tmss-form-horizontal" id="<?= $lv_sec; ?>_frm">
    <?= gethtml('tmss_actcod','hidden',''); ?>
		<?= gethtml('hhrchrasgcodlst','hidden',''); ?>
		
    <div class="container-fluid" role="tabpanel">
			<ul class="nav nav-pills nav-secondary nav-pills-no-bd" role="tablist">
				<li role="presentation" class="active"><a href="#<?= $lv_sec; ?>_tab001" role="tab" data-toggle="tab"><?= $vew_lang->assistance; ?></a></li>
				<li class="pull-right"><h4># <strong><?= $vew_data->hhrasscod; ?><?= gethtml('hhrasscod','hidden',$vew_data->hhrasscod); ?></strong></h4></li>
			</ul>
			<div class="tab-content tmss-tab-content">
              ...
              <div class="card">
                ...
                <div class="card-body tmss-card-body-edit">
                  <?php 
                    echo vew_boot($lv_col210, array('label'=>$vew_lang->code,'input'=>gethtml('hhrasscodext', 'doccodext', $vew_data->hhrasscodext, ($vew_lck_total?$lv_always_disabled:$lv_default) ) ));
                    echo vew_boot($lv_col210, array('label'=>$vew_lang->staff,'input'=>vew_boot(	array('style'=>'search', 'readonly'=>($vew_data->hhrasscod!=''?true:$vew_readonly) ),array('input'=>gethtml('srcobjtxt', 'typeahead', $vew_data->srcobjtxt, ($vew_data->hhrasscod!=''?$lv_always_disabled:$lv_default)))))	);
                    echo gethtml('srcobjcod','hidden',$vew_data->srcobjcod);
                    echo gethtml('srcobjtyp','hidden',$lv_srcobjtyp);
                    echo vew_boot($lv_col210, array('label'=>$vew_lang->date, 'input'=>gethtml('hhrassdte', 'docdte', $vew_data->hhrassdte, ($vew_lck_total || $vew_lck_parcial?$lv_always_disabled:$lv_default_dte) ) ));
										echo vew_boot($lv_col210,array('label'=>$vew_lang->assistance,'input'=>gethtml('hhrassflg','checkbox',$vew_data->hhrasscod,$lv_default)));
                      ...
                      <?php
                        foreach($vew_data->chrtypasg as $lv_row){
                          ...
                          echo '<tr>'.
                                ...
                                '<td><div class="'.($lv_tmeasg=='M'?'':'hidden').'">'.gethtml('asghrs', 'docnum0601', $vew_doc->getTagValue($lv_row['hhrasschratr'],'hrs'), ($vew_lck_total || $vew_lck_parcial?$lv_always_disabled:$lv_default) ).'</div></td>'.
                                '</tr>';
                        }
                      ?>
                    </tbody>
							...
							<div class="card">
								<div class="card-header"><div class="card-title"><?= $vew_lang->records; ?></div></div>
								<div class="card-body tmss-card-body-edit">												
								</div>
							</div>
						</div>
	...
	<script>
        ...
        tmssTypeahead($("#<?= $lv_sec; ?> #srcobjtxt"), "edutch", lo_get, {'afterAssign': function(){$("#<?= $lv_sec; ?> #srcobjcod").change();}});
			<?php } ?>
			<?php if($lv_srcobjtyp=='HHR_EMP'){ ?>
        // hhremp - typeahead
        var lo_get = {"fldsec" : "<?= $lv_sec; ?>", "fldasg":{ "srcobjtxt":"hhremptxt", "srcobjcod":"hhrempcod"}};
        tmssTypeahead($("#<?= $lv_sec; ?> #srcobjtxt"), "hhremp", lo_get, {'afterAssign': function(){$("#<?= $lv_sec; ?> #srcobjcod").change();}});
			<?php } ?>
  ...
  <script>
    function <?= $lv_sec; ?>_fncext( lp_prm ) {
			if (lp_prm["action"]=="00") {
				var lv_sel = "";
				var lv_itm = $("#<?= $lv_sec; ?> #hhrchrasgtbl :checkbox:checked");
				var lv_asghrs;
				var lv_tmeasg;
				...
				$("#<?= $lv_sec; ?> #hhrchrasgcodlst").prop("value",lv_sel);
			}
		}
  </script>
