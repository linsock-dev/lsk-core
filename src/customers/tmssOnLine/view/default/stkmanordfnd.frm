...
<section id="<?= $lv_sec; ?>">
	<form method="POST" class="form-horizontal" id="<?= $lv_sec; ?>_frm">
		...
		<div class="container-fluid">
            ...
            <div class="card-body tmss-card-body-edit">
              <label><?= $vew_lang->responsible ?></label>
              <?= gethtml('docfndrsptxt','doccmt1x50','',$lv_default); ?>
              ...
              <label><?= $vew_lang->workorders ?></label>
              <?= gethtml('workorder','doccmt1x50','',$lv_default); ?>
  ...
  <script>
      ...
      var lv_pstdat = [	
                ...
                {name: "stkmanactcod", value: "EE"},
        				{name: "refarr", value: '<?= html_entity_decode( $vew_data->refarr); ?>'}
              ];
                                                         
			tmssCallProcess("?prg=stkmanord&act=17", lv_pstdat, function(data) {
        ...
        if( Array.isArray(data) ) {
          ...
          lv_tbl += "<tr>"
											 	+ "<td>"
                          + "<input type='checkbox' name='rowchk' data-ordcod='" + data[i]["stkmanordmatcod"] + "'>"
                          + "<textarea class='hidden' id='" + data[i]["stkmanordmatcod"] + "_data'>" + JSON.stringify(data[i]) + "</textarea>"
                        	+ "<td>"
                          // Datos de Orden de Trabajo
                          + "<strong>" + (data[i]["stkmanorddte"] ?? '') + " - " + (data[i]["stkmanordtxt"] ?? '') + " - " + (data[i]["rsptxt"] ?? '') + "</strong><br>" 
                          // Datos de Material
                          + "#" + (data[i]["matcod"] ?? '') + " - " + (data[i]["mattxt"] ?? '') + (data[i]["matsercodext"] ? " - " + data[i]["matsercodext"] : '') + "<br>"
                          + (data[i]["stkmanordmatatr"] ? "Descripci&oacute;n: " + data[i]["stkmanordmatatr"] : '')
          								+ "</td>"
                        + "</td>"
										+"</tr>";
						i++;
