<?php	
  ...
  $vew_tbl['sveL'] =array('per'=>!$vew_readonly,'acc'=>$lv_sec.'_fnc({action: '.chr(39).'zcuclh_frmact01_00'.chr(39).'});'); 
  $vew_tbl['sveR'] =array('per'=>!$vew_readonly,'acc'=>$lv_sec.'_fnc({action: '.chr(39).'zcuclh_frmact01_00'.chr(39).'});'); 
  $vew_tbl['del']  =array('per'=>$vew_sec->hasPermission('HLT','EVL','04') && $vew_data->evlcod!='','acc'=>$lv_sec.'_fnc({action: '.chr(39).'zcuclh_frmact01_x4'.chr(39).'});');
  if($vew_data->evllon!=''){
        ...
        <div role="tabpanel" class="tab-pane active" id="<?php echo $lv_sec; ?>_tab001">
          <div class="row">
            ...
            <div class="card">
              <div class="card-body tmss-card-body-edit">
                <?php 
                	echo vew_boot($lv_col210,array('label'=>'Inf. Farmacovigilancia','input'=>gethtml('fvrpt',array(''=>'','0'=>'No','1'=>'Si'),$vew_data->fvrpt,$lv_default) 
                                                  ));
                ?>
            ...
            <div class="card" id="evlyesinf_div">
              ...
              <div class="card-body tmss-card-body-edit">
                ...
                <?= vew_boot($lv_col210,array('label'=>$vew_lang->evolution, 'input'=>gethtml('evlevl','doccmt2x50',$vew_data->evlevl,$lv_default) ));?>          
              </div><!-- /Card Boby -->
            </div><!-- /Card -->           
          </div><!-- /Row -->
  ...
  <script>
    // IMPRIMIR
			$("#<?php echo $lv_sec; ?> #btnprn").on("click",function(e){
				window.open("?prg=zcuclh&act=hltpatevlprn&prm_evlcod=<?php echo $vew_data->evlcod; ?>");
			});
  </script>
  <script>
    // agregar archivos
  ...
  <script>
    ...
    function <?php echo $lv_sec; ?>_fncext( lp_prm ) {
			...
			if (lp_prm["action"]=="prn") {
				...
				if (lp_prm["action"]=="zcuclh_frmact01_x4") {
					BootstrapDialog.confirm({
						title: 'Borrar Evolucion',
						message: '¿Desea borrar el documento?',
						type: BootstrapDialog.TYPE_WARNING,
						callback: function(result) {
							if(result) { <?php echo $lv_sec; ?>_fnc({action: "zcuclh_frmact01_04"}); }
						}
					});
					return false;
				}
				if (lp_prm["action"]=="zcuclh_frmact01_00" ) {
          if ($("#<?php echo $lv_sec; ?> #evlinfprc").prop("value")=="1"){
            ...
            tmssCallProcessFile("?prg=zcuclh&act=zcuclh_frmact01_00", lo_dat, <?= $lv_sec; ?>_fncbckext);
            // update file data
            return false;
          }else{
