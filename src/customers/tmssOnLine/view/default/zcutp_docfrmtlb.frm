<?php
	...
	//	posicion (pos),	permiso (per), texto (ttl), icono (icn), clase (css), id (id), accion (acc)
	$vew_tbl_int = array();
	...
	//$vew_tbl_int['modL'] = array('pos'=>'L','per'=>$vew_sec->hasPermission($lv_mdlcod, $lv_prgcod,'02'), 'ttl'=>$vew_lang->modify, 'id'=>'','icn'=>'fas fa-pencil-alt', 'css'=>'btn navbar-btn tmss-navbar-btn tmssAlwaysEnabled tmssHiddeOnEdit tmss-desk-btn', 'acc'=>$lv_sec.'_fnc({action: '.chr(39).'02'.chr(39).'});' );
	//$vew_tbl_int['modR'] = array('pos'=>'R','per'=>$vew_sec->hasPermission($lv_mdlcod, $lv_prgcod,'02'), 'ttl'=>$vew_lang->modify, 'id'=>'','icn'=>'fas fa-pencil-alt', 'css'=>'btn navbar-btn tmss-navbar-btn tmssAlwaysEnabled tmssHiddeOnEdit tmss-mob-btn', 'acc'=>$lv_sec.'_fnc({action: '.chr(39).'02'.chr(39).'});' );
	...
	$vew_tbl_int['delsep'] = array('pos'=>'D','per'=>$vew_sec->hasPermission($lv_mdlcod,$lv_prgcod,'04') && $vew_readonly, 'ttl'=>'', 'id'=>'','icn'=>'', 'css'=>'divider', 'acc'=>'' );
	$vew_tbl_int['del'] = array('pos'=>'D','per'=>$vew_sec->hasPermission($lv_mdlcod,$lv_prgcod,'04') && $vew_readonly, 'ttl'=>$vew_lang->delete, 'id'=>'','icn'=>'fas fa-trash-alt', 'css'=>'tmss-Opt', 'acc'=>$lv_sec.'_fnc({action: '.chr(39).'04'.chr(39).'});' );
  ...
  $vew_tbl_int['sveR'] = array('pos'=>'R','per'=>true, 'ttl'=>$vew_lang->save, 'id'=>'','icn'=>'fas fa-save', 'css'=>'btn navbar-btn tmss-navbar-btn tmssAlwaysEnabled tmssHiddeOnRead btn-success tmss-mob-btn', 'acc'=>$lv_sec.'_fnc({action: '.chr(39).'00'.chr(39).'});' );
	...
	//$vew_tbl_int['clsR'] = array('pos'=>'', 'per'=>false);
	$vew_tbl_int['prnR'] = array('pos'=>'R','per'=>true, 'ttl'=>$vew_lang->print, 'id'=>'','icn'=>'fas fa-print', 'css'=>'btn navbar-btn tmss-navbar-btn', 'acc'=>'' );
			...
			<?php
				foreach($vew_tbl_int as $lv_row) {
					if($lv_row['per'] && $lv_row['pos']=='L'){
						echo '<a href="#" id="'.$lv_row['id'].'" onclick="'.$lv_row['acc'].'"  class="'.$lv_row['css'].'" title="'.$lv_row['ttl'].'"><i class="'.$lv_row['icn'].'"></i><span class="hidden-xs"> '.$lv_row['ttl'].'</span></a>';
					}
				}
			?>
    </ul>
        ...
        <div class="btn-group dropdown">
          ...
          <form class="dropdown-menu dropdown-menu-right tmssBrandMnuUsr" aria-labelledby="dLabel">
            ...
            <?php if(isset($vew_tbl_int['rfrsh'])){ 
            	$vew_tbl_int['rfrsh']['id']=$vew_tbl_int['rfrsh']['id']??'';
  						$vew_tbl_int['rfrsh']['acc']=$vew_tbl_int['rfrsh']['acc']??'';
  						$vew_tbl_int['rfrsh']['icn']=$vew_tbl_int['rfrsh']['icn']??'';
  						$vew_tbl_int['rfrsh']['css']=$vew_tbl_int['rfrsh']['ttl']??'';
  						$vew_tbl_int['rfrsh']['ttl']=$vew_tbl_int['rfrsh']['ttl']??'';
            ?>
