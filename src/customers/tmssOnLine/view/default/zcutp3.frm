				...
				<nav class="navbar navbar-default tmss-navbar">
					<div class="container-fluid">
						<ul class="nav navbar-nav tmss-navbar-left">
							<?php if(!$vew_readonly) { ?><a href="#" onclick="<?php echo $lv_sec; ?>_fnc({action: 'evlhem00'});" class="btn btn-default navbar-btn tmssAlwaysEnabled btn-success" title="<?php echo $vew_lang->save;  ?>"><span class="fas fa-save"></span><span class="hidden-xs"> <?php echo $vew_lang->save; ?></span></a><?php } ?>
							<?php if ($vew_sec->hasPermission('HLT','EVL','04') && $vew_data->evlcod!='') { ?><a href="#" onclick="<?php echo $lv_sec; ?>_fnc({action: 'evlinfx4'});" class="btn btn-default navbar-btn tmssAlwaysEnabled btn-danger" title="<?php echo $vew_lang->delete; ?>"><span class="fas fa-trash-alt"></span><span class="hidden-xs"> <?php echo $vew_lang->delete; ?></span></a><?php } ?>
				...
				<div class="container-fluid" role="tabpanel"> 
					...
					<div class="tab-content tmss-tab-content">
						...
						<div role="tabpanel" class="tab-pane active" id="<?php echo $lv_sec; ?>_tab001">
							<?php
								echo vew_boot($lv_col210,array('label'=>'Se infundió?', 'input'=>gethtml('evlinfprc','yesno',$vew_data->evlinfprc,$lv_default) ));
							?>
							<div id="evlnoinf_div">
	...
	<script>
		...
		var <?php echo $lv_sec; ?>_hotmatset = {
			...
			columns: [
				...
				{type: "autocomplete", data: "atrendtme", width: 30, renderer: <?php echo $lv_sec; ?>_hotmat_renderer <?php echo ($vew_readonly?', readOnly: true':''); ?> },
  ...
  <script>
    ...
    function <?php echo $lv_sec; ?>_fnc( lp_prm ) {
			if (lp_prm["action"]=="prn") {
				...
				if (lp_prm["action"]=="evlhem00"){if(!tmssCheckRequiredFields($("#<?php echo $lv_sec; ?>_frm"))){return false;}}				
				...
				if (lp_prm["action"]=="evlhem00" && $("#<?php echo $lv_sec; ?> #evlinfprc").prop("value")=="1" ) {
					var lv_err = 0;
