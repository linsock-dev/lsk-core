  ...
  $lv_lnk = '?prg=sysappprg';
	...
	function armar_menu( $lp_mnu, &$vew_lang ) {
		...
		foreach( $lp_mnu as $lv_row ) {
			...
			$lv_pic=($lv_row['prgpic']==''?'':(substr($lv_row['prgpic'],0,6)=='class:'?substr($lv_row['prgpic'],6,strlen($lv_row['prgpic'])-6):'fa fa-ban'));
			// módulo
			if ( $lv_row['prgtypcod']==0 ) {
				$lv_row['mdlpic'] = strtolower($lv_row['mdlpic']);
				$lv_pic=($lv_row['mdlpic']==''?'':(substr($lv_row['mdlpic'],0,6)=='class:'?substr($lv_row['mdlpic'],6,strlen($lv_row['mdlpic'])-6):'fa fa-ban'));
				if (count($lv_row['mnulst'])!=0){ $lv_buffer_sub = armar_menu( $lv_row['mnulst'], $vew_lang ); }
				$lv_buffer .= '<li data-jstree='.chr(39).'{"icon":"'.($lv_pic!=''?$lv_pic:'fa fa-folder-open-o').'"}'.chr(39).' data-mdlcod="'.$lv_row['mdlcod'].'" data-prgcod="**">'.$vew_lang->get($lv_row['mdltxt']).' ('.$lv_row['mdlcod'].')'.'<ul>'.$lv_buffer_sub.'</ul></li>';
			// carpeta
			} else if ( $lv_row['prgtypcod']==3 ) {
				if (count($lv_row['mnulst'])!=0){ $lv_buffer_sub=armar_menu( $lv_row['mnulst'], $vew_lang ); }
				$lv_buffer .= '<li data-jstree='.chr(39).'{"icon":"'.($lv_pic!=''?$lv_pic:'fa fa-folder-open-o').'"}'.chr(39).' data-mdlcod="'.$lv_row['mdlcod'].'" data-prgcod="'.$lv_row['prgcod'].'">'.$vew_lang->get($lv_row['prgtxt']).' ('.$lv_row['prgcod'].')'.'<ul>'.$lv_buffer_sub.'</ul></li>';
			// separador
			} else if ( $lv_row['prgtypcod']==2 ) {
				$lv_buffer .= '<li data-jstree='.chr(39).'{"icon":"'.($lv_pic!=''?$lv_pic:'fa fa-minus').'"}'.chr(39).' data-mdlcod="'.$lv_row['mdlcod'].'" data-prgcod="'.$lv_row['prgcod'].'">------------------------------ ('.$lv_row['prgcod'].')</li>';
			// programa
			} else if ( $lv_row['prgtypcod']==1 ) {
				$lv_buffer .= '<li data-jstree='.chr(39).'{"icon":"'.($lv_pic!=''?$lv_pic:'fa fa-cogs').'"}'.chr(39).' data-mdlcod="'.$lv_row['mdlcod'].'" data-prgcod="'.$lv_row['prgcod'].'">'.$vew_lang->get($lv_row['prgtxt']).' ('.$lv_row['prgcod'].')</li>';
			}
			$lv_count++;
		}
		return $lv_buffer;
...
<section id="<?= $lv_sec; ?>" data-title="<?= $lv_title; ?>">
  ...
  <nav class="navbar navbar-default tmss-navbar tmss-navbar-fixed">
	...
	<form method="POST" class="form-horizontal tmss-form-horizontal" id="<?= $lv_sec; ?>_frm">
		...
		<div class="container-fluid">
		<?= '<div id="sysappprglst"><ul>'.armar_menu( $vew_data->sysprg, $vew_lang ).'</ul></div>'; ?>
		</div>
	</form>
	<script>
		...
		$("#<?= $lv_sec; ?> #btnnew").on("click",function(e){
			var lv_itm = $("#<?= $lv_sec; ?> #sysappprglst").jstree().get_selected(true);
