<?php
	...
	foreach($vew_data->stkmovdocmat as $lv_row) {
		...
		if(($lv_row['matusebch']=='1'&&$lv_row['matbchcodext']=='')||($lv_row['matuseser']=='1'&&$lv_row['matsercodext']=='')){
			if($lv_row['sysdocrejcod']!='0') {
				$lv_cmt =$lv_row['sysdocrejtxt'];
			}else{
				if(!in_array( $lv_row['matcod'],$vew_datastk)){
				$lv_cmt ='Sin stock';
			}
			}
