<?php
	// url del formulario 
  $lv_lnk = '?prg=syssecusrgrp&prm_usrcod='.$vew_data->usrcod;
  ...
  <form method="POST" class="form-horizontal tmss-form-horizontal" id="<?= $lv_sec; ?>_frm">
    <?= gethtml('tmss_actcod','hidden',''); ?>
		...
		<div id="syssecusrgrplst">
			<ul>
				<?php
					$lv_buffer='';
					foreach($vew_sysgrp as $lv_row) {
						$lv_alw=false;
						foreach($vew_usrgrp as $lv_row2){if($lv_row['usrgrpcod']==$lv_row2['usrgrpcod']){$lv_alw=true;}}
						$lv_buffer .= '<li data-jstree='.chr(39).'{"icon":"fas fa-angle-right","selected":'.($lv_alw?'true':'false').'}'.chr(39).' data-usrgrpcod="'.$lv_row['usrgrpcod'].'">'.$lv_row['usrgrptxt'].' ('.$lv_row['usrgrpcod'].')</li>';
					}
					echo $lv_buffer;
				?>
			</ul>
	...
	<script>
		tmssLoadScript("jstree",function(){
			$("#syssecusrgrplst").on("ready.jstree", function(e, data) {
			...
			if (lp_prm["action"]=="00") {
				...
				for(var i=0; i<lv_itm.length; i++) {
					if ( lv_itm[i].children.length==0 ) {
						lv_sel += (lv_sel==""?"":String.fromCharCode(9)) + lv_itm[i].data["usrgrpcod"];
					}
				}
