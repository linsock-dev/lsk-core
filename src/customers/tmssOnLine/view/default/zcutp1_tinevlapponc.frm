				...
				<nav class="navbar navbar-default tmss-navbar">
					<div class="container-fluid">
						<ul class="nav navbar-nav tmss-navbar-left">
							<?php if(!$vew_readonly) { ?><a href="#" onclick="<?php echo $lv_sec; ?>_fnc({action: 'evlapponc00'});" class="btn btn-default navbar-btn tmssAlwaysEnabled btn-success" title="<?php echo $vew_lang->save;  ?>"><span class="fas fa-save"></span><span class="hidden-xs"> <?php echo $vew_lang->save; ?></span></a><?php } ?>
							<?php if ($vew_sec->hasPermission('HLT','EVL','04') && $vew_data->evlcod!='') { ?><a href="#" onclick="<?php echo $lv_sec; ?>_fnc({action: 'evlapponcx4'});" class="btn btn-default navbar-btn tmssAlwaysEnabled btn-danger" title="<?php echo $vew_lang->delete; ?>"><span class="fas fa-trash-alt"></span><span class="hidden-xs"> <?php echo $vew_lang->delete; ?></span></a><?php } ?>
              <?php if ($vew_sec->hasPermission('HLT','EVL','05') && $vew_data->evlcod!='') { ?><a href="#" id="btnprn" class="btn btn-default navbar-btn tmssAlwaysEnabled tmssHiddeOnEdit" title="<?php echo $vew_lang->print; ?>"><span class="fas fa-print"></span><span class="hidden-xs"> <?php echo $vew_lang->print; ?></span></a><?php } ?>
						</ul>
				...
				<div class="container-fluid" role="tabpanel">
					...
					<div class="tab-content tmss-tab-content">
						...
						<div role="tabpanel" class="tab-pane active" id="<?php echo $lv_sec; ?>_tab001">
							<?php
								echo vew_boot($lv_col210,array('label'=>'Se aplic&oacute;?', 'input'=>gethtml('evlinfprc','yesno',$vew_data->evlinfprc,$lv_default) ));
							?>
							<div id="evlnoinf_div">
								<div class="form-group tmss-form-group">
									...
									<div class="col-sm-10">
                    <?php 
                    	//gethtml('evlcncmtv', array(''=>'','SV'=>'SIN VIALES','EN'=>'ENFERMEDAD','ND'=>'PACIENTE NO DISPONIBLE','OT'=>'OTROS'), $vew_data->evlcncmtv); 
                    	gethtml('evlcncmtv', $vew_data->evlcncmtvlst, $vew_data->evlcncmtv); 
                    ?></div>
								</div>
								<?php
	...
	<script>
		...
		<?php if ($vew_sec->hasPermission('HLT','EVL','05') && $vew_data->evlcod!='') { ?>
			...
			$("#<?php echo $lv_sec; ?> #btnprn").on("click",function(e){
				window.open("?prg=zcutp1&act=evlraqprn&prm_evlcod="+<?php echo $vew_data->evlcod; ?>);
			});
		<?php } ?>
	</script>
  <script>
    ...
    function <?php echo $lv_sec; ?>_fnc( lp_prm ) {
			if (lp_prm["action"]=="prn") {
				...
				if (lp_prm["action"]=="evlapponcx4") {
					...
					BootstrapDialog.confirm({
						...
						callback: function(result) {
							if(result) { <?php echo $lv_sec; ?>_fnc({action: "evlraq04"}); }
						}
					});
					return false;
				}
				if (lp_prm["action"]=="evlapponc00"){if(!tmssCheckRequiredFields($("#<?php echo $lv_sec; ?>_frm"))){return false;}}
				...
				if (lp_prm["action"]=="evlapponc00" && $("#<?php echo $lv_sec; ?> #evlinfprc").prop("value")=="1" ) {
					var lv_err = 0;
					...
					for (var i=0; i<lo_dat.length; i++) {
						if ( Object.keys(lo_dat[i]).length!=0 ) {
							if (	(lo_dat[i]["p"]===undefined || lo_dat[i]["p"]==="") &&
								...
								lv_err += (	lo_dat[i]["p"]===undefined || lo_dat[i]["p"]==="" ||
														lo_dat[i]["t"]===undefined || lo_dat[i]["t"]==="" ||
														lo_dat[i]["fc"]===undefined || lo_dat[i]["fc"]==="" ||
														lo_dat[i]["fr"]===undefined || lo_dat[i]["fr"]==="" ||
														lo_dat[i]["ta"]===undefined || lo_dat[i]["ta"]===""?1:0)
								lv_arr.push({	"p":lo_dat[i]["p"],
															"t":lo_dat[i]["t"],
															"fc":lo_dat[i]["fc"],
															"fr":lo_dat[i]["fr"],
															"ta":lo_dat[i]["ta"]
