<?php
	/* url del formulario */
  $lv_lnk = 'index.php?prg=zcuau1_nash';
...
<section id="<?= $lv_sec; ?>" data-title="<?= $lv_title; ?>">
  ...
  <form method="POST" class="form-horizontal tmss-form-horizontal" id="<?= $lv_sec; ?>_frm">
		...
		<div class="container-fluid">
			<div class="row">
				...
				<div class="col-sm-7">
					...
					<div class="text-center"><h1 style="margin: 10px !important; padding: 10px !important">NASH</h1></div>
						...
						<table class="table table-condensed table-bordered table-striped table-hover sort_table" id="divtblpat">
							<thead>
								<tr>
									...
									<th data-sortid="maxupddte">Ult.Seg. <span class="fas" style="float: right;"></span></th></tr>
							</thead>
							<tbody>
							</tbody>
						</table>
	...
	<form method="POST" id="zcuau2expfrm" name="zcuau2expfrm">
	...
	<script>
		...
		function <?= $lv_sec; ?>_GridRefresh001(lp_flt) {
			...
			tmssCallProcess("?prg=zcuau1_nash&act=28", lv_pstdat, function(data){
				...
				$("#<?= $lv_sec; ?> #divtblpat tr[name='patrow']").on("click", function(e){
					...
					tmssLink("index.php?prg=zcuau1_nash&act=02&prm_patcod="+$(this).data("patcod")+"&prm_prscod="+$(this).data("prscod")+"&prm_spccod="+$(this).data("spccod"), [{target: "_new_section",post_data: lv_pst_dat}] );
					e.preventDefault();
					e.stopPropagation();
				});
				<?php } ?>
				...
				$("#<?= $lv_sec; ?> #divtblprs tr[name='prsrow']").on("click", function(e){
					...
					tmssCallProcess("?prg=zcuau1_nash&act=18", lv_pstdat, function(data) {
						BootstrapDialog.show({
							...
							onshow: function(dialog) {
								...
								$(lv_frm).find("#patlst tbody tr").on("click", function(e) {
									...
									tmssLink("?prg=zcuau1_nash&act=02&prm_patcod="+$(this).data("patcod"), [{target: "_new_section",post_data: lv_pst_dat}] );
									e.stopPropagation();
									e.preventDefault();
									dialog.close();
								});
	...
	<script>
		$("#<?= $lv_sec; ?> #btnnew").on("click", function(e){
			...
			tmssLink("index.php?prg=zcuau1_nash&act=01&prm_prscod="+$(this).data("prscod")+"&prm_spccod="+$(this).data("spccod"), [{target: "_new_section",post_data: lv_pst_dat}] );
			e.preventDefault();
			e.stopPropagation();
		});
