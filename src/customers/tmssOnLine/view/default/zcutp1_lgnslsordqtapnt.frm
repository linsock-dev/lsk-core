<?php
	...
	if($vew_actcod=='movdocmovret'){
    $pdf->setfont('helvetica', 'B', 24);
    $pdf->text( 115, 12, 'ORDEN DE RETIRO');
    $pdf->setfont('helvetica', 'B', 12);
    $pdf->text( 115, 25, 'ID Orden: #'.$vew_data->stkmovdoccod);
  }else{ 
	$pdf->setfont('helvetica', 'B', 24);
    ...
    $pdf->setfont('helvetica', 'B', 12);
    $pdf->text( 115, 35, 'Fecha: '.	date_format($vew_data->stkmovdocdte, 'd/m/Y'));	// fecha
  }
$pdf->setfont('helvetica', '', 8);	
...
$pdf->setfont('helvetica', '', 9);
  $pdf->text( 8, 70,  $vew_data->dstobjadrstr 
              . ($vew_data->dstobjadrstrnum!=''?' '.$vew_data->dstobjadrstrnum:'') 
              . ($vew_data->dstobjadrstrflr!=''?' '.$vew_data->dstobjadrstrflr:'') 
              . ($vew_data->dstobjadrstrunt!=''?' '.$vew_data->dstobjadrstrunt:'') 
              . ($vew_data->dstobjadrstrbld!=''?' '.$vew_data->dstobjadrstrbld:'') 
              . ($vew_data->dstobjadrcty!=''?' - '.$vew_data->dstobjadrcty:'') 
              . ($vew_data->dstobjadrpstcod!=''?' ('.$vew_data->dstobjadrpstcod.')':'')
              . ($vew_data->dstobjlndregtxt!=''?' '.$vew_data->dstobjlndregtxt:'')
            );
$pdf->setfont('helvetica', '', 9);
	if($vew_actcod!='movdocmovret'){
	$pdf->text( 135, 63,  'Condición de Pago:     '.$vew_data->paytrmtxt);
	$pdf->text( 135, 70,  'Dirección de Entrega:  A CONVENIR');
  }
    ...
    $lv_buffer .= '<thead><tr style="background-color:#C6C6C6;">'.
                  ...
                  '<td align="center" width="300">Num Serie</td>'.
                  '</tr></thead>';
  }else{
    $lv_buffer .= '<thead><tr style="background-color:#C6C6C6;">'.
	...
	$lv_docmatlst =[];
  if($vew_actcod=='movdocmovret'){
		$lv_docmatlst=$vew_data->stkmovdocmat;
  }else{
    $lv_docmatlst=$vew_data->slsordmat;
  }
    
	foreach($lv_docmatlst as $lv_row) {
      ...
      if($vew_actcod=='movdocmovret'){
        ...
        $lv_buffer .= '<tr>'.
                      ...
                      '<td align="right"  width="300">'.($lv_row['matsercodext']??'').'</td>'.
                      '</tr>';
        ...
        $lv_buffer .= '<tr>'.          						
                      '<td align="left"  width="80">'.$lv_row['matcod'].'</td>'
                      .'<td align="left"  width="300">'.utf8_encode($lv_row['mattxt']??'').'</td>'
                      .'<td align="right" width="80">'.number_format(($lv_row['matqty']??0),2).'</td>'
                      .'<td align="left"  width="80">'.$lv_row['matuntcod'].'</td>'
                      .'<td align="right" width="80">'.number_format(($lv_row['matprc']??0),2).'</td>'          						
                      .'<td align="right" width="80">'.number_format(($lv_row['matqty']??0)*($lv_row['matprc']??0),2).'</td>'
                      .'</tr>';
        $lv_tot += ($lv_row['matqty']??0)*($lv_row['matprc']??0);
      }
		}	 
	}
    ...
    $pdf->text( 135, 247, 'TOTAL: '.($vew_data->curcod??'').'     '.number_format(($lv_tot??0),2));
  }
	// PIE
    ...
    foreach($vew_txt as $lv_row){
      if(utf8_encode($lv_row['txttypcodext']=='OBSCOT')){
    ...
    $pdf->text( 10, 255, 'Observaciones: ' .utf8_encode($vew_data->srccnttxt));	 // OBSERVACIONES CONTACTO    
    $pdf->setfont('helvetica', 'B', 13);
    $pdf->text( 10, 263, 'RETIRO DE EQUIPOS BAJO CONFORMIDAD:');
      $pdf->setfont('helvetica', 'B', 10);
    $pdf->text( 10, 285, 'FIRMA');
    $pdf->text( 170, 285, 'ACLARACION');
  }
	
  if($vew_actcod!='movdocmovret'){
  	$pdf->Output('Presupuesto.pdf', 'I');	
  }else{
    $pdf->Output('OrdenDeRetiro.pdf', 'I');
  } 
?>