<?php
final class hhrlabuniController extends tmssController {
  
	const MODEL = 'hhrlabuni';					// **************************
	const VIEW  = 'hhrlabuni';					// **************************
	const ID = 'hhrlabunicod';					// **************************
  protected $co_reg;
	private $lo_mdl;
  private $data = array();
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':			
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(	self::ID=>$this->lo_mdl->hhrlabunicod	) );				// ********************
					return $this->getView();
        } else {
      ...
      case '#18': 
				$lv_prm = array('vewmaxrec' =>'10',
												'vewfldflt' => (isset($lp_prm['hhrlabunitxt'])?'[~fltrow~]hhrlabunitxt'.chr(9).''.chr(9).$lp_prm['hhrlabunitxt'].chr(9).chr(9).chr(9):'').
																			'[~fltrow~]docsts'.chr(9).'='.chr(9).chr(9).'A'.chr(9).chr(9)
												);		
				$lo_data = $this->lo_mdl->getList($lv_prm);
				$lv_ret = array();
				foreach($lo_data as $lv_row){
					$lv_ret[] = array('hhrlabunicod'=>$lv_row['hhrlabunicod'],'hhrlabunitxt'=>$lv_row['hhrlabunitxt']);
				}
				$lv_retjsn = json_encode( $this->co_reg->document->array_utf8_converter(array('data'=>$lv_ret)) );
