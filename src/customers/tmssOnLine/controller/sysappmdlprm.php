<?php
final class sysappmdlprmController extends tmssController {
	const MODEL = 'sysappmdlprm';
	const VIEW  = 'sysappmdlprm';
	const ID = 'mdlcod';
  protected $co_reg;
	private $lo_mdl;
  private $data = array();
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':        
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(	'mdlcod'=>$this->lo_mdl->mdlcod	) );											// ********************
					return $this->getView();
        } else {
      ...
      case '#02': case '#03': case '#001':									
				...
				// get param (KEY)																																		
				if ( isset($lp_prm['id']) ) {
					$lv_key = array( self::ID=>$lp_prm['id'] );																			// ********************
				} else {
      ...
      case '#04':
        ...
        break;
			...
			// GET PARAMETER. devuelve un parámetro específico
			case '#getParameter':
				$lo_post = $this->co_reg->request->post;
				$lv_prm = array();
				if( ($lo_post['mdlcod']??'')!=''){ $lv_prm['mdlcod'] = $lo_post['mdlcod']; }
				if( ($lo_post['prmcod']??'')!=''){ $lv_prm['mdlatrval001'] = $lo_post['prmcod']; }				
				$lo_rs = $this->lo_mdl->getParameter( $lv_prm );
