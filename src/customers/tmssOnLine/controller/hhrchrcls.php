<?php
final class hhrchrclsController extends tmssController {
	const MODEL = 'hhrchrcls';
	const VIEW  = 'hhrchrcls';
	const ID = 'hhrchrclscod';
  protected $co_reg;
	private $lo_mdl;
  private $data = array();
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':			
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(	self::ID=>$this->lo_mdl->hhrchrclscod	) );
					return $this->getView();
        } else {
      ...
      case '#18': 
				$lv_prm = array('vewmaxrec' =>'10',
												'vewfldflt' => (isset($lp_prm['hhrchrclstxt'])?'[~fltrow~]hhrchrclstxt'.chr(9).''.chr(9).$lp_prm['hhrchrclstxt'].chr(9).chr(9).chr(9):'').
																			'[~fltrow~]docsts'.chr(9).'='.chr(9).chr(9).'A'.chr(9).chr(9)
												);		
				$lo_data = $this->lo_mdl->getList($lv_prm);
				return $this->co_reg->document->getJson( array('errtyp'=>$this->lo_mdl->errtyp,'errcod'=>$this->lo_mdl->errcod,'errtxt'=>$this->lo_mdl->errtxt,'data'=>$lo_data) );        break;
    }
  }
