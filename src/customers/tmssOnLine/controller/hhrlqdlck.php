<?php
final class hhrlqdlckController extends tmssController {
  
	const MODEL = 'hhrlqdlck';					// **************************
	const VIEW  = 'hhrlqdlck';					// **************************
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':			
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(	self::ID=>$this->lo_mdl->hhrlqdlckcod	) );				// ********************
					return $this->getView();
        } else {
      ...
      case '#18': 
				$lv_prm = array('vewmaxrec' =>'10',
												'vewfldflt' => (isset($lp_prm['hhrlqdlcktxt'])?'[~fltrow~]hhrlqdlcktxt'.chr(9).''.chr(9).$lp_prm['hhrlqdlcktxt'].chr(9).chr(9).chr(9):'').
																			'[~fltrow~]docsts'.chr(9).'='.chr(9).chr(9).'A'.chr(9).chr(9)
												);		
				$lo_data = $this->lo_mdl->getList($lv_prm);
				$lv_ret = array();
				foreach($lo_data as $lv_row){
					$lv_ret[] = array('hhrlqdlckcod'=>$lv_row['hhrlqdlckcod'],'hhrlqdlcktxt'=>$lv_row['hhrlqdlcktxt']);
				}
