<?php
final class eduprfController extends tmssController {
	const MODEL = 'eduprf';
	const VIEW  = 'eduprf';
	const ID = 'eduprfcod';
  protected $co_reg;
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':        
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(self::ID=>$this->lo_mdl->eduprfcod	) );
      ...
      case '#18':
        ...
        $lv_prm = array('vewmaxrec' =>'10',
												'vewfldflt' =>(isset($lp_prm['eduprftxt'])?'[~fltrow~]eduprftxt'.chr(9).''.chr(9).$lp_prm['eduprftxt'].chr(9).chr(9).chr(9):'').
