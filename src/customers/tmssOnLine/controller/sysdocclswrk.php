<?php
final class sysdocclswrkController extends tmssController {
	const MODEL = 'sysdocclswrk';
	const VIEW  = 'sysdocclswrk';
	const ID = 'sysdocclswrkcod';
	protected $co_reg;
	private $lo_mdl;
  private $data = array();
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':
				...
				$lv_buffer = $this->co_reg->request->post['sysdocwrk'];
				if ($lv_buffer!='') {
					$lv_buffer = html_entity_decode($lv_buffer);
					$lv_docwrk_arr = json_decode($lv_buffer,true);
					foreach( $lv_docwrk_arr as $lv_row ) {
				...
				$lo_docwrkmdl = $this->co_reg->load->model( self::MODEL );
				$lv_prm = array('vewfldflt' =>'[~fltrow~]dcw.sysdocclscod'.chr(9).'='.chr(9).chr(9). $this->data['sysdocclscod'] .chr(9).chr(9));
				$lo_rs = $lo_docwrkmdl->getList( $lv_prm );
				$this->lo_mdl->docwrk = $lo_rs;				
      ...
      // LIST. devuelve lista de workflows para clase de documento
      case '#18':
