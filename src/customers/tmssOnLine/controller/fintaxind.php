<?php
final class fintaxindController extends tmssController {
	const MODEL = 'fintaxind';
	const VIEW  = 'fintaxind';
	const ID = 'fintaxindcod';
  protected $co_reg;
	private $lo_mdl;
  private $data = array();
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
			...
			// LIST. devuelve lista de documentos
      case '#': case '#08':
      ...
      case '#00':        
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(self::ID=>$this->lo_mdl->fintaxindcod) );
      ...
      case '#17':
				$lv_prm = array('vewfldflt' => (isset($lp_prm['fintaxindtxt'])?'[~fltrow~]i.fintaxindtxt'.chr(9).''.chr(9).$lp_prm['fintaxindtxt'].chr(9).chr(9).chr(9):'').
