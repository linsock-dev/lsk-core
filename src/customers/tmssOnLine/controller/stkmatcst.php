<?php
final class stkmatcstController extends tmssController {
  
	const MODEL = 'stkmatcst';					// **************************
	const VIEW  = 'stkmatcst';					// **************************
	const ID = 'matcod';								// **************************
	protected $co_reg;
	private $lo_mdl;
  private $data = array();
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(	'matclscod'=>$this->lo_mdl->matclscod	) );				// ********************
					return $this->getView();
        } else {
      ...
      case '#04':
        ...
        break;
			...
			// LIST by TEXT
      case '#18': 
				$lo_data = $this->lo_mdl->getListByText( array(), $lp_prm );				
