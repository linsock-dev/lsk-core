<?php
final class grldatlndtwnController extends tmssController {
	const MODEL = 'grldatlndtwn';
	const VIEW  = 'grldatlndtwn';
	const ID = 'lndtwncod';
  protected $co_reg;
	private $lo_mdl;
  private $data = array();
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':        
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(	'lndtwncod'=>$this->lo_mdl->lndtwncod	) );
      ...
      case '#18':
				...
				$lv_prm = array('vewmaxrec' =>'50',
												'vewfldflt' =>(isset($lp_prm['lndtwntxt'])?'[~fltrow~]lt.lndtwntxt'.chr(9).''.chr(9).$lp_prm['lndtwntxt'].chr(9).chr(9).chr(9):'').
																			(isset($lp_prm['lndcod'])?'[~fltrow~]lt.lndcod'.chr(9).'='.chr(9).chr(9).$lp_prm['lndcod'].chr(9).chr(9):'').
																			(isset($lp_prm['lndregcod'])?'[~fltrow~]lt.lndregcod'.chr(9).'='.chr(9).chr(9).$lp_prm['lndregcod'].chr(9).chr(9):'').
																			($lo_post['vewfldflt']??'').
																			'[~fltrow~]lt.docsts'.chr(9).'='.chr(9).chr(9).'A'.chr(9).chr(9)
												);
