<?php
final class educouController extends tmssController {
	const MODEL = 'educou';
	const VIEW  = 'educou';
	const ID = 'educoucod';
  protected $co_reg;
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':			
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(	self::ID=>$this->lo_mdl->educoucod	) );
      ...
      case '#18': 
				$lv_prm = array('vewmaxrec' =>'10',
												'vewfldflt' => (isset($lp_prm['educoutxt'])?'[~fltrow~]educoutxt'.chr(9).''.chr(9).$lp_prm['educoutxt'].chr(9).chr(9).chr(9):'').
