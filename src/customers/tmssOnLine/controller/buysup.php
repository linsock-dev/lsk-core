<?php
final class buysupController extends tmssController {
	const CONTROLLER = 'buysup';
	const MODEL = 'buysup';
	const VIEW  = 'buysup';
	const ID = 'supcod';
	const OBJTYP ='BUY_SUP';
  protected $co_reg;
	private $lo_mdl;
  private $data = array();
    ...
    switch( $lp_act ) {
      ...
      case '#00':
        ...
        if ( $this->lo_mdl->save( $lo_post ) ) {
					$this->lo_mdl->load( array(self::ID=>$this->lo_mdl->supcod	) );
      ...
      case '#04':
        ...
        break;
			...
			// GETLIST by TEXT. devuelve una lista de documentos segun texto
      case '#17': case '#18':
				$lv_prm = array('vewmaxrec' =>'10',
												'vewfldflt' => (isset($lp_prm['suptxt'])?'[~fltrow~]s.suptxt'.chr(9).''.chr(9).$lp_prm['suptxt'].chr(9).chr(9).chr(9):'').
      ...
      // MODIFICAR CLASE DE DOCUMENTO
			case '#32':
				$lo_post = $this->co_reg->request->post;
				if ( $this->lo_mdl->load( array('supcod'=>$lo_post['supcod']) )==false ) {
          return $this->co_reg->document->getJson( array('errtyp'=>$this->lo_mdl->errtyp,'errcod'=>$this->lo_mdl->errcod,'errtxt'=>$this->lo_mdl->errtxt) );
				} else if( !$this->lo_mdl->changeClass( array('supcod'=>$this->lo_mdl->supcod,'sysdocclscod'=>$lo_post['sysdocclscod']) ) ) {
					return $this->co_reg->document->getJson( array('errtyp'=>$this->lo_mdl->errtyp,'errcod'=>$this->lo_mdl->errcod,'errtxt'=>$this->lo_mdl->errtxt) );
				} else {
					$this->lo_mdl->load( array('supcod'=>$lo_post['supcod']) );
			...
			case '#33':
				...
				$lv_prm = array('lang'  => $this->co_reg->language,
												...
												'sec' 	=> $this->co_reg->sec,
												'url'		=> '?prg='.self::CONTROLLER.'&act=32',
												'data'  => $lo_post,
												'doccls'=> $lv_docclsarr
												);
			...
			// BUYSUPONETME. Devuelve la vista para el popup de proveedor eventual
      case '#buysuponetme':
        $this->lo_mdl->create();
        return $this->co_reg->document->getView( 'buysuponetme', array('data'=>$this->lo_mdl,'sysseclnk'=>$this->co_reg->load->controller('sysseclnk'),'objtyp'=>self::OBJTYP,'actcod'=>$this->data['actcod']) );
				break;
    }
  }
}
