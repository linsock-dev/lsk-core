<?php
final class hhrchrtypController extends tmssController {
	const MODEL = 'hhrchrtyp';
	const VIEW  = 'hhrchrtyp';
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':			
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(	self::ID=>$this->lo_mdl->hhrchrtypcod	) );
					return $this->getView();
        } else {
      ...
      case '#18': 
				...
				$lv_prm = array('vewmaxrec' =>'10',
												'vewfldflt' => (isset($lp_prm['hhrchrtyptxt'])?'[~fltrow~]ct.hhrchrtyptxt'.chr(9).''.chr(9).$lp_prm['hhrchrtyptxt'].chr(9).chr(9).chr(9):'').
																			(isset($lo_post['hhrchrtypcod'])?'[~fltrow~]ct.hhrchrtypcod'.chr(9).'='.chr(9).chr(9).$lo_post['hhrchrtypcod'].chr(9).chr(9):'').
