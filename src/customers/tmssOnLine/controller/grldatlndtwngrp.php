<?php
final class grldatlndtwngrpController extends tmssController {
	const MODEL = 'grldatlndtwngrp';
	const VIEW  = 'grldatlndtwngrp';
	const ID = 'lndtwngrpcod';
  protected $co_reg;
	private $lo_mdl;
  private $data = array();
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':        
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(	'lndtwngrpcod'=>$this->lo_mdl->lndtwngrpcod	) );				
      ...
      case '#18':
				...
				$lv_prm = array('vewmaxrec' =>'50',
												'vewfldflt' => (isset($lp_prm['lndtwngrptxt'])?'[~fltrow~]lndtwngrptxt'.chr(9).''.chr(9).$lp_prm['lndtwngrptxt'].chr(9).chr(9).chr(9):'').
																			($lo_post['vewfldflt']??'').
