<?php
final class grlcalController extends tmssController {
	const MODEL = 'grlcal';
	const VIEW  = 'grlcal';
	const ID = 'grlcalcod';
  protected $co_reg;
  ...
  public function index( $lp_act , $lp_prm = array() ) {
    ...
    switch( $lp_act ) {
      ...
      case '#00':			
        if ( $this->lo_mdl->save() ) {
					$this->lo_mdl->load( array(	self::ID=>$this->lo_mdl->grlcalcod	) );
      ...
      case '#18': 
				$lv_prm = array('vewmaxrec' =>'10',
												'vewfldflt' => (isset($lp_prm['grlcaltxt'])?'[~fltrow~]grlcaltxt'.chr(9).''.chr(9).$lp_prm['grlcaltxt'].chr(9).chr(9).chr(9):'').
