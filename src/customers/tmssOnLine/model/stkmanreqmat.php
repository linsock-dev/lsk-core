<?php
final class stkmanreqmat extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'stkmanreqmatcod';
  const SRCTYP = 'STK_MAN_REQ_MAT';                                                                                                                           // **************************************
  ...
  function save( $lp_dat=array(), $lp_authCheck=true ) {
    ...
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? ($lp_authCheck?'02':'12') : ($lp_authCheck?'01':'11'));
		...
		//$this->data['stkmanreqmatatr'] = '<stkmanreqmattxt>'.(isset($this->data['stkmanreqmattxt'])?utf8_encode(htmlentities($this->data['stkmanreqmattxt'])):'').'</stkmanreqmattxt>';
		$lo_out_data = array();
