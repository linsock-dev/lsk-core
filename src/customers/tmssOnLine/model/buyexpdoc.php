<?php
final class buyexpdoc extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'buyexpdoccod';
  const SRCTYP = 'BUY_EXP_DOC';                                                                                                                           // **************************************
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                      ...
                                      $this->co_reg->db->sqldat($lp_in,'buyexpdoccod'),
                                      $this->co_reg->db->sqldte($lp_in,'buyexpdocdte'),
                                      $this->co_reg->db->sqldat($lp_in,'buyexptypcod'),
                                      $this->co_reg->db->sqlnum($lp_in,'buyexpdoctot',3),
                                      $this->co_reg->db->sqldat($lp_in,'buyexpdocnum'),
                                      $this->co_reg->db->sqldat($lp_in,'supcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'impobjtyp'),
																			$this->co_reg->db->sqldat($lp_in,'impobjcod'),
																			$this->co_reg->db->sqldat($lp_in,'buyexpdoccmt'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
                                      $this->co_reg->db->sqldat($lp_in,'sysdocrejcod'),
                                      $this->co_reg->db->sqldat($lp_in,'buyexpdocatr', false)
																		);
		$this->sysdata['sqltxt'] = 'BUY_EXP_DOC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
