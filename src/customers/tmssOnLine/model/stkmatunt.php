<?php
final class stkmatunt extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $this->data = $lp_dat;
    $lv_act = ( isset($lp_dat['matuntcodsrc']) && !empty($lp_dat['matuntcodsrc']) ? '02' : '01' );
		$lo_out_data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
																			...
																			$this->co_reg->db->sqldat($lp_in,'matunttxt'),
																			$this->co_reg->db->sqldat($lp_in,'matunttypcod'),
																			$this->co_reg->db->sqlnum($lp_in,'matuntqty'),
																			$this->co_reg->db->sqldat($lp_in,'matuntbse'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'STK_MAT_UNT_DEF (?,?,?,?,?,?,?,?,?,?)';
