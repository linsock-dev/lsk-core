<?php
final class sysfnc extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		...
		$this->data['sysfncatrval001'] = '<sysfncgrp>'.$lp_dat['sysfncgrp'].'</sysfncgrp>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,
																				...
																				$this->co_reg->db->sqldat($lp_in,'sysfnccodext'), 
																				$this->co_reg->db->sqldat($lp_in,'sysfnctxt',false), 
																				$this->co_reg->db->sqldat($lp_in,'sysfncttl',false), 
																				$this->co_reg->db->sqldat($lp_in,'sysfncdes',false), 
																				$this->co_reg->db->sqldat($lp_in,'docsts'),
																				$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																				$this->co_reg->db->sqldat($lp_in,'usrautlvl'),
																				$this->co_reg->db->sqldat($lp_in,'sysfnchde'),
																				$this->co_reg->db->sqldat($lp_in,'mdlcod'),
																				$this->co_reg->db->sqldat($lp_in,'sysfncpic',false),
																				$this->co_reg->db->sqldat($lp_in,'sysfncatrval001',false)
																			);
		$this->sysdata['sqltxt'] = 'SYS_FNC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
