<?php
final class edustuass extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'edustuasscod';
	const SRCTYP = 'EDU_STU_ASS';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
																			$this->co_reg->db->sqldat($lp_in,'edustuasscod'),
																			$this->co_reg->db->sqldat($lp_in,'eduplncod'),
																			$this->co_reg->db->sqldat($lp_in,'eduplndtecod'),
																			$this->co_reg->db->sqldat($lp_in,'stucod'),
																			$this->co_reg->db->sqldat($lp_in,'edustuass'),
																			$this->co_reg->db->sqldte($lp_in,'edustuassdte'),
																			$this->co_reg->db->sqldat($lp_in,'edustuassatr001',false),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'EDU_STU_ASS_DEF (?,?,?,?,?,?,?,?,?,?,?,?)';
