<?php
final class grlprcschcnd extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'prcschcndcod';
  const SRCTYP = 'GRL_PSC_CND';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'prcschcndcod'), 
																			$this->co_reg->db->sqldat($lp_in,'prcschcod'), 
																			$this->co_reg->db->sqldat($lp_in,'prccndcod'), 
																			$this->co_reg->db->sqldat($lp_in,'prcschcndrow'), 
																			$this->co_reg->db->sqldat($lp_in,'prcschcndrowstr'), 
																			$this->co_reg->db->sqldat($lp_in,'prcschcndrowend'), 
																			$this->co_reg->db->sqldat($lp_in,'prccndforcod'), 
																			$this->co_reg->db->sqldat($lp_in,'prcschcndman'), 
																			$this->co_reg->db->sqldat($lp_in,'finacccod'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'prccndttl',false),
																			$this->co_reg->db->sqldat($lp_in,'prcschcndstd')
																		);
		$this->sysdata['sqltxt'] = 'GRL_PRC_SCH_CND_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
