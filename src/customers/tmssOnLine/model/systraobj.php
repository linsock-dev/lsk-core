<?php
final class systraobj extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function delete( $lp_dat=array() ) {
    ...
    $this->data = $lp_dat;
		$lo_out_data = array();
		return $this->call_sp( '04', $this->data, $lo_out_data );
  }

	// RELEASE. libera la orden y sus objetos
  function release( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
    $this->data = $lp_dat;
		$lo_out_data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,
																			$this->co_reg->db->sqldat($lp_in,'systraobjcod'),
																			...
																			//$this->co_reg->db->sqldat($lp_in,'sysobjcnt',false),
																			$this->co_reg->db->sqljsn($lp_in,'sysobjcnt')
																		);
		$this->sysdata['sqltxt'] = 'SYS_TRA_OBJ_DEF (?,?,?,?,?,?,?,?,?)';
