<?php
final class tsrmovdocval extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'tsrmovdocvalcod';
  const SRCTYP = 'TSR_MOV';
  ...
  function save( $lp_dat=array() ) {
    ...
    $this->data['tsrmovdocvalatr'] = '<tsrmovdocvaltxt>'.(isset($this->data['tsrmovdocvaltxt'])?utf8_encode(htmlentities($this->data['tsrmovdocvaltxt'])):'').'</tsrmovdocvaltxt>';
    $lo_out_data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'tsrmovdocvalcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'tsrmovdocvalcodext'),
                                      $this->co_reg->db->sqldat($lp_in,'tsrmovdocvaltxt'),
                                      $this->co_reg->db->sqldte($lp_in,'tsrmovdocvaldte'),
                                      ...
                                      $this->co_reg->db->sqlnum($lp_in,'tsrmovdocvalamt'),
                                      ...
                                      $this->co_reg->db->sqldat($lp_in,'bnkacccod'),
                                     	$this->co_reg->db->sqldat($lp_in,'bnktjtcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'cshvalcod'),
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
                                      $this->co_reg->db->sqldat($lp_in,'bnkchkcod'),
																		);
		$this->sysdata['sqltxt'] = 'TSR_MOV_DOC_VAL_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
