<?php
final class grldatacc extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
	...
	function load( $lp_key=array() ) {
		$this->data = $lp_key;
		return $this->call_sp( '05', $lp_key, $this->data );
	}
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																		$this->co_reg->db->sqldat($lp_in,'accnum'),
																		$this->co_reg->db->sqldat($lp_in,'finacccod'),
																		$this->co_reg->db->sqldat($lp_in,'accsrctyp'),
																		$this->co_reg->db->sqldat($lp_in,'accsrccod'),
																		$this->co_reg->db->sqldat($lp_in,'docsts')
																	);
		$this->sysdata['sqltxt'] = 'GRL_DAT_ACC_DEF(?,?,?,?,?,?,?,?)';
