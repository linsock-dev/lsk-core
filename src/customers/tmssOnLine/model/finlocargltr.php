<?php
final class finlocargltr extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function load( $lp_key=array(),$lp_authCheck=true) {
		//$lp_prm = array('argltrcodext' => $lp_key['argltrcodext']);
		$lo_out_data = array();
		if( $this->call_sp( ($lp_authCheck?'03':'13'), $lp_key, $lo_out_data ) ) {
			if( count($lo_out_data)>0){
				$this->data = $lo_out_data[0];
				$this->data['recordset'] = $lo_out_data;
			}
			/*
			if( count($lo_out_data)>0){
				$this->data['docsts'] 			= $lo_out_data[0]['docsts'];
				$this->data['argltrcodext'] = $lo_out_data[0]['argltrcodext'];
				$this->data['cteusr'] 			= $lo_out_data[0]['cteusr'];
				$this->data['ctedte'] 			= $lo_out_data[0]['ctedte'];
				$this->data['updusr'] 			= $lo_out_data[0]['updusr'];
				$this->data['upddte'] 			= $lo_out_data[0]['upddte'];
			}
			$this->data['recordset'] = $lo_out_data;
			*/
			return true;
  ...
  function delete( $lp_dat=array(),$lp_authCheck=true ) {
	...
	// DELETE. borra el objeto (multiples registros de la misma clave)
  function deleteByArgltrcodext( $lp_dat=array()) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
    $this->data = $lp_dat;		
		$lo_out_data = array();
		return $this->call_sp( '24', $this->data, $lo_out_data );
  }
  ...
  // GET LIST. devuelve recordset de objetos (lista agrupada)
 	function getListSingle( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if($lo_vew==null){ $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions( $lp_vewopt );
		$lo_out_data = array();
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'argltrcodext'),
                                     	$this->co_reg->db->sqldat($lp_in,'taxcatcodsrc'),
                                     	$this->co_reg->db->sqldat($lp_in,'taxcatcoddst'),
                                     	$this->co_reg->db->sqldat($lp_in,'argltroprtyp'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata,'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'FIN_LOC_ARG_LTR_DEF (?,?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' || $lp_action=='18' || $lp_action=='28' || $lp_action=='03' || $lp_action=='13' ) {
      if($this->errtyp!='E'){
				$lp_out = $lo_rs;
      }else{
