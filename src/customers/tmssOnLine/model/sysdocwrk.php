<?php
final class sysdocwrk extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $lp_dat['wrkflwcnd'] = '<typ>'.($lp_dat['wrkflwcndtyp']??'').'</typ>'.
      											'<val>'.($lp_dat['wrkflwcndval']??'').'</val>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                    	...
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwcodext'),
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwtxt'),
                                     	$this->co_reg->db->sqldat($lp_in,'wrkflwcnd', false),
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_DOC_WRK_DEF (?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' ) {
      ...
      if ( $lo_rs && count($lo_rs)>0 ) {
		...
		if ( $lp_action=='03' && $this->errcod==0 ) {
			...
			$lv_prm = array('vewfldflt' =>'[~fltrow~]ws.wrkflwcod'.chr(9).'='.chr(9).chr(9).$lp_in['wrkflwcod'].chr(9).chr(9),
											'vewfldord' =>'ws.wrkflwstprow' );
			$lp_out['sysdocwrkstp'] = $lo_stpmdl->getList( $lv_prm );
		}

		//		A C T I V I D A D E S
		if ( $lp_action=='03' && $this->errcod==0 ) {
			$lo_actmdl = $this->co_reg->load->model('sysdocwrkact');
			$lv_prm = array('vewfldflt' =>'[~fltrow~]a.wrkflwcod'.chr(9).'='.chr(9).chr(9).$lp_in['wrkflwcod'].chr(9).chr(9),
											'vewfldord' =>'a.wrkflwstpcod' );
			$lp_out['sysdocwrkact'] = $lo_actmdl->getList( $lv_prm );
		}
