<?php
final class sysappprgvewcol extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function load( $lp_key=array() ) {
		$lo_out_data = array();
		if ( $this->call_sp( '03', $lp_key, $lo_out_data ) ) {
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    ...
    $this->sysdata['sqlprm'] =array($lp_action, $this->co_reg->sec->usrcod, 
																		$this->co_reg->db->sqldat($lp_in,'vewfldcod'), 
																		$this->co_reg->db->sqldat($lp_in,'vewcod'), 
																		$this->co_reg->db->sqldat($lp_in,'vewfld',false), 
																		$this->co_reg->db->sqldat($lp_in,'vewfldord'), 
																		$this->co_reg->db->sqldat($lp_in,'vewfldttl',false), 
																		$this->co_reg->db->sqldat($lp_in,'vewfldwth'), 
																		$this->co_reg->db->sqldat($lp_in,'vewfldalg',false), 
																		$this->co_reg->db->sqldat($lp_in,'vewflddatpas'), 
																		$this->co_reg->db->sqldat($lp_in,'vewfldflt'), 
																		$this->co_reg->db->sqldat($lp_in,'sysappfld',false), 
																		$this->co_reg->db->sqldat($lp_in,'docsts'),
																		$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_APP_PRG_VEW_COL_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
