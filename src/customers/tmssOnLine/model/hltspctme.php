<?php
final class hltspctme extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'spctmecod';
  const SRCTYP = 'HLT_SPC_TME';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'spctmecod'), 
																			$this->co_reg->db->sqldat($lp_in,'spccod'), 
																			$this->co_reg->db->sqldat($lp_in,'prscod'), 
																			$this->co_reg->db->sqldat($lp_in,'delcod'), 
																			$this->co_reg->db->sqldat($lp_in,'tmeday'), 
																			strtoupper(isset($lp_in['tmestr'])?date('Ymd').str_ireplace(':','',$lp_in['tmestr']).'00':'00000000000000'), 
																			strtoupper(isset($lp_in['tmeend'])?date('Ymd').str_ireplace(':','',$lp_in['tmeend']).'00':'00000000000000'), 
																			$this->co_reg->db->sqldat($lp_in,'tmeqty'), 
																			$this->co_reg->db->sqldat($lp_in,'tmefrq'), 
																			$this->co_reg->db->sqldat($lp_in,'tmeovr'), 
																			$this->co_reg->db->sqldat($lp_in,'tmeatr',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_SPC_TME_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
