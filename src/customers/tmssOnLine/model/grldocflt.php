<?php
final class grldocflt extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'vewfltcod';
  const SRCTYP = 'GRL_DOC_FLT';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'vewfltcod'), 
																			$this->co_reg->db->sqldat($lp_in,'vewflttxt'), 
																			$this->co_reg->db->sqldat($lp_in,'usrcod'), 
																			$this->co_reg->db->sqldat($lp_in,'vewcod'),
																			$this->co_reg->db->sqldat($lp_in,'vewfltpub',false),
																			$this->co_reg->db->sqldat($lp_in,'vewfltdef',false),
																			str_replace(chr(39),'^',($lp_in['vewfltdat']??'')),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_DOC_FLT_DEF (?,?,?,?,?,?,?,?,?,?,?,?)';
