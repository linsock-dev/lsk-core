<?php
final class hltcatval extends tmssAction {
  protected $co_reg;
  private $data = array();
  private $sysdata = array();
  const ID = 'hltcatvalcod';
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                      $this->co_reg->db->sqldat($lp_in,'hltcatvalcod'),
																			...
																			$this->co_reg->db->sqldat($lp_in,'hltcatval'),
																			$this->co_reg->db->sqldat($lp_in,'hltcatfrm'),
																			$this->co_reg->db->sqldat($lp_in,'hltcatto'),
																			$this->co_reg->db->sqldat($lp_in,'hltcatptr'),
																			$this->co_reg->db->sqldat($lp_in,'hltcatpts'),
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_CAT_VAL_DEF (?,?,?,?,?,?,?,?,?,?,?,?)';
