<?php
final class grldatper extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
	...
	function load( $lp_key=array() ) {
		$this->data['persrctyp'] = $lp_key['persrctyp'];
		$this->data['persrccod'] = $lp_key['persrccod'];
		$lo_out_data = array();
  ...
  //  CALL SP.  llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																		$this->co_reg->db->sqldat($lp_in,'pernum'),
																		$this->co_reg->db->sqldte($lp_in,'perbrndte'),
																		$this->co_reg->db->sqldat($lp_in,'persex'),
																		$this->co_reg->db->sqldat($lp_in,'civstscod'),
																		$this->co_reg->db->sqldat($lp_in,'pernatcod'),
																		$this->co_reg->db->sqldat($lp_in,'hhrmedcovcod'),
																		$this->co_reg->db->sqldat($lp_in,'hhrmedcovtxt'),
																		$this->co_reg->db->sqldat($lp_in,'hhrmedcovaflpln'),
																		$this->co_reg->db->sqldat($lp_in,'hhrmedcovaflnum'),
																		$this->co_reg->db->sqldat($lp_in,'persrctyp'),
																		$this->co_reg->db->sqldat($lp_in,'persrccod'),
																		$this->co_reg->db->sqldat($lp_in,'docsts'),
                                    $this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																	);
		$this->sysdata['sqltxt'] = 'GRL_DAT_PER_DEF(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
