<?php
final class finpaytrm extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array(),$lp_authCheck=true) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
		$lp_dat['paytrmatr'] = '<man>'.(isset($lp_dat['PayTrmAtrMan'])? ($lp_dat['PayTrmAtrMan']=='on' || $lp_dat['PayTrmAtrMan']=='1' ? '1' : '0') :'0').'</man>';
    $this->data = $lp_dat;
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew = null, $lp_authCheck=true) {
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'paytrmcodext'),
                                     	$this->co_reg->db->sqldat($lp_in,'paytrmtxt'),
                                     	$this->co_reg->db->sqldat($lp_in,'paytrmgrp'),
                                     	$this->co_reg->db->sqldat($lp_in,'paytrmbseduedte'),
                                     	$this->co_reg->db->sqldat($lp_in,'paytrmbseduedteqty'),
                                     	$this->co_reg->db->sqldat($lp_in,'paytrmbseduedtefnc'),
                                     	$this->co_reg->db->sqldat($lp_in,'paytrmatr'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata,'view_options', false)
																		);
    $this->sysdata['sqltxt'] = 'FIN_PAY_TRM_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' || $lp_action=='18') {
      ...
      if ( $lo_rs && count($lo_rs)>0 ) {
		...
		if ( ($lp_action=='03' || $lp_action=='13' ) && $this->errcod==0 ) {
			$lo_duemdl = $this->co_reg->load->model('finpaytrmdue');
			$lv_prm = array('vewfldflt' =>'[~fltrow~]td.paytrmcod'.chr(9).'='.chr(9).chr(9). $this->data[self::ID] .chr(9).chr(9) );
			$lp_out['due'] = $lo_duemdl->getList( $lv_prm );
		}
