<?php
final class finmovdocacc extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'finmovdocacccod';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'finmovdocacccod'), 
																			$this->co_reg->db->sqldat($lp_in,'finmovdoccod'), 
																			$this->co_reg->db->sqldat($lp_in,'finacccod'),
																			$this->co_reg->db->sqldat($lp_in,'finacctxt'),
																			$this->co_reg->db->sqlnum($lp_in,'finmovdocacctot'),
																			$this->co_reg->db->sqldat($lp_in,'finmovdocaccblc'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'FIN_MOV_DOC_ACC_DEF (?,?,?,?,?,?,?,?,?,?,?)';
