<?php
final class edumodpln extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'edumodplncod';
  ...
  // GET CUR LIST. devuelve recordset de objetos (plan)
  function getCurList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if($lo_vew==null) { $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
		if ( $this->call_sp( '15', $lp_prm, $lo_out_data ) ) {
			$this->data = $lo_out_data;
		} else {
      $this->sysdata['sqlerr'] = $this->co_reg->db->getLastErrorMessage();
		}
  ...
  // GET CAR LIST. devuelve recordset de objetos (carrera)
  function getCarList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if($lo_vew==null) { $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
		if ( $this->call_sp( '16', $lp_prm, $lo_out_data ) ) {
			$this->data = $lo_out_data;
		} else {
      $this->sysdata['sqlerr'] = $this->co_reg->db->getLastErrorMessage();
		}
  ...
  // GET COU LIST. devuelve recordset de objetos (curso)
  function getCouList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if($lo_vew==null) { $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
  ...
  // GET SUB LIST. devuelve recordset de objetos (materias)
  function getSubList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if($lo_vew==null) { $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	$this->co_reg->db->sqldat($lp_in,'edumodplncod'), 
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'edumodplnatr'),
																		);
		$this->sysdata['sqltxt'] = 'EDU_MOD_PLN_DEF (?,?,?,?,?,?,?,?,?,?,?,?)';
    ...
    // devuelve recordset (segun la operacion)
		if ( $lp_action=='08' || $lp_action=='15' || $lp_action=='16' || $lp_action=='17' || $lp_action=='18'  ) {
      if($this->errtyp!='E'){
				$lp_out = $lo_rs;
      }else{
