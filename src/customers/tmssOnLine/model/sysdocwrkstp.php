<?php
final class sysdocwrkstp extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'wrkflwstpcod';
  ...
  function save( $lp_dat=array() ) {
    ...
    $lp_dat['wrkflwstpcnd'] = '<typ>'.(isset($lp_dat['wrkflwstpcndtyp']) ? $lp_dat['wrkflwstpcndtyp'] : '').'</typ>'.
      											'<val>'.(isset($lp_dat['wrkflwstpcndval']) ? $lp_dat['wrkflwstpcndval'] : '').'</val>';
    
    $lp_dat['wrkflwstpatr'] = '';
    // añadir validación de isset para los campos
    if(isset($lp_dat['wrkflwstpres'])){
      $i = 0; 
      foreach($lp_dat['wrkflwstpres'] as $lv_row){ 
        if(isset($lv_row['wrkflwstprestyp'])){
          $lp_dat['wrkflwstpatr'] .= '<res'.$i.'>';
          $lp_dat['wrkflwstpatr'] .= '<typ>'.($lv_row['wrkflwstprestyp']).'</typ>'.
                                    '<val>'.($lv_row['wrkflwstpresval']??'').'</val>';
          $lp_dat['wrkflwstpatr'] .= '</res'.$i.'>';
          $i++;
        }
      }
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwstpcod'),
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwcod'),
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwstpfrmcod'),
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwstptxt'),
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwstprow'),
                                     	(isset($lp_in['wrkflwstpcnd'])?$lp_in['wrkflwstpcnd']:''),
                                     	(isset($lp_in['wrkflwstpatr'])?$lp_in['wrkflwstpatr']:''),
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_DOC_WRK_STP_DEF (?,?,?,?,?,?,?,?,?,?,?,?)';
