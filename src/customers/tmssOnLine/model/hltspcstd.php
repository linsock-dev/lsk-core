<?php
final class hltspcstd extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	$this->co_reg->db->sqldat($lp_in,'spcstdcod'), 
																			$this->co_reg->db->sqldat($lp_in,'spcstdcodext'), 
																			$this->co_reg->db->sqldat($lp_in,'spcstdtxt'), 
																			$this->co_reg->db->sqldat($lp_in,'spccod'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($lp_in,'spcstdatr',false),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
                                     	$this->co_reg->db->sqldat($lp_in,'spcstdcmt',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_SPC_STD_DEF (?,?,?,?,?,?,?,?,?,?,?)';
