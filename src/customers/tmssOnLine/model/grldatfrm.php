<?php
final class grldatfrm extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  // GET FIELD LIST. devuelve lista de campos
  function getFieldList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if($lo_vew==null) {$lo_vew = $this->co_reg->load->model('grlvew');}
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions( $lp_vewopt );
		$lo_out_data = array();
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                     	$this->co_reg->db->sqldat($lp_in,'frmdatcod'),
                                      ...
                                      $this->co_reg->db->sqldat($lp_in,'sysdocfrmcod'),
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'frmdat', false),
                                     	$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_DAT_FRM_DEF (?,?,?,?,?,?,?,?,?,?,?)';
    ...
    //  GRLDATFRMFLD. carga los campos grabados del formulario
    if($lp_action=='03' && $this->errcod==0){
      $lo_grlfrmfldmdl = $this->co_reg->load->model('grldatfrm');
      $lv_prm = array('vewfldflt' =>'[~fltrow~]f.frmdatcod'.chr(9).'='.chr(9).chr(9).$lp_out['frmdatcod'].chr(9).chr(9));
      $lp_out['grldatfrmfld'] = $lo_grlfrmfldmdl->getFieldList( $lv_prm );
    }

		//  GRLDATUPL. carga definicion de archivos adjuntos a formulario
    if($lp_action=='03' && $this->errcod==0){
			$lo_grldatuplmdl = $this->co_reg->load->model('grldatupl');
			$lv_prm = array('vewfldflt'=> '[~fltrow~]f.flesrctyp'.chr(9).'='.chr(9).chr(9).'GRL_FRM'.chr(9).chr(9).
																		'[~fltrow~]f.flesrccod'.chr(9).'='.chr(9).chr(9).$lp_out['frmdatcod'].chr(9).chr(9));
			$lp_out['grldatfrmfle'] = $lo_grldatuplmdl->getList( $lv_prm );
		}
