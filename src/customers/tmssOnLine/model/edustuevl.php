<?php
final class edustuevl extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'evlcod';
	const SRCTYP = 'EDU_STU_EVL';
  ...
  // GET CONTROL LIST. devuelve recordset de datos (...)
  function getControlList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if ($lo_vew==null) { $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'eduevlatr001'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduevlatr002'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduevlatr003'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduevlatr004'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduevlatr005'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduevlatr006'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduevlatr007'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduevlatr008'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduevlatr009'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduplncod'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduplndtecod'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
                                     	$this->co_reg->db->sqldat($lp_in,'evlrelsts')
																		);
		$this->sysdata['sqltxt'] = 'EDU_STU_EVL_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' || $lp_action=='09' || $lp_action=='18') {
      if($this->errtyp!='E'){
				$lp_out = $lo_rs;
      }else{
