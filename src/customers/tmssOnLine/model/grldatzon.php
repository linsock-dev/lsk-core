<?php
final class grldatzon extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  // GET LIST AGRUP. deuelve recordset de objetos
  function getListAgrup( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_sys=false ) {
    if ( $lo_vew==null ) { $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
		if ( $this->call_sp( '28', $lp_prm, $lo_out_data, $lp_sys ) ) {
			$this->data = $lo_out_data;
		} else {
      $this->sysdata['sqlerr'] = $this->co_reg->db->getLastErrorMessage();
		}
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'adrzoncodext'), 
																			$this->co_reg->db->sqldat($lp_in,'adrzontxt'), 
																			$this->co_reg->db->sqldat($lp_in,'objtyp'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_DAT_ZON_DEF (?,?,?,?,?,?,?,?,?)';
