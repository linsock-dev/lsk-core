<?php
final class hltpatcls extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'hltpatclscod';
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	$this->co_reg->db->sqldat($lp_in,'hltpatclscod'), 
																			$this->co_reg->db->sqldat($lp_in,'hltpatclscodext'), 
																			$this->co_reg->db->sqldat($lp_in,'hltpatclstxt'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_PAT_CLS_DEF (?,?,?,?,?,?,?,?)';
