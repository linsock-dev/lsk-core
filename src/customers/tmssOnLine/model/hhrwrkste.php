<?php
final class hhrwrkste extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'wrkstecod';
	const OBJTYP = 'HHR_WKS';	
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'wrkstecodext'), 
																			$this->co_reg->db->sqldat($lp_in,'wrkstetxt'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'wrkstedes'),
																			$this->co_reg->db->sqldat($lp_in,'wrkstetyp')
																		);
		$this->sysdata['sqltxt'] = 'HHR_WRK_STE_DEF (?,?,?,?,?,?,?,?,?,?)';
