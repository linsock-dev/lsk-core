<?php
final class crmcntmtv extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array(), $lp_authCheck=true ) {
    ...
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? ($lp_authCheck?'02':'12') : ($lp_authCheck?'01':'11'));
		...
		$this->data['crmcntmtvatr'] = '<crmcntmtvfrmnme>'.$this->data['crmcntmtvfrmnme'].'</crmcntmtvfrmnme>';
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                      $this->co_reg->db->sqldat($lp_in,'crmcntmtvcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'crmcntmtvcodext'),
                                      $this->co_reg->db->sqldat($lp_in,'crmcntmtvtxt'),
                                      $this->co_reg->db->sqldat($lp_in,'crmcntmtvact'),
                                      ...
                                      $this->co_reg->db->sqldat($lp_in,'crmcntmtvfrm'),
                                      $this->co_reg->db->sqldat($lp_in,'crmcntmtvatr'),
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
                                      $this->co_reg->db->sqldat($lp_in,'autcod')
																		);
		$this->sysdata['sqltxt'] = 'CRM_CNT_MTV_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' || $lp_action=='18') {
      ...
      if ( $lo_rs && count($lo_rs)>0 ) {
		...
		if ( ($lp_action=='03' || $lp_action=='13') && $this->errcod==0 ) {
			$lo_cntmtvstsmdl = $this->co_reg->load->model('crmcntmtvsts'); 
			$lv_prm = array('vewfldflt'	=>'[~fltrow~]ms.crmcntmtvcod'.chr(9).'='.chr(9).chr(9).$this->data[self::ID].chr(9).chr(9).
																		'[~fltrow~]ms.docsts'.chr(9).'='.chr(9).chr(9).'A'.chr(9).chr(9));
			$lp_out['crmcntmtvsts'] = $lo_cntmtvstsmdl->getList($lv_prm);
		}
