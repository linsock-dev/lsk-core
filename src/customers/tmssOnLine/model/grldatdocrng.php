<?php
final class grldatdocrng extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'docrngcod';
	const SRCTYP = 'GRL_RNG';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'docrngtxt'), 
																			$this->co_reg->db->sqldat($lp_in,'objtypcod'), 
																			$this->co_reg->db->sqldat($lp_in,'docrngstrnum'), 
																			$this->co_reg->db->sqldat($lp_in,'docrngendnum'), 
																			$this->co_reg->db->sqldat($lp_in,'docrngcurnum'), 
																			$this->co_reg->db->sqldat($lp_in,'docrngatr',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_DAT_DOC_RNG_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?)';
