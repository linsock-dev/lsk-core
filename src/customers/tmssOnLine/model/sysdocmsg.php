<?php
final class sysdocmsg extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'sysdocmsgcod';
  ...
  function save( $lp_dat=array() ) {
    ...
    $this->data = $lp_dat;
		...
		$this->data['sysdocmsgatr']='<msgtyp>'.($lp_dat['sysdocmsgatrtyp']??'').'</msgtyp>'.
																'<msgfrmcnd>'.($lp_dat['sysdocmsgatrfrmcnd']??'').'</msgfrmcnd>'.
																'<msgfrm>'.($lp_dat['sysdocmsgatrfrm']??'').'</msgfrm>'.
																'<icn>'.($lp_dat['sysdocmsgatrfrm']??'').'</icn>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'sysdocmsgcodext'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdocmsgtxt',false), 
																			$this->co_reg->db->sqldat($lp_in,'objtypcod'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdocmsgatr',false),
																			$this->co_reg->db->sqldat($lp_in,'autcod'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_DOC_MSG_DEF (?,?,?,?,?,?,?,?,?,?,?)';
