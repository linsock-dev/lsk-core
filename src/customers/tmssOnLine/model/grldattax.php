<?php
final class grldattax extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
	...
	function load( $lp_key=array() ) {
		$this->data['taxsrctyp'] = $lp_key['taxsrctyp'];
		$this->data['taxsrccod'] = $lp_key['taxsrccod'];
		$lo_out_data = array();
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																		$this->co_reg->db->sqldat($lp_in,'taxnum'),
																		$this->co_reg->db->sqldat($lp_in,'taxcatcod'),
																		$this->co_reg->db->sqldat($lp_in,'taxcod001'),
																		$this->co_reg->db->sqldat($lp_in,'taxcod002'),
																		$this->co_reg->db->sqldte($lp_in,'taxactstr'),
																		$this->co_reg->db->sqldat($lp_in,'idttypcod'),
																		$this->co_reg->db->sqldat($lp_in,'taxcod001'),
																		$this->co_reg->db->sqldat($lp_in,'docsts'),
																		$this->co_reg->db->sqldat($lp_in,'taxsrctyp'),
																		$this->co_reg->db->sqldat($lp_in,'taxsrccod'),
																		$this->co_reg->db->sqldat($lp_in,'lndcod'),
																		$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																	);
		$this->sysdata['sqltxt'] = 'GRL_DAT_TAX_DEF(?,?,?,?,?,?,?,?,null,?,?,?,?,?,?,?)';
