<?php
final class hltspcgrp extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $this->data['spcgrpatr'] = 		'<ord>'.($lp_dat['spcgrpatrord']??'').'</ord>' . 
      														'<img>'.($lp_dat['spcgrpatrimg']??'').'</img>' .
      														'<spc>'.($lp_dat['spcgrpatrspc']??'').'</spc>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	$this->co_reg->db->sqldat($lp_in,'spcgrpcod'), 
																			$this->co_reg->db->sqldat($lp_in,'spcgrpcodext'), 
																			$this->co_reg->db->sqldat($lp_in,'spcgrptxt'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($lp_in,'spcgrpatr',false),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_SPC_GRP_DEF (?,?,?,?,?,?,?,?,?)';
    ...
    //		ESPECIALIDADES. Se recuperan las especialidades de la clasificacion/grupo
		 if ( $lp_action=='03' && $this->errcod==0 ) {
      $lo_spcmdl = $this->co_reg->load->model('hltspc');
  	 	$lv_prm = array('vewfldflt' => '[~fltrow~]spccod'.chr(9).'IN'.chr(9).chr(9).str_replace(',', chr(10) , $this->co_reg->document->getTagValue($lp_out['spcgrpatr'],'spc')).chr(9).chr(9));
			$lo_rs = $lo_spcmdl->getList($lv_prm);
    	$lp_out['spcgrpatrspc'] = $lo_rs;
	 	}
