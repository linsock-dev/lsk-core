<?php
final class syssecusrprm extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  // SAVE. save single object (new or modify)
  function add( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
    $this->data = $lp_dat;
    $lv_act = '01'; //( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		$lo_out_data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	// GET DEFINITIONS. devuelve de la base de datos central la definición de parámetros
	public function getDefinitions( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if($lo_vew==null){$lo_vew = $this->co_reg->load->model('grlvew');}
		$lv_vewopt = $lo_vew->parseViewOptions( $lp_vewopt );
		$lv_sqlprm = array( '08', $this->co_reg->sec->usrcod, strtoupper(isset($lp_vewopt)?$lv_vewopt:''));
		$lv_sqltxt = 'SYS_SEC_USR_PRM_DEF (?,?,null,null,null,null,null,null,?)';		
    $this->sysdata['sqlstm'] = $this->co_reg->db->getSqlStatement($lv_sqltxt,$lv_sqlprm);
		$lo_rs = $this->co_reg->db->sqlstoredprocedure( $lv_sqltxt , $lv_sqlprm, 0 );
		return $lo_rs;
	}
	
	//  CALL_SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'prmcod'), 
																			$this->co_reg->db->sqldat($lp_in,'prmfld',false), 
																			$this->co_reg->db->sqldat($lp_in,'prmval',false), 
																			$this->co_reg->db->sqldat($lp_in,'prmvaldef',false), 
																			$this->co_reg->db->sqldat($lp_in,'prmobjtyp'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'), 
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'usrprmfltkey'), 
																			$this->co_reg->db->sqldat($lp_in,'usrprmfltinc'), 
																			$this->co_reg->db->sqldat($lp_in,'usrprmflttyp')
																		);
		$this->sysdata['sqltxt'] = 'SYS_SEC_USR_PRM_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
