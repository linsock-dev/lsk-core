<?php
final class grldatbnk extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		$lo_out_data = array();
		return $this->call_sp( $lv_act, $this->data, $this->data );
  }
	
	// LOAD. carga el objeto
	function load03( $lp_key=array() ) {
		$this->data = $lp_key;
		$lo_out_data = array();
	...
	function load( $lp_key=array() ) {
		$this->data['bnksrctyp'] = ($lp_key['bnksrctyp']??'');
		$this->data['bnksrccod'] = ($lp_key['bnksrccod']??'');
		$lo_out_data = array();
		return $this->call_sp( '05', $this->data, $this->data );
	}
	
	// DELETE. carga el objeto
	function delete( $lp_key=array() ) {
		$this->data = $lp_key;
		$lo_out_data = array();
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {		
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'bnknum'), 
																			$this->co_reg->db->sqldat($lp_in,'bnkcod'), 
																			$this->co_reg->db->sqldat($lp_in,'bnkacccbu'), 
																			$this->co_reg->db->sqldat($lp_in,'bnkbch'), 
																			$this->co_reg->db->sqldat($lp_in,'bnkaccnum'), 
																			$this->co_reg->db->sqldat($lp_in,'bnkacctyp'), 
																			$this->co_reg->db->sqldat($lp_in,'paymthcod'), 
																			$this->co_reg->db->sqldat($lp_in,'idttypcod'), 
																			$this->co_reg->db->sqldat($lp_in,'taxcod001'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'), 
																			$this->co_reg->db->sqldat($lp_in,'bnksrctyp'), 
																			$this->co_reg->db->sqldat($lp_in,'bnksrccod'), 
																			$this->co_reg->db->sqldat($lp_in,'bnkacccbuals'), 
																			$this->co_reg->db->sqldat($lp_in,'bnktjtnum'), 
																			$this->co_reg->db->sqldat($lp_in,'bnktjttxt'), 
																			$this->co_reg->db->sqldte($lp_in,'bnktjtenddte'), 
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_DAT_BNK_DEF(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
