<?php
final class buysup extends tmssAction2 {
	const OBJTYP = 'BUY_SUP';
  function initialize(){ $this->ID = 'supcod'; }
  ...
  function save( $lp_dat=array(), $lp_authCheck=true ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
		$lp_dat['adrnme001'] = ($lp_dat['suptxt']??'');
  ...
  function changeClass( $lp_dat=array() ) {
	...
	protected function call_sp( $lp_action, $lp_in, &$lp_out ) {		
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'supcodext'),
																			$this->co_reg->db->sqldat($lp_in,'suptxt'),
                                     	$this->co_reg->db->sqldat($lp_in,'supcmt'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata,'view_options', false),
                                     	$this->co_reg->db->sqldat($lp_in,'sysdocclscod'),	
                                     	$this->co_reg->db->sqldat($lp_in,'paytrmcod'),
                                      $this->co_reg->db->sqldat($lp_in,'buysuponetme')
																		);
		$this->sysdata['sqltxt'] = 'BUY_SUP_DEF (?,?,?,?,?,?,?,?,?,?,?,?)';
