<?php
final class sysdocfrmfld extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'sysdocfrmfldcod';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocfrmfldcod'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocfrmfldcodext'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocfrmcod'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysfldinptyp'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocfrmfldord'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocfrmfldatr', false),
                                     	$this->co_reg->db->sqldat($lp_in, 'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata, 'view_options')
																		);
		$this->sysdata['sqltxt'] = 'SYS_DOC_FRM_FLD_DEF (?,?,?,?,?,?,?,?,?,?,?)';
