<?php
final class stkmatcls extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'matclscod';
	const SRCTYP = 'STK_CLS';
  ...
  function save( $lp_dat=array() ) {
    ...
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		...
		$this->data['matclsatr']= '<matclsord>'.$this->data['matclsord'].'</matclsord>'.
															'<matclsimg>'.$this->data['matclsimg'].'</matclsimg>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in, 'matclscodext'),
                                     	$this->co_reg->db->sqldat($lp_in, 'matclstxt'),
																			$this->co_reg->db->sqldat($lp_in, 'matclsatr', false), 
                                    	$this->co_reg->db->sqldat($lp_in, 'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false)
                                     	//(isset($this->sysdata['view_options'])?$this->sysdata['view_options']:'')	
																		);
		$this->sysdata['sqltxt'] = 'STK_MAT_CLS_DEF (?,?,?,?,?,?,?,?,?)';
