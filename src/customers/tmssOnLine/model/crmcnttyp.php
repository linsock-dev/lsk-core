<?php
final class crmcnttyp extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array(), $lp_authCheck=true ) {
    ...
    $this->data = $lp_dat;
    //Nuevo
		$this->data['crmcnttypatr'] = '<ord>'.$lp_dat['crmcnttypatrord'].'</ord>'
																	.'<clr>'.$lp_dat['crmcnttypatrclr'].'</clr>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_authCheck=true ) {
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                      ...
                                      $this->co_reg->db->sqldat($lp_in,'crmcnttypcodext'),
                                      $this->co_reg->db->sqldat($lp_in,'crmcnttyptxt'),
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
                                      $this->co_reg->db->sqldat($lp_in,'autcod'),
                                      $this->co_reg->db->sqldat($lp_in,'crmcnttypatr',false)
                                     );
    $this->sysdata['sqltxt'] = 'CRM_CNT_TYP_DEF (?,?,?,?,?,?,?,?,?,?)';
