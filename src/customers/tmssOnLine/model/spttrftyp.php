<?php
final class spttrftyp extends tmssAction {
  protected $co_reg;
  private $data = array(); 
	private $sysdata = array();
	const ID = 'spttrftypcod';
  const OBJTYP = 'SPT_TRT';
  ...
  function save( $lp_dat=array(), $lp_authCheck=true ) {
    ...
    $lp_dat['spttrftypatr']='<prd>'.($lp_dat['spttrftypprd']??'').'</prd>'.
      											'<act>'.($lp_dat['spttrftypact']??'').'</act>'.
                            '<trn>'.($lp_dat['spttrftyptrn']??'').'</trn>'. 
                            '<ptn>'.($lp_dat['spttrftypptn']??'').'</ptn>';
    $this->data = $lp_dat;
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_authCheck=true ) {
    ...
    $lv_buscod = (isset($this->sysdata['buscod'])?$this->sysdata['buscod']:$this->co_reg->sec->buscod);
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $lv_buscod, 
                                     ...
                                     $this->co_reg->db->sqldat($lp_in,'spttrftypcodext'),
                                     $this->co_reg->db->sqldat($lp_in,'spttrftyptxt'),
                                     $this->co_reg->db->sqldat($lp_in,'spttrftypatr'),
                                     $this->co_reg->db->sqldat($lp_in,'docsts'),
                                     $this->co_reg->db->sqldat($this->sysdata,'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'SPT_TRF_TYP_DEF (?,?,?,?,?,?,?,?,?)';
