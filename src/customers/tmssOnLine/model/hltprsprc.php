<?php
final class hltprsprc extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'prsprccod'), 
																			$this->co_reg->db->sqldat($lp_in,'prscod'), 
																			$this->co_reg->db->sqldat($lp_in,'patcod'), 
																			$this->co_reg->db->sqldat($lp_in,'spccod'), 
																			$this->co_reg->db->sqldat($lp_in,'prsprctyp'), 
																			$this->co_reg->db->sqlnum($lp_in,'prsprcval'), 
																			$this->co_reg->db->sqldat($lp_in,'curcod'), 
																			$this->co_reg->db->sqldte($lp_in,'prsprcdtestr'), 
																			$this->co_reg->db->sqldte($lp_in,'prsprcdteend'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'hltdisclscod')
																		);
		$this->sysdata['sqltxt'] = 'HLT_PRS_PRC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
