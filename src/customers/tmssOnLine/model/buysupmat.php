<?php
final class buysupmat extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'buysupmatcod';
  const SRCTYP = 'BUY_SUP_MAT';                                                                                                                           // **************************************
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	$this->co_reg->db->sqldat($lp_in,'buysupmatcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'supcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'matcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'supmatcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'supmattxt'),
                                     	$this->co_reg->db->sqldat($lp_in,'supmatuntcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'supmatatr',false),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata,'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'BUY_SUP_MAT_DEF (?,?,?,?,?,?,?,?,?,?,?,?)';
