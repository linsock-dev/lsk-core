<?php
final class hltcat extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'hltcattxt'), 
																			$this->co_reg->db->sqldat($lp_in,'hltcattyp'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_CAT_DEF (?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='03' && $this->errcod==0 ) {
			...
			$lv_vew = array('vewfldflt' =>'[~fltrow~]hltcatcod'.chr(9).'='.chr(9).chr(9).$lp_in['hltcatcod'].chr(9).chr(9).
                      							'[~fltrow~]docsts'.chr(9).'='.chr(9).chr(9).'A'.chr(9).chr(9),
											'vewmaxrec' =>'100');
      $lv_prm = array(self::ID => $lp_in[self::ID]);
			$lp_out['hltcatval'] = $lo_docval->getList( $lv_vew, $lv_prm );
		}
