<?php
final class edutchlqddoc extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'edutchlqddoccod';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'edutchlqddoccod'), 
																			...
																			$this->co_reg->db->sqldat($lp_in,'edutchlqddoccodext'), 
																			$this->co_reg->db->sqldat($lp_in,'edutchlqddoctxt'), 
																			$this->co_reg->db->sqlnum($lp_in,'edutchlqddoctot'),
																			$this->co_reg->db->sqldat($lp_in,'curcod'), 
																			$this->co_reg->db->sqldat($lp_in,'edutchlqddocatr001',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'EDU_TCH_LQD_DOC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
