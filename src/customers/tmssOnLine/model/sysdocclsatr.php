<?php
final class sysdocclsatr extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'sysdocclsatrcod';
  ...
  // GET LIST by TEXT. get list of objects by description
  function getListByText( $lp_vewopt=array(), $lp_prm=array() ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,
																			$this->co_reg->db->sqldat($lp_in,'sysdocclsatrcod'),
																			$this->co_reg->db->sqldat($lp_in,'objtyp'),
																			$this->co_reg->db->sqldat($lp_in,'sysdocclsatrcodext'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdocclsatrtxt'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
    $this->sysdata['sqltxt'] = 'SYS_DOC_CLS_ATR_DEF (?,?,?,?,?,?,?,?)';
