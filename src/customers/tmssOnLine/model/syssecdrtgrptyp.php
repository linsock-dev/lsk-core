<?php
final class syssecdrtgrptyp extends tmssAction{
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	private $buscod = '';
	const ID = 'syssecdrtgrptypcod';
	const OBJTYP = 'SYS_DGT';
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, 
																			$this->co_reg->db->sqldat($lp_in,'syssecdrtgrptypcod'), 
                                     	$this->co_reg->db->sqldat($lp_in,'syssecdrtgrpcod'), 
                                     	$this->co_reg->db->sqldat($lp_in,'syssecdrttypcod'), 
                                     	$this->co_reg->db->sqldat($lp_in,'syssecdrtgrptypdefval',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_SEC_DRT_GRP_TYP_DEF (?,?,?,?,?,?,?,?)';
