<?php
final class sysdocwrkact extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'wrkflwactcod';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwactcod'),
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwcod'),
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwstpcod'),
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwactevt'),
                                    	$this->co_reg->db->sqldat($lp_in,'wrkflwacttyp'),
																			(isset($lp_in['wrkflwactval'])?utf8_decode($lp_in['wrkflwactval']):''),
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_DOC_WRK_ACT_DEF (?,?,?,?,?,?,?,?,?,?,?)';
