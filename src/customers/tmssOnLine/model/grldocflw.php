<?php
final class grldocflw extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'docflwposcod';
  const SRCTYP = 'GRL_FLW';
  ...
  // DELETE ALL. borra el objeto
  function deleteAll( $lo_dat=array() ) {
	...
	// GET DOCUMENT TREE. devuelve el arbol de documentos
  function getDocumentTree( $lp_key=array() ) {
		$this->call_sp( '05', $lp_key, $this->data );
		return $this->data;
  }
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'docflwposcod'), 
																			$this->co_reg->db->sqldat($lp_in,'srcobjtyp'), 
																			$this->co_reg->db->sqldat($lp_in,'srcobjcod'), 
																			$this->co_reg->db->sqldat($lp_in,'srcposcod'), 
																			$this->co_reg->db->sqlnum($lp_in,'srcposqty'), 
																			$this->co_reg->db->sqldat($lp_in,'srcuntcod'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'), 
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_DOC_FLW_DEF (?,?,?,?,?,?,?,?,?,?,?)';
