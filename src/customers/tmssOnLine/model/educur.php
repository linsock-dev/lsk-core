<?php
final class educur extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_authCheck=true ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'educurcodext'),
                                     	$this->co_reg->db->sqldat($lp_in,'educurtxt'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata, 'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'EDU_CUR_DEF (?,?,?, ?,?,?,?,?)';
    ...
    if ( $lp_action=='08' || $lp_action=='18') {
      ...
      $lo_plnmdl = $this->co_reg->load->model('educurpln');
      $lv_prm = array('vewfldflt' => '[~fltrow~]p.educurcod'.chr(9).'='.chr(9).chr(9).$this->data[self::ID].chr(9).chr(9));
      $lo_rs = $lo_plnmdl->getList($lv_prm);
      $lp_out['educurpln'] = $lo_rs;
    }
