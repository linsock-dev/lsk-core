<?php
final class sysdocclsfrm extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocclsfrmcod'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocclscod'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocfrmcod'),
                                      $this->co_reg->db->sqldat($lp_in, 'cndtyp'),
                                      $this->co_reg->db->sqldat($lp_in, 'cndval'),
                                      $this->co_reg->db->sqldat($lp_in, 'sysdocclsfrmreq'),
                                     	$this->co_reg->db->sqldat($lp_in, 'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata, 'view_options')
																		);
		$this->sysdata['sqltxt'] = 'SYS_DOC_CLS_FRM_DEF (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
