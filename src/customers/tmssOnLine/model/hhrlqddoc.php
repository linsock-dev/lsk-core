<?php
final class hhrlqddoc extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'hhrlqddoccod';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'hhrlqddoccod'), 
																			...
																			$this->co_reg->db->sqldat($lp_in,'hhrlqddoccodext'), 
																			$this->co_reg->db->sqldat($lp_in,'hhrlqddoctxt'), 
																			$this->co_reg->db->sqldte($lp_in,'hhrlqddocdte'),
																			$this->co_reg->db->sqlnum($lp_in,'hhrlqddocqty'),
																			$this->co_reg->db->sqlnum($lp_in,'hhrlqddocprc'),
																			$this->co_reg->db->sqlnum($lp_in,'hhrlqddoctot'),
																			$this->co_reg->db->sqldat($lp_in,'curcod'), 
																			$this->co_reg->db->sqldat($lp_in,'hhrlqddocatr001',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HHR_LQD_DOC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
