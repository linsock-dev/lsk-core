<?php
final class tsrbnkacc extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'bnkacccod';
  const OBJTYP = 'TSR_BNA';  
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $lv_buscod,
                                     	$this->co_reg->db->sqldat($lp_in,'bnkacccod'),
																			$this->co_reg->db->sqldat($lp_in,'bnkacccodext'), 
                                     	$this->co_reg->db->sqldat($lp_in,'bnkacctxt'),
                                    	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($lp_in,'curcod'),
                                      $this->co_reg->db->sqldat($this->sysdata,'view_options',false),
                                    );
		$this->sysdata['sqltxt'] = 'TSR_BNK_ACC_DEF (?,?,?,?,?,?,?,?,?)';
    ...
    //		A C C
		if ( ($lp_action=='01' || $lp_action=='02' || $lp_action=='03'  || $lp_action=='11' || $lp_action=='12' || $lp_action=='13') && $this->errcod==0 ) {
			...
			$lp_in['accsrccod'] = $this->data[self::ID];																	 
