<?php
final class crmcntprt extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'crmcntprtcod';
  ...
  function save( $lp_dat=array(), $lp_authCheck=true ) {
    ...
    $lp_dat['crmcntprtatr'] = '<ord>'.(isset($lp_dat['crmcntprtatrord'])?$lp_dat['crmcntprtatrord']:'').'</ord>'
                              .'<clr>'.(isset($lp_dat['crmcntprtatrclr'])?$lp_dat['crmcntprtatrclr']:'').'</clr>';
    $this->data = $lp_dat; 
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_authCheck=true ) {
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                      $this->co_reg->db->sqldat($lp_in,'crmcntprtcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'crmcntprtcodext'),
                                      $this->co_reg->db->sqldat($lp_in,'crmcntprttxt'),
                                      $this->co_reg->db->sqldat($lp_in,'crmcntprtord'),
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
                                      $this->co_reg->db->sqldat($lp_in,'crmcntprtatr', false)
																		);
    $this->sysdata['sqltxt'] = 'CRM_CNT_PRT_DEF (?,?,?,?,?,?,?,?,?,?)';
