<?php
final class grldatmsg extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  // GET MESSAGE LIST. devuelve recordset de objetos (mensajes)     
  function getMessageList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if( $lo_vew==null ) { $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
		if ( $this->call_sp((($lp_prm['srcobjtyp']!='GRL_RPT')?'28':'29'), $lp_prm, $lo_out_data ) ) {
			$this->data = $lo_out_data;
		} else {
      $this->sysdata['sqlerr'] = $this->co_reg->db->getLastErrorMessage();
		}
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'msgnum'), 
																			$this->co_reg->db->sqldat($lp_in,'srcobjtyp'),
																			$this->co_reg->db->sqldat($lp_in,'srcobjcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'srcobjcod002'),
																			$this->co_reg->db->sqldat($lp_in,'sysdocmsgcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'msgatr',false),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
                                     	$this->co_reg->db->sqldat($lp_in,'errobj',false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_DAT_MSG_DEF (?,?,?,?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' || $lp_action=='28' || $lp_action=='29') {
      if($this->errtyp!='E'){
				$lp_out = $lo_rs;
      }else{
