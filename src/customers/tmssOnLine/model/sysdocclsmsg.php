<?php
final class sysdocclsmsg extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $lp_dat['sysdocclsmsgatr'] = '<evt>'.($lp_dat['sysdocclsmsgatrevt']??'').'</evt>';
    $this->data = $lp_dat;
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'sysdocclsmsgcod'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdocclscod'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdocmsgcod'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdocclsmsgatr',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_DOC_CLS_MSG_DEF (?,?,?,?,?,?,?,?,?)';
