<?php
final class edulqddoc extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'edulqddoccod';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                     	$this->co_reg->db->sqldat($lp_in,'edulqddoccod'),
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'edulqddoccodext'),
                                     	$this->co_reg->db->sqldat($lp_in,'edulqddoctxt'),
                                     	$this->co_reg->db->sqldte($lp_in,'edulqddocdte'),
                                     	$this->co_reg->db->sqlnum($lp_in,'edulqddocqty'),
                                     	$this->co_reg->db->sqlnum($lp_in,'edulqddocprc'),
                                     	$this->co_reg->db->sqlnum($lp_in,'edulqddoctot'),
                                     	$this->co_reg->db->sqldat($lp_in,'curcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'edulqddocatr001'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata, 'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'EDU_LQD_DOC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
