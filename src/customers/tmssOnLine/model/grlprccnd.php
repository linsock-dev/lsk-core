<?php
final class grlprccnd extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'prccndcod';
  const SRCTYP = 'GRL_PCD';
  ...
  function save( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
		...
		$lp_dat['prccndatr'] = '<mdlcod>'.($lp_dat['prccndatrmdlcod']??'').'</mdlcod>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->errcod = 0;
		$this->errtxt = '';
    $lv_buscod = (isset($lp_in['buscod'])?$lp_in['buscod']:$this->co_reg->sec->buscod);
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $lv_buscod,
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndcod'),
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndcodext'),
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndtxt'),
                                     	$this->co_reg->db->sqldat($lp_in, 'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndcatcod'),
                                     	$this->co_reg->db->sqldat($lp_in, 'fintaxtypcod'),
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndatr', false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_PRC_CND_DEF (?,?,?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' ) {
      ...
      if ( $lo_rs && count($lo_rs)>0 ) {
		...
		if ( $lp_action=='03' && $this->errcod==0 ) {
			...
			$lv_prm = array('vewfldflt' =>'[~fltrow~]pca.prccndcod'.chr(9).'='.chr(9).chr(9).$lp_in['prccndcod'].chr(9).chr(9),
											'vewfldord'=>'pca.prccndaccseqord');
			$lp_out['prccndaccseq'] = $lo_accmdl->getList( $lv_prm );
		}
