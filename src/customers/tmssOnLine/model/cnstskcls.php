<?php
final class cnstskcls extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'cnstskclscod';
  const SRCTYP = 'CNS_TSC';                                                                                                                           // **************************************
  ...
  function save( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
		...
		$lp_dat['cnstskclsatr'] = '<regass>'.(isset($lp_dat['cnstskclsatrregass'])?(strtolower($lp_dat['cnstskclsatrregass'])=='on' || $lp_dat['cnstskclsatrregass']=='1' ?'1':'0'):'0').'</regass>'.
															'<regprg>'.(isset($lp_dat['cnstskclsatrregprg'])?(strtolower($lp_dat['cnstskclsatrregprg'])=='on' || $lp_dat['cnstskclsatrregprg']=='1' ?'1':'0'):'0').'</regprg>';
    $this->data = $lp_dat;
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'cnstskclscodext'), 
                                      $this->co_reg->db->sqldat($lp_in,'cnstskclstxt'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'cnstskclsatr',false)
																		);
		$this->sysdata['sqltxt'] = 'CNS_TSK_CLS_DEF (?,?,?,?,?,?,?,?,?)';
