<?php
final class grlprcsch extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'prcschcod';
  const SRCTYP = 'GRL_PSC';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in, 'prcschcodext'),
                                     	$this->co_reg->db->sqldat($lp_in, 'prcschtxt'),
                                     	$this->co_reg->db->sqldat($lp_in, 'mdlcod'),
                                     	$this->co_reg->db->sqldat($lp_in, 'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata, 'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_PRC_SCH_DEF (?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' ) {
      ...
      if ( $lo_rs && count($lo_rs)>0 ) {
		...
		if ( $lp_action=='03' && $this->errcod==0 ) {
			$lo_prcschcndmdl = $this->co_reg->load->model('grlprcschcnd');
			$lv_prm = array('vewfldflt' =>'[~fltrow~]psc.prcschcod'.chr(9).'='.chr(9).chr(9).$lp_in['prcschcod'].chr(9).chr(9));
			$lp_out['prcschcnd'] = $lo_prcschcndmdl->getList( $lv_prm );
		}
    
    // P A R Á M E T R O    S I N C R O N I Z A C I Ó N
    if ( $lp_action=='03' && $this->errcod==0 ) {
      $lv_prcper = true;
    	// busco parámetro
      ...
      if(count($lo_prm_rs)>0){
        if($lo_prm_rs[0] != $this->co_reg->sec->buscod){
          $lv_prcper = false;
        }
      }
      $lp_out['prcper'] = $lv_prcper;
    }
