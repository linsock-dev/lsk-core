<?php
final class sysappprgvew extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $this->data = $lp_dat;
		...
		$this->data['vewatr'] = '<typcod>'.($lp_dat['vewatrtypcod']??'').'</typcod>'.
														'<typprg>'.($lp_dat['vewatrtypprg']??'').'</typprg>'.
														'<typtxt>'.($lp_dat['vewatrtyptxt']??'').'</typtxt>';
														
		$lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		
		// para claves primarias manuales:
		// asigno el codigo nuevo al codigo existente luego de determinar la accion
		$this->data[self::ID] = ($this->data[self::ID]!=''?$this->data[self::ID]:($this->data['vewcodnew']??''));
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'vewttl'),
																			$this->co_reg->db->sqldat($lp_in,'vewsrc'),
																			$this->co_reg->db->sqldat($lp_in,'vewsrcsys'),
																			$this->co_reg->db->sqldat($lp_in,'vewpic'),
																			$this->co_reg->db->sqldat($lp_in,'vewdefmaxrec'),
																			$this->co_reg->db->sqldat($lp_in,'vewalwsrt'),
																			$this->co_reg->db->sqldat($lp_in,'vewalwflt'),
																			$this->co_reg->db->sqldat($lp_in,'vewselmod'),
																			$this->co_reg->db->sqldat($lp_in,'vewslc'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'vewatr',false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_APP_PRG_VEW_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='03' && $this->errcod==0 ) {
      ...
      $lv_prm=array('vewfldflt' => 	'[~fltrow~]vc.vewcod'.chr(9).'='.chr(9).chr(9).$lp_in[self::ID].chr(9).chr(9).
                                    '[~fltrow~]v.docsts'.chr(9).'='.chr(9).chr(9).'A'.chr(9).chr(9));
			$lp_out['sysvewcol'] = $lo_sysvewcolmdl->getList( $lv_prm );
		}
