<?php
final class edustdloc extends tmssAction2 {
	const OBJTYP = 'EDU_STL';	
  function initialize(){ $this->ID = 'stdloccod'; }
  ...
  function save( $lp_dat=array(), $lp_authCheck=true ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
		$lp_dat['adrnme001'] = $lp_dat['stdloctxt'];
	...
	protected function call_sp( $lp_action, $lp_in, &$lp_out ) {
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'stdloccodext'),
                                     	$this->co_reg->db->sqldat($lp_in,'stdloctxt'),
                                     	$this->co_reg->db->sqldat($lp_in,'stdloccmt'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
                                     	$this->co_reg->db->sqldat($lp_in,'sysdocclscod')
																		);
		$this->sysdata['sqltxt'] = 'EDU_STD_LOC_DEF (?,?,?,?,?,?,?,?,?,?)';
