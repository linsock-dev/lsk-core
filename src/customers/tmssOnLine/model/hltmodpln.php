<?php
final class hltmodpln extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'hltmodplncod';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'hltmodplncod'), 
																			...
																			$this->co_reg->db->sqldat($lp_in,'hltmodplnatr',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_MOD_PLN_DEF (?,?,?,?,?,?,?,?,?,?,?)';
