<?php
final class educurpln extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  // GET CUR LIST. devuelve recorset de objetos (plan)
  ...
  // GET CAR LIST. devuelve recordset de objetos (carreras)
  ...
  // GET COU LIST. devuelve recordset de objetos (cursos)
  ...
  // GET SUB LIST. devuele recordset de objetos (materias)
  function getSubList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if($lo_vew==null) { $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
		if ( $this->call_sp( '22', $lp_prm, $lo_out_data ) ) {
			$this->data = $lo_out_data;
		} else {
      $this->sysdata['sqlerr'] = $this->co_reg->db->getLastErrorMessage();
		}
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqltxt'] = 'EDU_CUR_PLN_DEF (?,?,?,?,?,?,?,?,?,?)';
    ...
    // devuelve recordset (segun la operacion)
		if ( $lp_action=='08' || $lp_action=='18' || $lp_action=='15' || $lp_action=='16' || $lp_action=='17' ) {
      if($this->errtyp!='E'){
				$lp_out = $lo_rs;
      }else{
