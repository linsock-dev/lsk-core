<?php
final class stkqmstag extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'qmstagcod';
  const OBJTYP = 'STK_QGP';
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                     	$this->co_reg->db->sqldat($lp_in,'qmstagcod'),
																			$this->co_reg->db->sqldat($lp_in,'qmstagcodext'),
                                      $this->co_reg->db->sqldat($lp_in,'qmstagtxt'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata,'view_options')
                                    );
		$this->sysdata['sqltxt'] = 'STK_QMS_TAG_DEF (?,?,?,?,?,?,?,?)';
