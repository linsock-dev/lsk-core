<?php
final class hhrlqdlck extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'hhrlqdlckcod';
	const SRCTYP = 'HHR_LQD_LCK';
  ...
  function save( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
		...
		$lp_dat['hhrlqdlckatr'] = '<hhr_cha>'.(strtoupper(isset($lp_dat['hhrlqdlckatrcha'])?$lp_dat['hhrlqdlckatrcha']:'')=='ON'?'X':'').'</hhr_cha>'.
															'<hhr_lic>'.(strtoupper(isset($lp_dat['hhrlqdlckatrlic'])?$lp_dat['hhrlqdlckatrlic']:'')=='ON'?'X':'').'</hhr_lic>'.
															'<hhr_sub>'.(strtoupper(isset($lp_dat['hhrlqdlckatrsub'])?$lp_dat['hhrlqdlckatrsub']:'')=='ON'?'X':'').'</hhr_sub>'.
															'<hhr_ass>'.(strtoupper(isset($lp_dat['hhrlqdlckatrass'])?$lp_dat['hhrlqdlckatrass']:'')=='ON'?'X':'').'</hhr_ass>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	// GET UNLOCK PERIOD. devuelve el período habilitado para la carga de documentación de HHRR
	function getUnlockPeriod( $lp_objtyp ) {
		$lp_prm = array('hhrlqdlckatr'=>'<objtyp>'.$lp_objtyp.'</objtyp>');
		$this->data = array('hhrlqdlckstrdte'=>'','hhrlqdlckenddte'=>'');
    $lo_out_data = array();
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
																			$this->co_reg->db->sqldat($lp_in,'hhrlqdlckcod'),
																			$this->co_reg->db->sqldte($lp_in,'hhrlqdlckstrdte'),
																			$this->co_reg->db->sqldte($lp_in,'hhrlqdlckenddte'),
																			$this->co_reg->db->sqldat($lp_in,'hhrlqdlckatr',false),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HHR_LQD_LCK_DEF (?,?,?,?,?,?,?,?,?)';
