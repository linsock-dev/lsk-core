<?php
final class edutchprc extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_authCheck=true ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	$this->co_reg->db->sqldat($lp_in,'tchprccod'),
                                      ...
                                      $this->co_reg->db->sqldat($lp_in,'edustuass'),
                                    	$this->co_reg->db->sqldat($lp_in,'tchprctyp'),
                                     	$this->co_reg->db->sqlnum($lp_in,'tchprcval'),
                                      $this->co_reg->db->sqldat($lp_in,'curcod'),
                                      $this->co_reg->db->sqldte($lp_in,'tchprcdtestr'),
                                      $this->co_reg->db->sqldte($lp_in,'tchprcdteend'),
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),	
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'EDU_TCH_PRC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
