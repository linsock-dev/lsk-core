<?php
final class grldatprc extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'prccndrowcod';
  const SRCTYP = 'GRL_DAT_PRC';
  ...
  // GET LIST  WITH SEQUENCE. devuelve recordset de objetos de precios con secuencia de acceso
  function getListWithSequence( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if ($lo_vew==null) { $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                     	$this->co_reg->db->sqldat($lp_in,'prccndrowcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'srcobjtyp'),
                                     	$this->co_reg->db->sqldat($lp_in,'srcobjcod001'),
                                     	$this->co_reg->db->sqldat($lp_in,'srcobjcod002'),
                                     	$this->co_reg->db->sqldat($lp_in,'prcschcndrow'),
                                     	$this->co_reg->db->sqldat($lp_in,'prcschcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'prccndcod'),
                                     	$this->co_reg->db->sqlnum($lp_in,'prccndval'),
                                     	$this->co_reg->db->sqldat($lp_in,'prccndcurcod'),
                                     	$this->co_reg->db->sqlnum($lp_in,'prccndqty'),
                                     	$this->co_reg->db->sqldat($lp_in,'prccnduntcod'),
                                     	$this->co_reg->db->sqlnum($lp_in,'prccndtot'),
                                     	$this->co_reg->db->sqldat($lp_in,'curcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata,'view_options', false),
                                     	$this->co_reg->db->sqldat($lp_in,'prcchgman'),
                                     	$this->co_reg->db->sqldte($lp_in,'prccndstrdte'),
                                     	$this->co_reg->db->sqldte($lp_in,'prccndenddte'),
                                     	$this->co_reg->db->sqldat($lp_in,'prccndacccod'),
                                     	$this->co_reg->db->sqldat($lp_in,'prccndtxt'),
                                     	$this->co_reg->db->sqldat($lp_in,'prccndstd')
																		);
    $this->sysdata['sqltxt'] = 'GRL_DAT_PRC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,null,null,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' || $lp_action=='18' || $lp_action=='03') {
      if($this->errtyp!='E'){
				$lp_out = $lo_rs;
      }else{
    ...
    //		E S C A L A S  D E  P R E C I O S
		if ( ($lp_action=='08' || $lp_action=='18') && $this->errcod==0 ) {
			$lo_prcscamdl = $this->co_reg->load->model('grldatprcsca');
      foreach($lp_out as &$lv_row){
      	$lv_prm = array('vewfldflt' =>'[~fltrow~]s.prccndrowcod'.chr(9).'='.chr(9).chr(9).$lv_row['prccndrowcod'].chr(9).chr(9));
				$lv_row['grldatprcsca'] = $lo_prcscamdl->getList( $lv_prm );
      }
      unset($lv_row);
		}
