<?php
final class syssecusrgrp extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function load( $lp_key=array() ) {
		$lo_out_data = array();
		if ( $this->call_sp( '08', $lp_key, $lo_out_data ) ) {
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,  $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'syssecusrgrp')
																		);
		$this->sysdata['sqltxt'] = 'SYS_SEC_USR_GRP_DEF (?,?,?,?,?,?,?,?)';
