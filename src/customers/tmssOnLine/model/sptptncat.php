<?php
final class sptptncat extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_authCheck=true ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     ...
                                     $this->co_reg->db->sqldat($lp_in,'ptncatcodext'),
                                     $this->co_reg->db->sqldat($lp_in,'ptncattxt'), 
                                     $this->co_reg->db->sqldat($lp_in,'ptncatchgman'),
                                     $this->co_reg->db->sqldat($lp_in,'ptncatagestr'),
                                     $this->co_reg->db->sqldat($lp_in,'ptncatageend'),
                                     $this->co_reg->db->sqldat($lp_in,'ptncatsex'),
                                     $this->co_reg->db->sqldat($lp_in,'spttrfcod'),
                                     $this->co_reg->db->sqldat($lp_in,'docsts'),
                                     $this->co_reg->db->sqldat($this->sysdata,'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'SPT_PTN_CAT_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?)';
