<?php
final class sysdoctag extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array(), $lp_authCheck=true ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
		$lp_dat['sysdoctagatr'] = '<clr>'.($lp_dat['sysdoctagatrclr']??'').'</clr>';
    $this->data = $lp_dat;
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_authCheck=true ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'sysdoctagcod'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdoctagcodext'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdoctagtxt'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdoctagatr',false),
																			$this->co_reg->db->sqldat($lp_in,'objtypcod'), 
																			$this->co_reg->db->sqldat($lp_in,'autcod'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_DOC_TAG_DEF (?,?,?,?,?,?,?,?,?,?,?)';
