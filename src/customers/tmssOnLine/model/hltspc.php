<?php
final class hltspc extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'spccod';
  const SRCTYP = 'HLT_SPC';
  ...
  function save( $lp_dat=array() ) {
    ...
    $this->data = $lp_dat;
		$this->data['spcatrval001'] = '<hltprsrlscod>'.$this->data['hltprsrlscod'].'</hltprsrlscod>' . 
																	'<spcstkctr>'.(isset($this->data['spcstkctr'])?( $this->data['spcstkctr'] == 'on' || $this->data['spcstkctr'] == '1' ? '1' : '0' ): '0' ).'</spcstkctr>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'spccodext'), 
																			$this->co_reg->db->sqldat($lp_in,'spctxt'), 
																			$this->co_reg->db->sqldat($lp_in,'spctyp'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'spcctrtyp'),
																			$this->co_reg->db->sqldat($lp_in,'spcclr'),
																			$this->co_reg->db->sqldat($lp_in,'spcfrm'),
																			$this->co_reg->db->sqldat($lp_in,'spcord'),
																			$this->co_reg->db->sqldat($lp_in,'spcgrpcod'),
																			$this->co_reg->db->sqldat($lp_in,'spcplntyp'),
																			$this->co_reg->db->sqldat($lp_in,'spcatrval001',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_SPC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
      ...
      if ( $lo_rs && count($lo_rs)>0 ) {
		...
		if ( $lp_action=='03' && $this->errcod==0 ) {
			...
			$lv_prm = array('vewfldflt' =>'[~fltrow~]t.delcod'.chr(9).'<>'.chr(9).chr(9).'0'.chr(9).chr(9).
																		...
																		'[~fltrow~]t.spccod'.chr(9).'='.chr(9).chr(9).$lp_in['spccod'].chr(9).chr(9),
											'vewfldord' => 't.tmeday'	);
