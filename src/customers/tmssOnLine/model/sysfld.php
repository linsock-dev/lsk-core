<?php
final class sysfld extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	// GET FIELD. obtiene la definición de un campo (en memoria)
  public function GetField( $lp_fld ) {
    If ( count($this->data)==0 ) { $this->InitFields(); }
    return ( isset($this->data[$lp_fld])?$this->data[$lp_fld]:array());
  }
	
  // INIT FIELD. obitiene la definición de campos (inicializa memoria)
  function InitFields() {
		$lo_out_data = array();
		if ( $this->call_sp( '18', array(), $lo_out_data ) ) {
			$this->data = array();
			foreach($lo_out_data as $lv_row){ $this->data[$lv_row['sysfld']] = $lv_row; }
		} else {
      $this->sysdata['sqlerr'] = $this->co_reg->db->getLastErrorMessage();
		}
    return $this->data;
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, 
																			$this->co_reg->db->sqldat($lp_in,'sysfld'), 
																			$this->co_reg->db->sqldat($lp_in,'sysfldinptyp'), 
																			$this->co_reg->db->sqldat($lp_in,'sysfldoutsze'), 
																			$this->co_reg->db->sqldat($lp_in,'sysfldmaxlng'), 
																			$this->co_reg->db->sqldat($lp_in,'sysfldico',false), 
																			$this->co_reg->db->sqldat($lp_in,'sysfldbtnjs',false), 
																			$this->co_reg->db->sqldat($lp_in,'sysfldkeyprs',false), 
																			$this->co_reg->db->sqldat($lp_in,'sysfldkeyup',false), 
																			$this->co_reg->db->sqldat($lp_in,'sysfldonblur',false), 
																			$this->co_reg->db->sqldat($lp_in,'sysfldonfocus',false), 
																			$this->co_reg->db->sqldat($lp_in,'sysfldonkeydown',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_GRL_FLD_DEF (?,?, ?,?,?,?,?,?,?,?,?,?,?,?,?)';
