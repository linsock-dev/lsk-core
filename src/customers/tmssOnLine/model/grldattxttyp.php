<?php
final class grldattxttyp extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'txttypcod';
  const SRCTYP = 'GRL_TXT_TYP';	
  ...
  function save( $lp_dat=array() ) {
    ...
    $lp_dat['txttypatr'] = '<cls>'.($lp_dat['txttypatrcls']??'').'</cls>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_sys=false ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out, $lp_sys=false ) {
		$this->errtyp = 'S';
		$this->errcod = 0;
		$this->errtxt = '';
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'txttypcod'), 
																			$this->co_reg->db->sqldat($lp_in,'txttypcodext'),
																			$this->co_reg->db->sqldat($lp_in,'txttyptxt'), 
																			$this->co_reg->db->sqldat($lp_in,'txttypatr',false), 
																			$this->co_reg->db->sqldat($lp_in,'objtyp'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_DAT_TXT_TYP_DEF (?,?,?,?,?,?,?,?,?,?)';
    $this->sysdata['sqlstm'] = $this->co_reg->db->getSqlStatement($this->sysdata['sqltxt'],$this->sysdata['sqlprm']);
		$lo_rs = $this->co_reg->db->sqlstoredprocedure( $this->sysdata['sqltxt'] , $this->sysdata['sqlprm'], ($lp_sys==false?1:0) );
