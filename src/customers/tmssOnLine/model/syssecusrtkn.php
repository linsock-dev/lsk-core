<?php
final class syssecusrtkn extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function __construct( &$lp_reg ) { $this->co_reg = $lp_reg; }
	...
	// CREATE. crea un nuevo token de usuario
	function create( $lp_prm=array() ) {
		$this->sysdata['view_options'] = (isset($lp_prm['tknkey'])?$lp_prm['tknkey']:'');
		return $this->call_sp( '01', $lp_prm, $this->data );
	}
	
	// LOAD. carga un token
	function load( $lp_prm=array() ) {
		$lp_prm['secusrcod'] = 'TEMASIS_WEB';
		return $this->call_sp( '13', $lp_prm, $this->data );
	}
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->errcod = 0;
		$this->errtxt = '';
		$this->sysdata['sqlprm'] = array( $lp_action, (isset($lp_in['secusrcod'])?$lp_in['secusrcod']:$this->co_reg->sec->usrcod),
																			$this->co_reg->db->sqldat($lp_in,'acctkncod'),
																			$this->co_reg->db->sqldat($lp_in,'usrcod'),
																			$this->co_reg->db->sqldat($lp_in,'acctkn'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_SEC_USR_TKN_DEF (?,?,?,?,?,?)';
