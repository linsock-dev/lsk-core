<?php
final class stkqmsprf extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'qmsprfcod';
  const OBJTYP = 'STK_QPF';
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                     	$this->co_reg->db->sqldat($lp_in,'qmsprfcod'),
																			$this->co_reg->db->sqldat($lp_in,'qmsprfcodext'),
                                      $this->co_reg->db->sqldat($lp_in,'qmsprftxt'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
                                    );
		$this->sysdata['sqltxt'] = 'STK_QMS_PRF_DEF (?,?,?,?,?,?,?,?)';
