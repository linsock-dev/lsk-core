<?php 
final class cnsbudmat extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
		...
		$lp_dat['budmatatr'] ='<tmestr>'.($lp_dat['budmatatrtmestr']??'').'</tmestr>'.
													'<tmeend>'.($lp_dat['budmatatrtmeend']??'').'</tmeend>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'budmatrow'),
																			$this->co_reg->db->sqldat($lp_in,'budmatatr',false)
																		);
		$this->sysdata['sqltxt'] = 'CNS_BUD_MAT_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
