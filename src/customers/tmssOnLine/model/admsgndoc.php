<?php
final class admsgndoc extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,$this->co_reg->sec->buscod,
                                      $this->co_reg->db->sqldat($lp_in,'sgndoccod'),
																			$this->co_reg->db->sqldat($lp_in,'sgncod'),
                                      $this->co_reg->db->sqldat($lp_in,'flecod'),
																			$this->co_reg->db->sqldat($lp_in,'sgndocsrctyp'),
																			$this->co_reg->db->sqldat($lp_in,'sgndocsrccod001'),
                                      $this->co_reg->db->sqldat($lp_in.'sgndocsrccod002'),
                                      $this->co_reg->db->sqldat($lp_in,'fletxt'),
                                      $this->co_reg->db->sqldat($lp_in,'flenme'),
                                      $this->co_reg->db->sqldat($lp_in,'flesrcurl'),
                                      $this->co_reg->db->sqldat($lp_in,'flesrcpst'),
                                      $this->co_reg->db->sqldat($lp_in,'sgnpwd'),
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false)
                                    );
		$this->sysdata['sqltxt'] = 'ADM_SGN_DOC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
