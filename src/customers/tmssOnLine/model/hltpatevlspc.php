<?php
final class hltpatevlspc extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'evlspccod'),
																			$this->co_reg->db->sqldat($lp_in,'evlcod'), 
																			$this->co_reg->db->sqldat($lp_in,'spccod'), 
																			$this->co_reg->db->sqldat($lp_in,'evlatrval001',false), 
																			$this->co_reg->db->sqldat($lp_in,'evlatrval002',false),
																			$this->co_reg->db->sqldat($lp_in,'evlatrval003',false),
																			$this->co_reg->db->sqldat($lp_in,'evlatrval004',false),
																			$this->co_reg->db->sqldat($lp_in,'evlatrval005',false),
																			$this->co_reg->db->sqldat($lp_in,'evlatrval006',false),
																			$this->co_reg->db->sqldat($lp_in,'evlatrval007',false),
																			$this->co_reg->db->sqldat($lp_in,'evlatrval008',false),
																			$this->co_reg->db->sqldat($lp_in,'evlatrval009',false),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_PAT_EVL_SPC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
