<?php
final class syssecdrtgrp extends tmssAction{
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	private $buscod = '';
	const ID = 'syssecdrtgrpcod';
	const OBJTYP = 'SYS_DRG';
  ...
  function save( $lp_dat=array() ) {
    ...
    $this->data['syssecdrtgrpsys'] = (isset($lp_dat['syssecdrtgrpsys']) ? (strtolower($lp_dat['syssecdrtgrpsys']) == "on" || $lp_dat['syssecdrtgrpsys'] == "1" ? 1 : 0) : 0);
    $this->data['syssecdrtgrpatr'] = '<syssecdrtgrptyp>'.$this->data['syssecdrtgrptyp'].'</syssecdrtgrptyp>';
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,
																			$this->co_reg->db->sqldat($lp_in,'syssecdrtgrpcod'), 
																			$this->co_reg->db->sqldat($lp_in,'syssecdrtgrpcodext'), 
																			$this->co_reg->db->sqldat($lp_in,'syssecdrtgrptxt'), 
																			$this->co_reg->db->sqldat($lp_in,'syssecdrtgrpsys'),
                                     	$this->co_reg->db->sqldat($lp_in,'syssecdrtgrpatr',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_SEC_DRT_GRP_DEF (?,?,?,?,?,?,?,?,?)';
    ...
    //		D I R E C T I V A S
      ...
      // busca directivas relacionadas
      $lo_drtgrptyp = $this->co_reg->load->model('syssecdrtgrptyp');
			$lv_prm = array('vewfldflt' =>'[~fltrow~]gt.syssecdrtgrpcod'.chr(9).'='.chr(9).chr(9).$lp_out['syssecdrtgrpcod'].chr(9).chr(9));
			$lo_rs = $lo_drtgrptyp->getList( $lv_prm );
            
      // arma un string con los id de las directivas
      $lv_typstr = '';
      foreach( $lo_rs as $lv_row ){ $lv_typstr .= ( $lv_typstr != '' ? chr(10) : '' ).$lv_row['syssecdrttypcod']; };
      
     // carga las directivas 
      $lo_typmdl = $this->co_reg->load->model('syssecdrttyp');
      $lv_prm =  array( 'vewfldflt' => '[~fltrow~]dt.syssecdrttypcod'.chr(9).'IN'.chr(9).chr(9).$lv_typstr.chr(9).chr(9));
      $lo_typrs = $lo_typmdl->getList( $lv_prm );
			
      //une los dos array
      $lo_finalArr = array();
      foreach( $lo_rs as $lv_row ){
        foreach( $lo_typrs as $lv_row2 ){
          if( $lv_row['syssecdrttypcod'] == $lv_row2['syssecdrttypcod'] ){ 
          	array_push( $lo_finalArr, array_merge( $lv_row, $lv_row2 ) ); 
          }
        }
      } 
      
      $lp_out['syssecdrtgrptyp'] = $lo_finalArr;
		}
