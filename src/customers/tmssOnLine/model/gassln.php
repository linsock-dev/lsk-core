<?php
final class gassln extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'slncod';
  const SRCTYP = 'GAS_SLN';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'slncod'), 
																			$this->co_reg->db->sqldat($lp_in,'slncodext'),
                                      $this->co_reg->db->sqldat($lp_in,'slntblcodext'), 
																			$this->co_reg->db->sqldat($lp_in,'slntblqty'), 
																			$this->co_reg->db->sqldat($lp_in,'slntxt'), 
																			$this->co_reg->db->sqldat($lp_in,'slntyp'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'cuscod'),
																			$this->co_reg->db->sqldat($lp_in,'strloccod'),
																			$this->co_reg->db->sqldat($lp_in,'sysdocclscod_ord'),
																			$this->co_reg->db->sqldat($lp_in,'sysdocclscod_stk'),
																			$this->co_reg->db->sqldat($lp_in,'slsprclstcod')
																		);
		$this->sysdata['sqltxt'] = 'GAS_SLN_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' || $lp_action=='13') {
      ...
      if ( $lo_rs && count($lo_rs)>0 ) {
		...
		if ( $lp_action=='03' && $this->errcod==0 ) {
			$lo_gasslntblmdl = $this->co_reg->load->model('gasslntbl');
			$lv_prm = array('vewfldflt' =>'[~fltrow~]slncod'.chr(9).'='.chr(9).chr(9). $this->data[self::ID] .chr(9).chr(9) ,
											'vewfldord' =>'slntblcodext');
			$lp_out['slntbls'] = $lo_gasslntblmdl->getList( $lv_prm );
		}
