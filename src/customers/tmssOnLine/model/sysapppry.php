<?php
final class sysapppry extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array() ) {
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,                                 
																			$this->co_reg->db->sqldat($lp_in,'sysappprycod'),
                                     	$this->co_reg->db->sqldat($lp_in,'sysappprycodext'), 
																			$this->co_reg->db->sqldat($lp_in,'sysappprytxt'),
																			$this->co_reg->db->sqldat($lp_in,'sysappprytkn'),
																			$this->co_reg->db->sqldat($lp_in,'sysappprysrctyp'), 
                                     	$this->co_reg->db->sqldat($lp_in,'sysappprysrcdat'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																		);
    $this->sysdata['sqltxt'] = 'SYS_APP_PRY_DEF (?,?,?,?,?,?,?,?,?,?,?)';
    ...
    //		A P I S
		if ( $lp_action=='03' && $this->errcod==0 ) {
			$lo_pryapimdl = $this->co_reg->load->model('sysapppryapi');
			$lv_prm = array('vewfldflt' =>'[~fltrow~]pa.sysappprycod'.chr(9).'='.chr(9).chr(9).$lp_out['sysappprycod'].chr(9).chr(9));
			$lo_rs = $lo_pryapimdl->getList( $lv_prm );
              
      //arma un string con los id de las directivas
      $lv_apistr = '';
      foreach( $lo_rs as $lv_row ){ $lv_apistr .= ( $lv_apistr != '' ? chr(10) : '' ).$lv_row['sysappapicod']; };
      
     //carga las directivas en la tabla de directivas
      $lo_apimdl = $this->co_reg->load->model('sysappapi');
      $lv_prm =  array( 'vewfldflt' => '[~fltrow~]a.sysappapicod'.chr(9).'IN'.chr(9).chr(9).$lv_apistr.chr(9).chr(9).
                                       '[~fltrow~]a.docsts'.chr(9).'='.chr(9).chr(9).'A'.chr(9).chr(9) );
      $lo_apirs = $lo_apimdl->getList( $lv_prm );
			
      //une los dos array para la tabla
      $lo_finalArr = array();
      foreach( $lo_rs as $lv_row ){
        foreach( $lo_apirs as $lv_row2 ){
          if( $lv_row['sysappapicod'] != $lv_row2['sysappapicod'] ){ continue; }
          array_push( $lo_finalArr, array_merge( $lv_row, $lv_row2 ) ); 
        }
      } 
      $lp_out['sysapppryapi'] = $lo_finalArr;
		}
