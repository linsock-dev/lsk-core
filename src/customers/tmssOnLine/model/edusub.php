<?php
final class edusub extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array(), $lp_authCheck=true ) {
    ...
    $this->data = $lp_dat;
    //Grabar el getTagValue - Pedido
		$this->data['edusubatr'] = '<subord>'.$lp_dat['edusubord'].'</subord>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_authCheck=true ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'edusubcodext'),
                                     	$this->co_reg->db->sqldat($lp_in,'edusubtxt'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
                                     	$this->co_reg->db->sqldat($lp_in,'eduevlfrm'),
                                     	$this->co_reg->db->sqldat($lp_in,'edusubatr')
																		);
		$this->sysdata['sqltxt'] = 'EDU_SUB_DEF (?,?,?,?,?,?,?,?,?,?)';
