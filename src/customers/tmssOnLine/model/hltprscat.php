<?php
final class hltprscat extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'hltprscatcod';
  ...
  // LOAD. devuelve recordset por prestador
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'hltprscatcod'), 
                                     	$this->co_reg->db->sqldat($lp_in,'hltcatcod'), 
                                     	$this->co_reg->db->sqldat($lp_in,'hltcatvalcod'), 
																			$this->co_reg->db->sqlnum($lp_in,'hltcatval'), 
																			$this->co_reg->db->sqldte($lp_in,'hltcatstrdte'), 
																			$this->co_reg->db->sqldte($lp_in,'hltcatenddte'), 
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_PRS_CAT_DEF (?,?,?,?,?,?,?,?,?,?,?,0)';
