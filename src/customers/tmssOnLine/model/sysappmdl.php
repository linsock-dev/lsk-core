<?php
final class sysappmdl extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		$this->data['mdlcod'] = $this->data['mdlcodext'];
		$lo_out_data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,  
																			...
																			$this->co_reg->db->sqldat($lp_in,'mdltxt'), 
																			$this->co_reg->db->sqldat($lp_in,'mdlord'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'syslngcod'), 
																			$this->co_reg->db->sqldat($lp_in,'mdlpic',false)
																		);
    $this->sysdata['sqltxt'] = 'SYS_APP_MDL_DEF (?,?,?,?,?,?,?,?,?)';
