<?php
final class sysappmdlprm extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		if($lv_act=='01'){ $this->data['mdlcod'] = $this->data['mdlcodext']; }
		$lo_out_data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	// GET PARAMETERS. devuelve parametros de usuario
  function getParameter( $lp_prm=array() ) {
		$lo_out_data = array();
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'mdlatrval001',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_APP_MDL_PRM_DEF (?,?,?,?,?,?,?)';
