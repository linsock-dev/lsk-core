<?php
final class fintaxcat extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array(),$lp_authCheck=true) {
    ...
    $this->data = $lp_dat;
    $lv_act = ($lp_dat['taxcatcodsve']=='' ? ($lp_authCheck?'01':'11') : ($lp_authCheck?'02':'12') ); // la accion se determina en el SP
		$lo_out_data = array();
  ...
  // GET LIST. devuelve recorset de objetos
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null,$lp_authCheck=true) {
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,$this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'taxcattxt'),
                                     	$this->co_reg->db->sqldat($lp_in,'taxcatdsc'),
																			$this->co_reg->db->sqldat($lp_in,'taxcatfct001'),
                                     	$this->co_reg->db->sqldat($lp_in,'taxcatfct002'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'FIN_TAX_CAT_DEF (?,?,?,?,?,?,?,?,?,?,?)';
