<?php
final class stkmatstklvl extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'stkmatlvlcod';
	const SRCTYP = 'STK_MAT';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'stkmatlvlcod'), 
																			$this->co_reg->db->sqldat($lp_in,'stkobjtyp'), 
																			$this->co_reg->db->sqldat($lp_in,'stkobjcod'), 
																			$this->co_reg->db->sqldat($lp_in,'stkcntcod'), 
																			$this->co_reg->db->sqldat($lp_in,'matcod'), 
																			$this->co_reg->db->sqlnum($lp_in,'stkmatlvlmax'), 
																			$this->co_reg->db->sqlnum($lp_in,'stkmatlvlmin'), 
																			$this->co_reg->db->sqlnum($lp_in,'stkmatlvlmip'), 
																			$this->co_reg->db->sqlnum($lp_in,'stkmatlvlres'), 
																			$this->co_reg->db->sqlnum($lp_in,'stkmatlvlrep'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'STK_MAT_STK_LVL_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
