<?php
final class hltprsrls extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $lp_dat['prsrlsatrval001'] = '<updusrprm>'.( isset($lp_dat['atrval001']) ? '1' : '0' ).'</updusrprm>';
    $this->data = $lp_dat;
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'prsrlscodext'), 
																			$this->co_reg->db->sqldat($lp_in,'prsrlstxt',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options'),
																			$this->co_reg->db->sqldat($lp_in,'prsrlsatrval001',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_PRS_RLS_DEF (?,?,?, ?,?,?,?,?,?)';
