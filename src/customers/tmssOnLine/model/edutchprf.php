<?php
final class edutchprf extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'edutchprfcod';
  ...
  public function setFormData( $lp_arr=array() ){
		//$this->co_reg->request->post['tchprfcod'] = (isset($lp_arr['tchprfcod'])?$lp_arr['tchprfcod']:''); 
		$this->co_reg->request->post['eduprfcod'] = (isset($lp_arr['eduprfcod'])?$lp_arr['eduprfcod']:''); 
		$this->co_reg->request->post['tchcod'] = (isset($lp_arr['tchcod'])?$lp_arr['tchcod']:'');
	}
	
	// CREATE. inicializa objeto
	function create() {
		$this->data = array();
		$this->sysdata = array();
	}
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                     	$this->co_reg->db->sqldat($lp_in,'tchprfcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'tchcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduprfcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'eduprfatr'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata, 'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'EDU_TCH_PRF_DEF (?,?,?,?,?,?,?,?,?)';
