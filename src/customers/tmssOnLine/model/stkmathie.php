<?php
final class stkmathie extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'mathiecod';
	const SRCTYP = 'STK_HIE';
  ...
  // LOAD PATH. Carga todos los niveles de una jerarquia
  function loadPath( $lp_key ) {
		$this->data[self::ID] = $lp_key[self::ID];
		$lo_out_data = array();
		if ( $this->call_sp( '13', $this->data, $lo_out_data ) ) {
			$this->data = $lo_out_data;
		} else {
      $this->sysdata['sqlerr'] = $this->co_reg->db->getLastErrorMessage();
		}
  ...
  // GET MATERIAL LIST. get list of objects
  function getMaterialList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if($lo_vew==null) {$lo_vew = $this->co_reg->load->model('grlvew');}
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'mathiecodext'), 
																			$this->co_reg->db->sqldat($lp_in,'mathietxt'),
																			$this->co_reg->db->sqldat($lp_in,'mathiepar'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'STK_MAT_HIE_DEF (?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' || $lp_action=='18' || $lp_action=='13') {
      if($this->errtyp!='E'){
				$lp_out = $lo_rs;
      }else{
