<?php
final class hhrtmereg extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'hhrtmeregcod';
  ...
  // GET LIST. deuelve recordset de objetos
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_sys=false ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'hhrtmeregcod'), 
																			$this->co_reg->db->sqldatetime((isset($lp_in['hhrtmeregdte'])?utf8_decode($lp_in['hhrtmeregdte']):'')), 
																			$this->co_reg->db->sqldat($lp_in,'srcobjtyp'), 
																			$this->co_reg->db->sqldat($lp_in,'srcobjcod001'), 
																			$this->co_reg->db->sqldat($lp_in,'srcobjcod002'),
                                      $this->co_reg->db->sqldat($lp_in,'hhrtmesrctyp'),
                                      $this->co_reg->db->sqldat($lp_in,'hhrtmesrccod001'),
                                      $this->co_reg->db->sqldat($lp_in,'hhrtmesrccod002'),
                                      $this->co_reg->db->sqlnum($lp_in,'hhrtmeregqty'),
                                      $this->co_reg->db->sqldat($lp_in,'hhrtmeregatr',false),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HHR_TME_REG_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
