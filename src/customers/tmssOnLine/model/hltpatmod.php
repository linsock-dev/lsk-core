<?php
final class hltpatmod extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'hltpatmodcod'), 
																			$this->co_reg->db->sqldat($lp_in,'patcod'), 
																			$this->co_reg->db->sqldat($lp_in,'hltmodcod'), 
																			$this->co_reg->db->sqldte($lp_in,'hltpatmodstrdte'),
																			$this->co_reg->db->sqldte($lp_in,'hltpatmodenddte'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'hltpatmodaut'),
																			$this->co_reg->db->sqldat($lp_in,'hltpatmodcmt',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_PAT_MOD_DEF (?,?,?,?,?,?,?,?,?,?,?,?)';
