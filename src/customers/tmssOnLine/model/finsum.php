<?php
final class finsum extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $lv_buscod,
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'finsumtot'),
                                     	$this->co_reg->db->sqldat($lp_in,'curcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options', false)
                                    );
		$this->sysdata['sqltxt'] = 'FIN_SUM_DEF (?,?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' || $lp_action=='18') {
      ...
      if ( $lo_rs && count($lo_rs)>0 ) {
		...
		if ( $lp_action=='03' && $this->errcod==0 ) {
			if($this->lo_finsumdoc==null){$this->lo_finsumdoc = $this->co_reg->load->model('finsumdoc');}
			$lv_prm = array('vewfldflt' =>'[~fltrow~]fd.finsumcod'.chr(9).'='.chr(9).chr(9).$lp_in['finsumcod'].chr(9).chr(9));
			$lp_out['finsumdoc'] = $this->lo_finsumdoc->getList( $lv_prm );
		} 
