<?php
final class slsordmat extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'slsordmatcod';
	const ID2 = 'slsordcod';
  const SRCTYP = 'SLS_ORD_MAT';
  ...
  function save( $lp_dat=array() ) {
    ...
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		...
		$this->data['slsordmatatr'] = '<slsordmatslstxt>'.utf8_decode($lp_dat['slsordmatslstxt']??'').'</slsordmatslstxt>';
  ...
  function getList($lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'slsordmatcod'), 
																			...
																			$this->co_reg->db->sqlnum($lp_in,'matprc'),
																			$this->co_reg->db->sqlnum($lp_in,'mattot'),
																			$this->co_reg->db->sqldat($lp_in,'curcod'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'slsordmatatr',false),
																			$this->co_reg->db->sqldat($lp_in,'docreftyp'),
																			$this->co_reg->db->sqldat($lp_in,'docrefcod'),
																			$this->co_reg->db->sqldat($lp_in,'docrefposcod'),
																			$this->co_reg->db->sqldat($lp_in,'sysdocrejcod'),
																			$this->co_reg->db->sqldat($lp_in,'slsordmatcmt',false)
																		);
		$this->sysdata['sqltxt'] = 'SLS_ORD_MAT_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
