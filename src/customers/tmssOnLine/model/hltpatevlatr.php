<?php
final class hltpatevlatr extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'evlatrcod',false), 
																			$this->co_reg->db->sqldat($lp_in,'medatrcod'),
																			$this->co_reg->db->sqldat($lp_in,'medatrval001',false), 
																			$this->co_reg->db->sqldat($lp_in,'medatrval002',false),
																			$this->co_reg->db->sqldat($lp_in,'medatrval003',false),
																			$this->co_reg->db->sqldat($lp_in,'docsts')
																		);
		$this->sysdata['sqltxt'] = 'HLT_PAT_EVL_ATR_DEF (?,?,?, ?,?,?,?,?,?,?)';
