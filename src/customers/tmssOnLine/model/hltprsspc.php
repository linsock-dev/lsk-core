<?php
final class hltprsspc extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'prsspccod';
  const SRCTYP = 'HLT_PRS_SPC';
  ...
  function __construct( &$lp_reg ) { $this->co_reg = $lp_reg; }
	...
	public function setFormData( $lp_arr=array() ){
		$this->co_reg->request->post['prsspccod'] = ($lp_arr['prsspccod']??''); 
		$this->co_reg->request->post['spccod'] = ($lp_arr['spccod']??''); 
		$this->co_reg->request->post['prscod'] = ($lp_arr['prscod']??''); 
		$this->co_reg->request->post['prsspcatr'] = (isset($lp_arr['mn'])?'<mn>'.$lp_arr['mn'].'</mn>':'').(isset($lp_arr['mp'])?'<mp>'.$lp_arr['mp'].'</mp>':''); 
	}
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'prsspccod'), 
																			$this->co_reg->db->sqldat($lp_in,'prscod'), 
																			$this->co_reg->db->sqldat($lp_in,'spccod'), 
																			$this->co_reg->db->sqldat($lp_in,'prsspcatr',false),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_PRS_SPC_DEF (?,?,?,?,?,?,?,?,?)';
