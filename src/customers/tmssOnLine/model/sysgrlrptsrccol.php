<?php
final class sysgrlrptsrccol extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'rptsrccolcod';
  const SRCTYP = '';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$lv_rptsrcsys = intval($lp_in['rptsrcsys']??'1');
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
																			$this->co_reg->db->sqldat($lp_in,'rptsrccolcod'), 
																			$this->co_reg->db->sqldat($lp_in,'rptsrccolcodext',false),
                                     	$this->co_reg->db->sqldat($lp_in,'rptsrccolcodint',false),
																			$this->co_reg->db->sqldat($lp_in,'rptsrccoltxt'), 
																			$this->co_reg->db->sqldat($lp_in,'rptsrccod'), 
																			$this->co_reg->db->sqldat($lp_in,'rptsrccolatr',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																			);
		$this->sysdata['sqltxt'] = 'SYS_GRL_RPT_SRC_COL_DEF (?,?,?,?,?,?,?,?,?,?,?)';
