<?php
final class slssvcmat extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'slssvcmatcod';
	const ID2 = 'slssvccod';
  const SRCTYP = 'SLS_SVC_MAT';
  ...
  function save( $lp_dat=array() ) {
    ...
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		$this->data['slssvcmatatr'] = '<slssvcmatslstxt>'.(isset($this->data['slssvcmatslstxt'])?utf8_decode($this->data['slssvcmatslstxt']):'').'</slssvcmatslstxt>';
		$lo_out_data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'slssvcmatcod'), 
																			...
																			$this->co_reg->db->sqldat($lp_in,'slssvcmatatr',false),
																			$this->co_reg->db->sqldte($lp_in,'slssvcstrdte'), 
																			$this->co_reg->db->sqldte($lp_in,'slssvcenddte')
																		);
		$this->sysdata['sqltxt'] = 'SLS_SVC_MAT_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
