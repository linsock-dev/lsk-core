<?php
final class syssecgrp extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'usrgrpcod';
  const SRCTYP = 'SYS_GRP';
  ...
  function save( $lp_dat=array() ) {
    ...
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		...
		if($lv_act=='01'){ $this->data['usrgrpcod'] = $this->data['usrgrpcodext']; }
		$this->data['usrgrpatr001'] = '<strpge>'.($this->data['strpge']??'').'</strpge>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																				...
																				$this->co_reg->db->sqldat($lp_in,'usrgrptxt'), 
																				$this->co_reg->db->sqldat($lp_in,'docsts'),
																				$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																				$this->co_reg->db->sqldat($lp_in,'usrautlvl'),
																				$this->co_reg->db->sqldat($lp_in,'usrgrpatr001',false)
																			);
		$this->sysdata['sqltxt'] = 'SYS_SEC_GRP_DEF (?,?,?,?,?,?,?,?,?)';
