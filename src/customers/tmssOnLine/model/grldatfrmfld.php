<?php
final class grldatfrmfld extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'frmdatfldcod';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod,
                                     	$this->co_reg->db->sqldat($lp_in,'frmdatfldcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'frmdatfldcodext'),
                                     	$this->co_reg->db->sqldat($lp_in,'frmdatcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'frmdatval', false),
                                     	$this->co_reg->db->sqldat($lp_in,'frmdocfldcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_DAT_FRM_FLD_DEF (?,?,?,?,?,?,?,?,?,?)';
