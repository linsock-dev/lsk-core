<?php 
final class grlprccndfor extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'prccndforcod';
  const SRCTYP = 'GRL_PCF';
  ...
  function save( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
		...
		$lp_dat['prccndforsrc'] = '<src>'.($lp_dat['prccndforsrcsrc']??'').'</src>'.
															'<val>'.($lp_dat['prccndforsrcval']??'').'</val>'.
															'<qty>'.($lp_dat['prccndforsrcqty']??'').'</qty>'.
															'<unt>'.($lp_dat['prccndforsrcunt']??'').'</unt>'.
															'<tot>'.($lp_dat['prccndforsrctot']??'').'</tot>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndforcod'),
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndforcodext'),
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndfortxt'),
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndforsrctyp'),
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndforsrc', false),
                                     	$this->co_reg->db->sqldat($lp_in, 'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndforsrczcu', false),
                                     	$this->co_reg->db->sqldat($lp_in, 'prccndforcmt')
																		);
		$this->sysdata['sqltxt'] = 'GRL_PRC_CND_FOR_DEF (?,?,?,?,?,?,?,?,?,?,?,?)';
