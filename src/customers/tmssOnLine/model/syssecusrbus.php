<?php
final class syssecusrbus extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function load( $lp_key=array() ) {
		$this->call_sp( '18', $lp_key, $this->data );
		return $this->data;
  }
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	// SET USER ACCEPTANCE TERMS OF USE. establecer Aceptación de Términos de Uso del usuario
	public function setUserAcceptanceTermsOfUse( $lp_buscod ) {
    $this->data = array( 'usrcod'=>$this->co_reg->sec->usrcod, 'buscod'=>$lp_buscod );		
		$this->call_sp( '21', $this->data, $this->data );
		return $this->data;
	}
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,
																			...
																			$this->co_reg->db->sqldat($lp_in,'buscod'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'syssecusrbus')
																		);
		$this->sysdata['sqltxt'] = 'SYS_SEC_USR_BUS_DEF (?,?,?,?,?,?)';
