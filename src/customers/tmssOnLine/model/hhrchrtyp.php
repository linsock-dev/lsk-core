<?php
final class hhrchrtyp extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'hhrchrtypcod';
  ...
  function save( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
		...
		$lp_dat['hhrchrtypatr'] = '<slrasg>'.($lp_dat['hhrchrtypatrslrasg']??'').'</slrasg>'.
															'<tmeasg>'.($lp_dat['hhrchrtypatrtmeasg']??'').'</tmeasg>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'hhrchrtypcodext'), 
																			$this->co_reg->db->sqldat($lp_in,'hhrchrtyptxt'), 
																			$this->co_reg->db->sqldat($lp_in,'hhrchrclscod'), 
																			$this->co_reg->db->sqldat($lp_in,'hhrchrtypatr',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'hhrchrtypcmt',false)
																		);
		$this->sysdata['sqltxt'] = 'HHR_CHR_TYP_DEF (?,?,?, ?,?,?,?,?,?,?,?)';
