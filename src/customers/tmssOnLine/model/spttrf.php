<?php
final class spttrf extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_authCheck=true ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->errcod = 0;
		$this->errtxt = '';
    $lv_buscod = (isset($this->sysdata['buscod'])?$this->sysdata['buscod']:$this->co_reg->sec->buscod);
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $lv_buscod, 
                                     $this->co_reg->db->sqldat($lp_in,'spttrfcod'),
                                     $this->co_reg->db->sqldat($lp_in,'spttrfcodext'),
                                     $this->co_reg->db->sqldat($lp_in,'spttrftxt'),
                                     $this->co_reg->db->sqldat($lp_in,'spttrftypcod'),
                                     $this->co_reg->db->sqldat($lp_in,'spttrfvercod'),
                                     $this->co_reg->db->sqldte($lp_in,'spttrfverstrdte'),
                                     $this->co_reg->db->sqldte($lp_in,'spttrfverenddte'),
                                     $this->co_reg->db->sqldat($lp_in,'docsts'),
                                     $this->co_reg->db->sqldat($this->sysdata,'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'SPT_TRF_DEF (?,?,?,?,?,?,?,?,?,?,?,?)';
    ...
    if(($lp_action=='03') || ($lp_action=='13') && $this->errcod == 0){
      ...
      $lv_prm = array('vewfldflt' =>'[~fltrow~]tl.spttrfvercod'.chr(9).'='.chr(9).chr(9).$this->co_reg->db->sqldata($lp_out['spttrfvercod']).chr(9).chr(9).
																			'[~fltrow~]tl.spttrfcod'.chr(9).'='.chr(9).chr(9).$this->co_reg->db->sqldata($lp_out['spttrfcod']).chr(9).chr(9));
      $lp_out['spttrflst'] = $lo_trflstmdl->getList($lv_prm);
    }
