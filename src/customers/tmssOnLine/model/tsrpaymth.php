<?php
final class tsrpaymth extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $lp_dat['tsrpaymthatr']='<tsrtou>'.$lp_dat['tsrpaymthatrtsrtou'].'</tsrtou><tsrtin>'.$lp_dat['tsrpaymthatrtsrtin'].'</tsrtin><tsrtid>'.$lp_dat['tsrpaymthatrtsrtid'].'</tsrtid>';
    $this->data = $lp_dat;
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array() ) {
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'paymthcodext'), 
																			$this->co_reg->db->sqldat($lp_in,'paymthtxt'),
                                     	$this->co_reg->db->sqldat($lp_in,'paymthtyp'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
                                     	$this->co_reg->db->sqldat($lp_in,'tsrpaymthatr',false)
																		);
    $this->sysdata['sqltxt'] = 'TSR_PAY_MTH_DEF (?,?,?,?,?,?,?,?,?,?)';
