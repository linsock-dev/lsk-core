<?php
final class syssecdrtgrpasg extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  // GET USER DIRECTIVES. devuelve las directivas asignadas al usuario (por empresa, usuario o rol)
  function getUserDirectives( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ){
    if($lo_vew==null){ $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
    $lo_out_data = array();
  ...
  //  CALL SP. llamada a base de datos
    ...
    $lp_in['buscod'] = ( !isset( $lp_in['buscod'] ) || $lp_in['buscod'] == '' ? (isset($lp_in['srcobjtyp'])?($lp_in['srcobjtyp']!='SYS_GRP'?'**':$this->co_reg->sec->buscod):$this->co_reg->sec->buscod) : $lp_in['buscod'] );
    $lv_usrcod = ( isset($lp_in['usrcod']) && $lp_in['usrcod'] != '' ? $lp_in['usrcod'] : $this->co_reg->sec->usrcod );
    $this->sysdata['sqlprm'] = array( $lp_action, $lv_usrcod, $lp_in['buscod'], 
																			$this->co_reg->db->sqldat($lp_in,'syssecdrtgrpasgcod'), 
		...
		$this->sysdata['sqltxt'] = 'SYS_SEC_DRT_GRP_ASG_DEF (?,?,?,?,?,?,?,?,?)';
