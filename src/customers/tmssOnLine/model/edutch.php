<?php
final class edutch extends tmssAction2 {
  const OBJTYP = 'EDU_TCH';
  function initialize(){ $this->ID = 'tchcod'; }
	
	// CREATE. inicializa el objeto
	function create() {
		parent::create();
		$this->data['tchprf'] = array();
	...
	function save($lp_dat=array(), $lp_authCheck=true ) {
		...
		$lp_dat['adrnme001'] = $lp_dat['tchtxt'];
	...
	protected function call_sp( $lp_action, $lp_in, &$lp_out ) {
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'tchcnthrs'),
																			$this->co_reg->db->sqldat($lp_in,'tchcmt'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($lp_in,'tchcodext'),    
																			$this->co_reg->db->sqldat($lp_in,'lndtwngrpcod'),
																			$this->co_reg->db->sqldat($lp_in,'tchprn'),
																			$this->co_reg->db->sqldat($lp_in,'tchtxt'),
																			$this->co_reg->db->sqldte($lp_in,'tchdte'),
																			$this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
																			$this->co_reg->db->sqldat($lp_in,'tchniknme'),
																			$this->co_reg->db->sqldte($lp_in,'tchinbdte'),
																			$this->co_reg->db->sqldte($lp_in,'tchoutdte'),
																			$this->co_reg->db->sqldat($lp_in,'tchoutcmt'),
																			$this->co_reg->db->sqldat($lp_in,'tchnat'),
																			$this->co_reg->db->sqldte($lp_in,'tchbrndte'),
																			$this->co_reg->db->sqldat($lp_in,'tchttl'),
																			$this->co_reg->db->sqldat($lp_in,'tchothstd'),
																			$this->co_reg->db->sqldat($lp_in,'tchjobhst'),
																			$this->co_reg->db->sqldat($lp_in,'catprccod'),
																			$this->co_reg->db->sqldat($lp_in,'sysdocclscod')
																		);
		$this->sysdata['sqltxt'] = 'EDU_TCH_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
    ...
    $lv_fndval = in_array($lp_action,$lv_actarr);
		...
		if($lp_action=='03' || $lp_action=='13') {
			...
			$lv_prm = array('vewfldflt' =>'[~fltrow~]tp.tchcod'.chr(9).'='.chr(9).chr(9).$this->data[$this->ID].chr(9).chr(9));
			$lp_out['tchprf'] = $lo_tchprf->getList($lv_prm);
		}
		...
		if ( $lv_fndval && $this->errcod==0 ) {
			...
			if ($lp_action=='03' || $lp_action=='13'){
				...
				$lp_out['taxnum']=$lo_taxmdl->taxnum;
			}
		}
		...
		if ( $lv_fndval && $this->errcod==0 ) {
			...
			if ($lp_action=='03' || $lp_action=='13') {
				...
				$lp_out['bnknum']=$lo_bnkmdl->bnknum;
			}
		}
