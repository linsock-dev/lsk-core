<?php
final class finsumdoc extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $lv_buscod,
                                     	$this->co_reg->db->sqldat($lp_in,'finsumcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'finsumdoccod'),
                                     	$this->co_reg->db->sqldat($lp_in,'docobjtyp'),
                                     	$this->co_reg->db->sqldat($lp_in,'docobjcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'docobjcodext'),
                                     	$this->co_reg->db->sqldte($lp_in,'docobjdte'),
                                     	$this->co_reg->db->sqldte($lp_in,'docobjduedte'),
																			$this->co_reg->db->sqldat($lp_in,'refobjtyp'), 
																			$this->co_reg->db->sqldat($lp_in,'refobjcod'), 
																			$this->co_reg->db->sqldat($lp_in,'refobjtxt'), 
																			$this->co_reg->db->sqldat($lp_in,'impobjtyp'), 
                                     	$this->co_reg->db->sqldat($lp_in,'impobjcod'),
                                     	$this->co_reg->db->sqlnum($lp_in,'finsumdoctot'),
                                     	$this->co_reg->db->sqlnum($lp_in,'finsumdoctotrst'),
                                     	$this->co_reg->db->sqldat($lp_in,'curcod'),
                                    	$this->co_reg->db->sqlnum($lp_in,'curexcrte',5),
                                    	$this->co_reg->db->sqldat($lp_in,'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata,'view_options',false),
                                    );
		$this->sysdata['sqltxt'] = 'FIN_SUM_DOC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
