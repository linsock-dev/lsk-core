<?php
final class sysappapi extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array() ) {
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, 
																			$this->co_reg->db->sqldat($lp_in,'sysappapicod'),
                                     	$this->co_reg->db->sqldat($lp_in,'sysappapicodext'), 
																			$this->co_reg->db->sqldat($lp_in,'sysappapitxt'),
																			$this->co_reg->db->sqldat($lp_in,'sysappapiver'), 
                                     	$this->co_reg->db->sqldat($lp_in,'sysappapiurl'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'sysappapicmt',false)
																		);
    $this->sysdata['sqltxt'] = 'SYS_APP_API_DEF (?,?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' ) {
      ...
      if ( $lo_rs && count($lo_rs)>0 ) {
		...
		// recupera el mapeo de apis
    if ( $lp_action=='03' && $this->errcod==0 ) {
      $lo_sysappapimapmdl = $this->co_reg->load->model('sysappapimap');
      $lv_prm=array('vewfldflt' => 	'[~fltrow~]m.sysappapicod'.chr(9).'='.chr(9).chr(9).$lp_in[self::ID].chr(9).chr(9).
                                    '[~fltrow~]m.docsts'.chr(9).'='.chr(9).chr(9).'A'.chr(9).chr(9));
			$lp_out['apimap'] = $lo_sysappapimapmdl->getList( $lv_prm );
		}
