<?php
final class hhrorgchtwrkemp extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'hhrorgchtwrkempcod';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'hhrorgchtwrkempcod'), 
																			$this->co_reg->db->sqldat($lp_in,'hhrorgchtwrkcod'), 
																			$this->co_reg->db->sqldat($lp_in,'hhremptmecod'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HHR_ORG_CHT_WRK_EMP_DEF (?,?,?,?,?,?,?,?, null, null, null)';
