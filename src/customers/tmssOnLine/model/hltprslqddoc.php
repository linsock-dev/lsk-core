<?php
final class hltprslqddoc extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'hltprslqddoccod';	
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'hltprslqddoccod'), 
																			...
																			$this->co_reg->db->sqldat($lp_in,'hltprslqddoccodext'), 
																			$this->co_reg->db->sqldat($lp_in,'hltprslqddoctxt'), 
																			$this->co_reg->db->sqlnum($lp_in,'hltprslqddoctot'),
																			$this->co_reg->db->sqldat($lp_in,'curcod'), 
																			$this->co_reg->db->sqldat($lp_in,'hltprslqddocatr001',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_PRS_LQD_DOC_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
