<?php
final class spttrflst extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'spttrflstcod';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null, $lp_authCheck=true ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     $this->co_reg->db->sqldat($lp_in,'spttrflstcod'),
                                     $this->co_reg->db->sqldat($lp_in,'spttrfcod'),
                                     $this->co_reg->db->sqldat($lp_in,'spttrfvercod'),    
                                     $this->co_reg->db->sqlnum($lp_in,'spttrfminval'),
                                     $this->co_reg->db->sqlnum($lp_in,'spttrfmaxval'),
                                     $this->co_reg->db->sqlnum($lp_in,'spttrfprc'),         
                                     $this->co_reg->db->sqldat($lp_in,'docsts'),
                                     $this->co_reg->db->sqldat($this->sysdata,'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'SPT_TRF_LST_DEF (?,?,?,?,?,?,?,?,?,?,?)';
