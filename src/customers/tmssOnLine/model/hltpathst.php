<?php
final class hltpathst extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  // GET LIST. devuelve recordset de objetos (detalle)
  function getDetail( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if ($lo_vew==null) {$lo_vew = $this->co_reg->load->model('grlvew');}
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
		if ( $this->call_sp( '03', $lp_prm, $lo_out_data ) ) {
			$this->data = $lo_out_data;
		} else {
      $this->sysdata['sqlerr'] = $this->co_reg->db->getLastErrorMessage();
		}
   return $this->data;
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'movid'), 
																			$this->co_reg->db->sqldat($lp_in,'patcod'), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'movdtestr'), 
																			$this->co_reg->db->sqldat($lp_in,'movdteend'),
																			$this->co_reg->db->sqldat($lp_in,'spccod'),
																			$this->co_reg->db->sqldat($lp_in,'hltplnid'),
																			$this->co_reg->db->sqldat($lp_in,'hltplndteid'),
																			$this->co_reg->db->sqldat($lp_in,'MovTyp'),
																			$this->co_reg->db->sqldat($lp_in,'MovRefNum'),
																			$this->co_reg->db->sqldat($lp_in,'MatCod'),
																			$this->co_reg->db->sqldat($lp_in,'MatQty'),
																			$this->co_reg->db->sqldat($lp_in,'MatUntCod'),
																			$this->co_reg->db->sqldat($lp_in,'BchCod'),
																			$this->co_reg->db->sqldat($lp_in,'SerCod'),
																			$this->co_reg->db->sqldat($lp_in,'movcmt',false)
																		);
		$this->sysdata['sqltxt'] = 'HLT_PAT_HST_DEF (?,?,?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
