<?php
final class sysdocfrmfldatr extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'sysdocfrmfldatrcod';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,
																			$this->co_reg->db->sqldat($lp_in, 'sysdocfrmfldatrcod'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocfrmfldatrcodext'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocfrmfldatrtxt'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocfrmfldatrelm'),
                                      $this->co_reg->db->sqldat($lp_in, 'sysdocfrmfldatrobj'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocfrmfldatrtgt'),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysdocfrmfldatrval'),
                                     	(isset($lp_in['sysdocfrmfldatrrpl'])?$lp_in['sysdocfrmfldatrrpl']:''),
                                     	$this->co_reg->db->sqldat($lp_in, 'sysfld'),
                                     	$this->co_reg->db->sqldat($lp_in, 'docsts'),
                                     	$this->co_reg->db->sqldat($this->sysdata, 'view_options')
																		);
		$this->sysdata['sqltxt'] = 'SYS_DOC_FRM_FLD_ATR_DEF (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
