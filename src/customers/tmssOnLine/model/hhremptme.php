<?php
final class hhremptme extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'hhremptmecod';
  const SRCTYP = 'HHR_EMP_TME';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'hhremptmecod'),
																			$this->co_reg->db->sqldat($lp_in,'hhrempcod'),
																			$this->co_reg->db->sqldte($lp_in,'hhremptmestr'),
																			$this->co_reg->db->sqldte($lp_in,'hhremptmeend'),
                                     	...
                                     	$this->co_reg->db->sqlnum($lp_in,'wrkstepft', 0)
																		);
		$this->sysdata['sqltxt'] = 'HHR_EMP_TME_DEF (?,?,?,?,?,?,?,?,?,?,?,?,null,null,null,?)';
