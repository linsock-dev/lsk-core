<?php
final class crmcntmtvsts extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'crmcntmtvstscod';
  ...
  function save( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
		...
		$lp_dat['crmcntmtvstsatr'] ='<usrasg>'.(isset($lp_dat['crmcntmtvstsusrasg'])?$lp_dat['crmcntmtvstsusrasg']:'').'</usrasg>'.
																'<usrres>'.(isset($lp_dat['crmcntmtvstsusrres'])?$lp_dat['crmcntmtvstsusrres']:'').'</usrres>';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,$this->co_reg->sec->buscod,
                                     	$this->co_reg->db->sqldat($lp_in,'crmcntmtvstscod'),
                                     	$this->co_reg->db->sqldat($lp_in,'crmcntmtvcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'crmcntstscodstr'),
                                     	$this->co_reg->db->sqldat($lp_in,'crmcntstsusrcod'),
                                     	$this->co_reg->db->sqldat($lp_in,'crmcntstscodend'),
                                     	$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
                                     	$this->co_reg->db->sqldat($lp_in,'crmcntmtvstsatr'),
																		);
		$this->sysdata['sqltxt'] = 'CRM_CNT_MTV_STS_DEF (?,?,?,?,?,?,?,?,?,?,?)';
