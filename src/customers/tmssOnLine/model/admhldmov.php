<?php
final class admhldmov extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'hldmovcod';
  const SRCTYP = 'ADM_HLD_MOV';
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'hldmovcod'),
                                      $this->co_reg->db->sqldat($lp_in,'hldcod'),
                                      $this->co_reg->db->sqldte($lp_in,'hldmovday'), 
                                      $this->co_reg->db->sqldat($lp_in,'hldmovtxt'), 
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false)
																		);
		$this->sysdata['sqltxt'] = 'ADM_HLD_MOV_DEF (?,?,?,?,?,?,?,?,?)';
