<?php
final class tsrbnkchk extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'bnkchkcod';
  const OBJTYP = 'TSR_BNC';
  ...
  function create() {
		$this->data = array();
		$this->sysdata = array();
		$this->lo_bnkmdl = ($this->lo_accmdl==null?$this->co_reg->load->model('grldatacc'):$this->lo_accmdl);
    $this->data['acc'] = $this->lo_accmdl;
	}
  ...
  //  CALL SP. llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $lv_buscod,
                                     	...
                                     	$this->co_reg->db->sqlnum($lp_in,'bnkchkfrtnum'),
                                     	$this->co_reg->db->sqlnum($lp_in,'bnkchkqty'),
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
                                     	$this->co_reg->db->sqlnum($lp_in,'bnkchkusdnum'),
                                      $this->co_reg->db->sqldat($this->sysdata,'view_options',false),
                                    );
		$this->sysdata['sqltxt'] = 'TSR_BNK_CHK_DEF (?,?,?,?,?,?,?,?,?,?)';
