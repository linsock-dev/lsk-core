<?php
final class spttch extends tmssAction2 {
    ...
    parent::create();
		...
		$this->data['spttchact'] = $this->co_reg->load->model('spttchact');
	}
  ...
  function save( $lp_dat=array(), $lp_authCheck=true ) {
	...
	protected function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqltxt'] = 'SPT_TCH_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?)';
    ...
    //		A C T
		if ( ($lp_action=='03' || $lp_action=='13') && $this->errcod==0 ) {
			...
			$lv_prm = array('vewfldflt' =>'[~fltrow~]ta.tchcod'.chr(9).'='.chr(9).chr(9).$this->data[$this->ID].chr(9).chr(9));
			$lp_out['spttchact'] = $lo_tchactmdl->getList($lv_prm);
		}
