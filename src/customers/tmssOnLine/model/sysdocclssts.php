<?php
final class sysdocclssts extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $lp_dat['sysdocclsstsatr'] = '<stsord>'.($lp_dat['sysdocclsstsord']??'').'</stsord>'.
																'<stsman>'.($lp_dat['sysdocclsstsman']??'').'</stsman>';
    $this->data = $lp_dat;
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'sysdocclsstscod'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdocclscod'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdocstscod'), 
																			$this->co_reg->db->sqldat($lp_in,'sysdocclsstsatr',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'SYS_DOC_CLS_STS_DEF (?,?,?,?,?,?,?,?,?)';
