<?php
final class sysdocchg extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'chgdoccod';
  const SRCTYP = 'SYS_CHG';
  ...
  // UPDATE ATRIBUTE. graba un atributo
  function updateAttibute( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
    $this->data = $lp_dat;
		$lo_out_data = array();
		return $this->call_sp( '12', $this->data, $lo_out_data );
  }
	
  // GETDETAIL. devuelve recordset de objetos (detalle)    
  function getDetail( $lp_prm=array() ) {
		$lo_out_data = array();
  ...
  // GETDETAIL. devuelve recordset de objetos (detalles)    
  function getVariousDetails( $lp_vewopt ) {
    $lo_vew = $this->co_reg->load->model('grlvew');
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
		if ( $this->call_sp( '14', array(), $lo_out_data ) ) {
			$this->data = $lo_out_data;
		} else {
      $this->sysdata['sqlerr'] = $this->co_reg->db->getLastErrorMessage();
		}
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                    	$this->co_reg->db->sqldat($lp_in,'chgdoccod'),
																			$this->co_reg->db->sqldat($lp_in,'chgdocsrctyp'), 
                                    	$this->co_reg->db->sqldat($lp_in,'chgdocsrccod'), 
                                      $this->co_reg->db->sqldat($lp_in,'docsts'),
                                      $this->co_reg->db->sqldat($this->sysdata, 'view_options', false),
																			$this->co_reg->db->sqldat($lp_in,'chgdocatr',false),
                                      $this->co_reg->db->sqldat($lp_in,'chgdocatrcod')
																		);
		$this->sysdata['sqltxt'] = 'SYS_DOC_CHG_DEF (?,?,?,?,?,?,?,?,?,?)';
    ...
    if ( $lp_action=='08' || $lp_action=='13' || $lp_action=='14') {
      if($this->errtyp!='E'){
				$lp_out = $lo_rs;
      }else{
