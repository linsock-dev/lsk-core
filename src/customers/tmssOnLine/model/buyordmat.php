<?php
final class buyordmat extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'buyordmatcod';
  const SRCTYP = 'BUY_ORD_MAT';                                                                                                                           // **************************************
  ...
  function save( $lp_dat=array() ) {
    ...
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		...
		$this->data['buyordmatatr'] = '<buyordmatbuytxt>'.(isset($this->data['buyordmatbuytxt'])?utf8_encode(htmlentities($this->data['buyordmatbuytxt'])):'').'</buyordmatbuytxt>';
		$lo_out_data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'buyordmatcod'),
                                     	...
                                     	$this->co_reg->db->sqldat($lp_in,'buyordmatatr', false),
		...
		$this->sysdata['sqltxt'] = 'BUY_ORD_MAT_DEF (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
