<?php
final class syssecusr extends tmssAction {
  protected $co_reg;
  private $data = array();
	private $sysdata = array();
	const ID = 'usrcod';
  const SRCTYP = 'SYS_USR';
	
  function __construct( &$lp_reg ) { $this->co_reg = $lp_reg; }
	function __get( $lp_key ) { return ( isset($this->data[$lp_key]) ? $this->data[$lp_key] : '' ); }
	function get( $lp_key ) { return ( isset($this->data[$lp_key]) ? $this->data[$lp_key] : '' ); }
	function getData(){ return $this->data; }
	function getsysdata( $lp_key ) { return ( isset($this->sysdata[$lp_key]) ? $this->sysdata[$lp_key] : '' ); }
	function __set( $lp_key, $lp_val ) { $this->data[$lp_key] = $lp_val; }
	function set( $lp_key, $lp_val ) { $this->data[$lp_key] = $lp_val; }
	function setData( $lp_dat=array() ) { $this->data = $lp_dat; }
	
	// CREATE. inicializa el objeto
	function create() {
		$this->data = array();
		$this->data['adr'] = $this->co_reg->load->model('grldatadr');
		$this->sysdata = array();
	}
	
  // SAVE. graba el objeto
  function save( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
    $lp_dat['adrnme001'] = ($lp_dat['usrtxt']??'');
    $this->data = $lp_dat;
    $lv_act = ( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		
		$this->data['usratr001']= '<strpge>'.($this->data['strpge']??'').'</strpge>'.
															'<debmod>'.($this->data['debmod']??'').'</debmod>'.
															'<wndsty>'.($this->data['wndsty']??'').'</wndsty>';
		
		if (strpos($this->data['usrcod'],'@')==true) {
			$this->errcod = -1;
			$this->errtxt = 'El codigo de usuario no puede contener el simbolo @.';
			return false;
		}

		$lo_out_data = array();
		return $this->call_sp( $lv_act, $this->data, $lo_out_data );
  }
  
  // LOAD. carga el objeto
  function load( $lp_key=array() ) {
		return $this->call_sp( '03', $lp_key, $this->data );
  }
	
  // DELETE. borra objeto
  function delete( $lp_dat=array() ) {
    if ( count($lp_dat)==0 ) { $lp_dat = $this->co_reg->request->post; }
    $this->data = $lp_dat;
		return $this->call_sp( '04', $this->data, $this->data );
  }
	
  // GET LIST. devuelve recordset de objetos
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
    if ($lo_vew==null) { $lo_vew = $this->co_reg->load->model('grlvew'); }
		$this->sysdata['view_options'] = $lo_vew->parseViewOptions($lp_vewopt);
		$lo_out_data = array();
		if ( $this->call_sp( '08', $lp_prm, $lo_out_data ) ) {
			$this->data = $lo_out_data;
		} else {
      $this->sysdata['sqlerr'] = $this->co_reg->db->getLastErrorMessage();
		}
    return $this->data;
  }
	
	// CHANGEPASSWORD. modifica la contrase�a de un usuario
	function changePassword( $lp_prm=array() ) {
		return $this->call_sp( '10', $lp_prm, $this->data );
	}
	
	// UNLOCK. desbloquea el usuario
	function unlock( $lp_prm=array() ) {
		return $this->call_sp( '17', $lp_prm, $this->data );
	}

	// checkUserLogin. verifica si el usuario/contraseña es válido
	function checkUserLogin( $lp_prm=array() ) {
		if ( strpos($lp_prm['usrcod'],'@')===false ) {
			// logueo con codigo de usuario
		} else {
			// logueo con email
			$lp_prm['usratr001'] = '<adreml>'.$lp_prm['usrcod'].'</adreml>';
			$lp_prm['usrcod'] = '';
			// logueo con Facebook
			if( (isset($lp_prm['lgnfcb'])?$lp_prm['lgnfcb']:'')=='1' && $lp_prm['usrpwd']==hash('sha256','Facebook') ){
				$lp_prm['usratr001'].='<lgnfcb>1</lgnfcb>';
			}
		}
		
		$lp_prm['usratr002'] = '';
		$lo_brwdat = get_browser(null, true);
		foreach($lo_brwdat as $lv_key=>$lv_val){ $lp_prm['usratr002'] .= '<'.$lv_key.'>'.$this->co_reg->db->sqldata($lv_val).'</'.$lv_key.'>'; }
		$lp_prm['usratr003'] =(isset($_SERVER['HTTP_CLIENT_IP'])?$_SERVER['HTTP_CLIENT_IP']:
													(isset($_SERVER['HTTP_X_FORWARDED_FOR'])?$_SERVER['HTTP_X_FORWARDED_FOR']:
													(isset($_SERVER['HTTP_X_FORWARDED'])?$_SERVER['HTTP_X_FORWARDED']:
													(isset($_SERVER['HTTP_X_CLUSTER_CLIENT_IP'])?$_SERVER['HTTP_X_CLUSTER_CLIENT_IP']:
													(isset($_SERVER['HTTP_FORWARDED_FOR'])?$_SERVER['HTTP_FORWARDED_FOR']:
													(isset($_SERVER['HTTP_FORWARDED'])?$_SERVER['HTTP_FORWARDED']:
													(isset($_SERVER['REMOTE_ADDR'])?$_SERVER['REMOTE_ADDR']:
													'UNKNOWN')))))));
		
		$lo_out_data = array();
		if ( $this->call_sp( '11', $lp_prm, $lo_out_data ) ) {
			$this->data['usrcod'] = $lo_out_data['usrcod'];
			$this->data['usrtxt'] = $lo_out_data['usrtxt'];
			$this->data['usratr001'] = $lo_out_data['usratr001'];	
			$this->data['pwddays'] = $lo_out_data['pwddays'];		
			$this->data['usrpwdchg'] = $lo_out_data['usrpwdchg'];		
			$this->data['usrsysacc'] = $lo_out_data['usrsysacc'];		
			$this->data['acctkn'] = $lo_out_data['acctkn']??'';		
			return true;
		} else {
			$this->data['data_sqlprm'] = $this->sysdata['sqlprm'];
			$this->data['data_sqltxt'] = $this->sysdata['sqltxt'];
			$this->data['data_sqlerr'] = $this->co_reg->db->getLastErrorMessage();
			return false;
		}
	}
	
	//  CALL SP.  llamada a base de datos
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		$this->errtyp = 'S';
		$this->errcod = 0;
		$this->errtxt = '';	
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod,  
																			$this->co_reg->db->sqldat($lp_in,'usrcod'), 
																			$this->co_reg->db->sqldat($lp_in,'usrtxt'), 
																			$this->co_reg->db->sqldat($lp_in,'usrpwd',false), 
																			$this->co_reg->db->sqldat($lp_in,'lngcod'), 
																			$this->co_reg->db->sqldat($lp_in,'adrcnthrs'),
																			$this->co_reg->db->sqlnum($lp_in,'usrwndrfhtme'),
																			$this->co_reg->db->sqldat($lp_in,'usrwndsty'),
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'usratr001',false),
																			$this->co_reg->db->sqldat($lp_in,'usratr002',false),
																			$this->co_reg->db->sqldat($lp_in,'usratr003',false),
																			$this->co_reg->db->sqldat($lp_in,'usrsysacc')
																		);
		$this->sysdata['sqltxt'] = 'SYS_SEC_USR_DEF (?,?, ?,?,null,?,?,?,?,?,null,?,?,?,?,?,?)';
		$this->sysdata['sqlstm'] = $this->co_reg->db->getSqlStatement($this->sysdata['sqltxt'],$this->sysdata['sqlprm']);
		$lo_rs = $this->co_reg->db->sqlstoredprocedure( $this->sysdata['sqltxt'] , $this->sysdata['sqlprm'] );

    // validacion [errcod,errtyp,errmsg-errvar(chr9),errtxt]
    $this->errcod = (intval($lo_rs[0]['errcod']??0));
    $this->errtyp = ($lo_rs[0]['errtyp']??($this->errcod<0?'E':'S'));
    $this->errtxt = (isset($lo_rs[0]['errmsg'])?$this->co_reg->language->message( $lo_rs[0]['errmsg'], explode(chr(9),($lo_rs[0]['errvar']??'')) ) : ($lo_rs[0]['errtxt']??'') );
	
    // devuelve recordset (segun la operacion)
    if ( $lp_action=='08' ) {
      if($this->errtyp!='E'){
				$lp_out = $lo_rs;
      }else{
        $lp_out=array('errtyp'=>$this->errtyp, 'errcod'=>$this->errcod, 'errtxt'=>$this->errtxt);
      }
			
		// devuelve ID (registro individual)
		} else {
      if ( $lo_rs && count($lo_rs)>0 ) {
        if( $this->errtyp!='E' ){
					$lp_out = $lo_rs[0];
					$this->data[self::ID] = ($lp_out[self::ID]??array_values($lp_out)[0]);
        }
				$this->errcod = intval($this->errcod);
			} else {
				$this->errtyp = 'E';
				$this->errcod = -9999;
				$this->errtxt = $this->co_reg->language->message('UnexpectedError', $this->sysdata['sqlstm'] );
			}
		}
		
		//		A D R
		if ( ($lp_action=='01' || $lp_action=='02' || $lp_action=='03') && $this->errcod==0 ) {
			$lo_grladr = $this->co_reg->load->model('grldatadr');
			$lp_in['sysecusr_buscod'] = '_SYS';
			$lp_in['adrsrctyp'] = self::SRCTYP;
			$lp_in['adrsrccod'] = $this->data[self::ID];
			if ($lp_action=='03') {
				$lo_grladr->load( $lp_in );
				$lp_out['adr'] = $lo_grladr;
			} else if ( $lo_grladr->save( $lp_in )==false ) {
				$this->errcod = $lo_grladr->errcod;
				$this->errtxt = $lo_grladr->errtxt;
			}
		}
		
		return ($this->errcod==0?true:false);
	}  
}
?>