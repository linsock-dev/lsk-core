<?php
final class syssecper extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lp_dat=array() ) {
    ...
    $this->data = $lp_dat;
    $lv_act = '02'; //( isset($lp_dat[self::ID]) && !empty($lp_dat[self::ID]) ? '02' : '01' );
		$lo_out_data = array();
  ...
  // GET USER PERMISSIONS. get list of objects
  function getUserPermissions( $lp_vewopt=array(), $lp_prm=array() ) {
		$lo_out_data = array();
    ...
    $this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, 
																			...
																			$this->co_reg->db->sqldat($lp_in,'oprcod'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false),
																			$this->co_reg->db->sqldat($lp_in,'usrperatr001',false),
																			$this->co_reg->db->sqldat($lp_in,'usrperatr002',false)
																		);
    $this->sysdata['sqltxt'] = 'SYS_SEC_PER_DEF (?,?,?,?,?,?,?,?,?,?,?)';
