<?php
final class grlrptcol extends tmssAction {
  protected $co_reg;
  private $data = array();
  ...
  function save( $lo_dat=array() ) {
    if(count($lo_dat)==0) { $lo_dat = $this->co_reg->request->post; }
    $this->data = $lo_dat;
    $lv_act = ( isset($lo_dat[self::ID]) && !empty($lo_dat[self::ID]) ? '02' : '01' );
		$lo_out_data = array();
  ...
  function getList( $lp_vewopt=array(), $lp_prm=array(), $lo_vew=null ) {
	...
	private function call_sp( $lp_action, $lp_in, &$lp_out ) {
		...
		$this->sysdata['sqlprm'] = array( $lp_action, $this->co_reg->sec->usrcod, $this->co_reg->sec->buscod, 
																			$this->co_reg->db->sqldat($lp_in,'rptcolcod'), 
																			$this->co_reg->db->sqldat($lp_in,'rptcolcodext',false), 
																			$this->co_reg->db->sqldat($lp_in,'rptcoltxt',false), 
																			$this->co_reg->db->sqldat($lp_in,'rptcod'), 
																			$this->co_reg->db->sqldat($lp_in,'rptsrccolcod'), 
																			$this->co_reg->db->sqldat($lp_in,'rptcolatr',false), 
																			$this->co_reg->db->sqldat($lp_in,'docsts'),
																			$this->co_reg->db->sqldat($this->sysdata,'view_options',false)
																		);
		$this->sysdata['sqltxt'] = 'GRL_RPT_COL_DEF (?,?,?,?,?,?,?,?,?,?,?)';
