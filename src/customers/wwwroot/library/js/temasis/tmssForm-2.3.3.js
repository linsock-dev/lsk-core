			...
			lp_obj.typeahead({
				onSelectAjaxData: function(data) {
					...
					for(var lv_key in lv_json_asg) {
						...
						//var lv_data_val = (eval( "data.data."+lv_json_asg[lv_key] ) == undefined ? eval( "data.data."+lp_data.fldasg[lv_json_asg[lv_key]] ) : eval( "data.data."+lv_json_asg[lv_key] ) );
				...
				ajax: {
					...
					preDispatch: function(query){
						...
						for(var key in lv_keys){
							...
							//} else if( lv_prm[ lv_keys[key] ] instanceof jQuery ) {
							} else if( lp_data.fldflt[ lv_keys[key] ] instanceof jQuery ) {
								//lv_pre.push( {name:"prm_"+lv_fldnme_no_alias, value:$(lv_prm[lv_fldnme]).val()} );
								lv_pre.push( {name:"prm_"+lv_fldnme_no_alias, value:$(lp_data.fldflt[lv_fldnme]).val()} );
							} else {
								//lv_pre.push( {name:"prm_"+lv_fldnme_no_alias, value:lv_prm[lv_fldnme]} );
				...
				if( $(this).prop("value")=="" ){  
					...
					for(var lv_key in lv_json_asg) {
						$("#"+lp_data.fldsec+" #"+lv_json_asg[lv_key]).prop("value", "");
					}
				}
			})
		});
 ...
 // pop up case
	...
	if(lp_data.popup){
		...
		lv_span.on("click", function(e) { e.preventDefault();
			...
			tmssPopup(lv_ttl, lv_url.substr(0, lv_url.indexOf("&") )+"&prm_vewcod="+lv_vew+"&prm_popup=sysdochdr_popup&prm_fldsec="+lp_data.fldsec+"&prm_fldflt="+lv_flt+"&prm_fldasg="+lv_asg, ( typeof lp_callback.afterAssign != "undefined" ? lp_callback.afterAssign : function(){} ) );
		});
	}else{
...
function tmssCallProcess( lp_url, lp_dat, lp_callback ) {
	showBackdrop();
	$.ajax({url:lp_url,method:"POST",data:lp_dat}).done(function(data){
		hideBackdrop();
...
function tmssCallProcessFile( lp_url, lp_dat, lp_callback ) {
	showBackdrop();
	$.ajax({url:lp_url,method:"POST",data:lp_dat,contentType:false,processData:false,cache:false}).done(function(data){
		hideBackdrop();
		if (typeof(data)==="string") {
			if (data.substring(0,10)=="/*script*/") {
				eval(data);
