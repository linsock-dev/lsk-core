		...
		tmssLoadScript("typeahead",function() {
			lp_obj.typeahead({
				...
				ajax: {
					...
					valueField: lv_txt,
					triggerLength: 1,
          headers:  {'Tmss-Usrcod': localStorage.getItem("Tmss-Usrcod"),
                     'Tmss-Usrtkn': localStorage.getItem("Tmss-Usrtkn")},
					method: "get",
...
function tmssCallProcess( lp_url, lp_dat, lp_callback ) {
	showBackdrop();
  const lv_headers = {"Tmss-Usrcod": localStorage.getItem("Tmss-Usrcod"),
                     "Tmss-Usrtkn": localStorage.getItem("Tmss-Usrtkn")};
...
function tmssCallProcessFile( lp_url, lp_dat, lp_callback ) {
  ...
  const lv_headers = {"Tmss-Usrcod": localStorage.getItem("Tmss-Usrcod"),
                     "Tmss-Usrtkn": localStorage.getItem("Tmss-Usrtkn")};
	$.ajax({url:lp_url,method:"POST",data:lp_dat, headers: lv_headers,contentType:false,processData:false,cache:false}).done(function(data){
		hideBackdrop();
		if (typeof(data)==="string") {
			if (data.substring(0,10)=="/*script*/") {
				eval(data);
...
//
// ---------------------------------
function tmssCallProcessBlob( lp_url, lp_dat, lp_callback ) {
	showBackdrop();
	...
	try {
		...
		req.setRequestHeader("Tmss-Usrcod", localStorage.getItem("Tmss-Usrcod"));
		req.setRequestHeader("Tmss-Usrtkn", localStorage.getItem("Tmss-Usrtkn"));
		req.responseType = "blob";
		req.onload = function (res) {
			hideBackdrop();
			lp_callback( req.response );
		}
		// prepara datos POST
