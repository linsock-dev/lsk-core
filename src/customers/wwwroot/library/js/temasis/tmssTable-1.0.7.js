...
class tmssTable {
      ...
      this.#obj.find(".tmss-tr-add .tmss-table-add-row-btn").click((e)=>{
        var lv_addrow = $(e.currentTarget).parents("tr"); 
          ...
          if($(th).find("input").is(":checkbox")){
            lv_newdat[this.#colarr[j]] = $(th).find("input").is(":checked")? 1 : 0;
          }else if($(th).find("input").val() != undefined && $(th).find("input").val().trim().length){
            lv_newdat[this.#colarr[j]] = $(th).find("input").val().trim().length;
          }
  ...
  #setTypeahead(lp_ind, lp_prop, lp_cell, lp_cfg){ 
    var lv_asgarr = [];
    var lv_cfg = lp_cfg.apply({row: lp_ind, prop: lp_prop, cell: lp_cell}, [lp_cell.is("td")?this.getValuesAtRow(lp_ind):this.#addval]);
  ...
  getValuesAtRow( lp_ind ){  
    return (lp_ind >= 0 && lp_ind < this.#rowind.length ? Object.assign({},this.#dat.get(this.#rowind[lp_ind])) : {});
  }
